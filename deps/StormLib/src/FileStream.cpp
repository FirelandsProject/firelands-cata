...
void SetLastError(int nError) { nLastError = nError; }
#endif
...
static bool Dummy_GetBitmap(TFileStream* pStream, TFileBitmap* pBitmap, DWORD Length, LPDWORD LengthNeeded)
{
...
static bool BaseFile_Read(TFileStream* pStream, // Pointer to an open stream
...
static bool BaseFile_GetPos(TFileStream* pStream, // Pointer to an open stream
...
static bool BaseFile_GetSize(TFileStream* pStream, // Pointer to an open stream
...
static bool BaseFile_Create(TFileStream* pStream, const TCHAR* szFileName, DWORD dwStreamFlags)
{
#ifdef PLATFORM_WINDOWS
    {
        DWORD dwWriteShare = (dwStreamFlags & STREAM_FLAG_WRITE_SHARE) ? FILE_SHARE_WRITE : 0;

        pStream->Base.File.hFile = CreateFile(szFileName, GENERIC_READ | GENERIC_WRITE, dwWriteShare | FILE_SHARE_READ, NULL, CREATE_ALWAYS, 0, NULL);
        if (pStream->Base.File.hFile == INVALID_HANDLE_VALUE)
            return false;
    }
#endif
...
static bool BaseFile_Open(TFileStream* pStream, const TCHAR* szFileName, DWORD dwStreamFlags)
{
#ifdef PLATFORM_WINDOWS
    {
        ...
        // Open the file
        pStream->Base.File.hFile = CreateFile(szFileName, dwDesiredAccess, dwWriteShare | FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
        if (pStream->Base.File.hFile == INVALID_HANDLE_VALUE)
            return false;
...
static bool BaseMap_Read(TFileStream* pStream, // Pointer to an open stream
    ULONGLONG* pByteOffset,                    // Pointer to file byte offset. If NULL, it reads from the current position
    void* pvBuffer,                            // Pointer to data to be read
    DWORD dwBytesToRead)                       // Number of bytes to read from the file
{
...
static bool BaseMap_GetPos(TFileStream* pStream, // Pointer to an open stream
    ULONGLONG& ByteOffset)                       // Pointer to file byte offset
{
...
static bool BaseMap_GetSize(TFileStream* pStream, // Pointer to an open stream
    ULONGLONG& FileSize)                          // Pointer where to store file size
{
...
static bool BaseHttp_Read(TFileStream* pStream, // Pointer to an open stream
    ULONGLONG* pByteOffset,                     // Pointer to file byte offset. If NULL, it reads from the current position
    void* pvBuffer,                             // Pointer to data to be read
    DWORD dwBytesToRead)                        // Number of bytes to read from the file
{
...
static bool BaseHttp_GetPos(TFileStream* pStream, // Pointer to an open stream
    ULONGLONG& ByteOffset)                        // Pointer to file byte offset
{
...
static bool BaseHttp_GetSize(TFileStream* pStream, // Pointer to an open stream
    ULONGLONG& FileSize)                           // Pointer where to store file size
{
...
static bool BaseHttp_Open(TFileStream* pStream, const TCHAR* szFileName, DWORD dwStreamFlags)
{
    ...
    {
        pStream->Base.Http.hInternet = InternetOpen(_T("StormLib HTTP MPQ reader"), INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
        if (pStream->Base.Http.hInternet == NULL)
            nError = GetLastError();
    }
    ...
    if (nError == ERROR_SUCCESS)
    {
        ...
        pStream->Base.Http.hConnect = InternetConnect(pStream->Base.Http.hInternet, szServerName, INTERNET_DEFAULT_HTTP_PORT, NULL, NULL, INTERNET_SERVICE_HTTP, dwFlags, 0);
        if (pStream->Base.Http.hConnect == NULL)
            nError = GetLastError();
    }
...
static bool LinearStream_Read(TLinearStream* pStream, // Pointer to an open stream
    ULONGLONG* pByteOffset,                           // Pointer to file byte offset. If NULL, it reads from the current position
    void* pvBuffer,                                   // Pointer to data to be read
    DWORD dwBytesToRead)                              // Number of bytes to read from the file
{
...
static bool LinearStream_GetBitmap(TLinearStream* pStream, TFileBitmap* pBitmap, DWORD Length, LPDWORD LengthNeeded)
{
    DWORD TotalLength;
    bool bResult = false;
...
static bool PartialStream_Read(TPartialStream* pStream, ULONGLONG* pByteOffset, void* pvBuffer, DWORD dwBytesToRead)
{
...
static bool PartialStream_GetPos(TPartialStream* pStream, ULONGLONG& ByteOffset)
{
    ByteOffset = pStream->VirtualPos;
    return true;
}

static bool PartialStream_GetSize(TPartialStream* pStream, // Pointer to an open stream
    ULONGLONG& FileSize)                                   // Pointer where to store file size
{
...
static bool PartialStream_GetBitmap(TPartialStream* pStream, TFileBitmap* pBitmap, DWORD Length, LPDWORD LengthNeeded)
{
    LPBYTE pbBitmap;
    DWORD TotalLength;
    DWORD BitmapSize = 0;
...
static const char* MpqKeyArray[] = {MpqeKey_Starcraft2_Install_deDE, MpqeKey_Starcraft2_Install_enGB, MpqeKey_Starcraft2_Install_enSG, MpqeKey_Starcraft2_Install_enUS, MpqeKey_Starcraft2_Install_esES,
    MpqeKey_Starcraft2_Install_esMX, MpqeKey_Starcraft2_Install_frFR, MpqeKey_Starcraft2_Install_itIT, MpqeKey_Starcraft2_Install_koKR, MpqeKey_Starcraft2_Install_plPL,
    MpqeKey_Starcraft2_Install_ptBR, MpqeKey_Starcraft2_Install_ruRU, MpqeKey_Starcraft2_Install_zhTW,

    MpqeKey_Diablo3_Install_deDE, MpqeKey_Diablo3_Install_enGB, MpqeKey_Diablo3_Install_enSG, MpqeKey_Diablo3_Install_enUS, MpqeKey_Diablo3_Install_esES, MpqeKey_Diablo3_Install_esMX,
    MpqeKey_Diablo3_Install_frFR, MpqeKey_Diablo3_Install_itIT, MpqeKey_Diablo3_Install_koKR, MpqeKey_Diablo3_Install_plPL, MpqeKey_Diablo3_Install_ptBR,
...
static bool EncryptedStream_Read(TEncryptedStream* pStream, // Pointer to an open stream
    ULONGLONG* pByteOffset,                                 // Pointer to file byte offset. If NULL, it reads from the current position
    void* pvBuffer,                                         // Pointer to data to be read
    DWORD dwBytesToRead)                                    // Number of bytes to read from the file
{
 ...
 * \a pStream Pointer to an open stream
 */
bool FileStream_IsReadOnly(TFileStream* pStream) { return (pStream->dwFlags & STREAM_FLAG_READ_ONLY) ? true : false; }
