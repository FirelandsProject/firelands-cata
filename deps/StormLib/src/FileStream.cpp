...
void SetLastError(DWORD dwErrCode) { dwLastError = dwErrCode; }
#endif
...
static bool BaseFile_Create(TFileStream* pStream)
{
#ifdef STORMLIB_WINDOWS
    {
        ...
        pStream->Base.File.hFile = CreateFile(pStream->szFileName, GENERIC_READ | GENERIC_WRITE, dwWriteShare | FILE_SHARE_READ, NULL, CREATE_ALWAYS, 0, NULL);
...
static bool BaseFile_Open(TFileStream* pStream, const TCHAR* szFileName, DWORD dwStreamFlags)
{
#ifdef STORMLIB_WINDOWS
    {
        ...
        // Open the file
        pStream->Base.File.hFile = CreateFile(szFileName, FILE_READ_DATA | FILE_READ_ATTRIBUTES | dwWriteAccess, FILE_SHARE_READ | dwWriteShare, NULL, OPEN_EXISTING, 0, NULL);
        if (pStream->Base.File.hFile == INVALID_HANDLE_VALUE)
            return false;
...
typedef ULONG(WINAPI* NTQUERYSECTION)(IN HANDLE SectionHandle, IN ULONG SectionInformationClass, OUT PVOID SectionInformation, IN SIZE_T Length, OUT PSIZE_T ResultLength);
...
static bool BlockStream_Read(TBlockStream* pStream, // Pointer to an open stream
    ULONGLONG* pByteOffset,                         // Pointer to file byte offset. If NULL, it reads from the current position
    void* pvBuffer,                                 // Pointer to data to be read
    DWORD dwBytesToRead)                            // Number of bytes to read from the file
{
...
static STREAM_INIT StreamBaseInit[4] = {BaseFile_Init, BaseMap_Init, BaseHttp_Init, BaseNone_Init};
...
static void FlatStream_UpdateBitmap(TBlockStream* pStream, // Pointer to an open stream
    ULONGLONG StartOffset, ULONGLONG EndOffset)
{
    LPBYTE FileBitmap = (LPBYTE)pStream->FileBitmap;
    DWORD BlockIndex;
    DWORD BlockSize = pStream->BlockSize;
...
static bool FlatStream_BlockCheck(TBlockStream* pStream, // Pointer to an open stream
    ULONGLONG BlockOffset)
{
    LPBYTE FileBitmap = (LPBYTE)pStream->FileBitmap;
    DWORD BlockIndex;
...
static bool FlatStream_BlockRead(TBlockStream* pStream, // Pointer to an open stream
    ULONGLONG StartOffset, ULONGLONG EndOffset, LPBYTE BlockBuffer, DWORD BytesNeeded, bool bAvailable)
{
...
static void PartStream_UpdateBitmap(TBlockStream* pStream, // Pointer to an open stream
    ULONGLONG StartOffset, ULONGLONG EndOffset, ULONGLONG RealOffset)
{
    PPART_FILE_MAP_ENTRY FileBitmap;
    DWORD BlockSize = pStream->BlockSize;
...
static bool PartStream_BlockCheck(TBlockStream* pStream, // Pointer to an open stream
    ULONGLONG BlockOffset)
{
    PPART_FILE_MAP_ENTRY FileBitmap;
...
static TFileStream* PartStream_Open(const TCHAR* szFileName, DWORD dwStreamFlags)
{
    ...
    static const char* MpqKeyArray[] = {// Starcraft II (Heart of the Swarm)
    ...
    static bool MpqeStream_BlockRead(TEncryptedStream * pStream, ULONGLONG StartOffset, ULONGLONG EndOffset, LPBYTE BlockBuffer, DWORD BytesNeeded, bool bAvailable)
    {
        DWORD dwBytesToRead;
...
#define BLOCK4_MAX_FSIZE 0x08040000  // Max size of one file
    ...
    static bool Block4Stream_BlockRead(TBlockStream * pStream, // Pointer to an open stream
        ULONGLONG StartOffset, ULONGLONG EndOffset, LPBYTE BlockBuffer, DWORD BytesNeeded, bool bAvailable)
    {
