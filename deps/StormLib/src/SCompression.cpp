...
typedef void (*COMPRESS)(void* pvOutBuffer, // [out] Pointer to the buffer where the compressed data will be stored
...
typedef int (*DECOMPRESS)(void* pvOutBuffer, // [out] Pointer to the buffer where to store decompressed data
...
static void Compress_LZMA(void* pvOutBuffer, int* pcbOutBuffer, void* pvInBuffer, int cbInBuffer, int* pCmpType, int nCmpLevel)
{
    ...
    nResult = LzmaEncode(destBuffer, &destLen, (Byte*)pvInBuffer, srcLen, &props, encodedProps, &encodedPropsSize, 0, &Progress, &SzAlloc, &SzAlloc);
    if (nResult != SZ_OK)
        return;
...
static int Decompress_LZMA_MPK(void* pvOutBuffer, int* pcbOutBuffer, void* pvInBuffer, int cbInBuffer)
{
    ...
    nResult = LzmaDecode(destBuffer, &destLen, srcBuffer + sizeof(LZMA_Props), &srcLen, srcBuffer, sizeof(LZMA_Props), LZMA_FINISH_END, &LzmaStatus, &SzAlloc);
    if (nResult != SZ_OK)
        return 0;
...
static int SCompDecompressInternal(TDecompressTable* table, size_t table_length, void* pvOutBuffer, int* pcbOutBuffer, void* pvInBuffer, int cbInBuffer, unsigned uValidMask = 0xFF)
{
...
int SCompDecompressMpk(void* pvOutBuffer, int* pcbOutBuffer, void* pvInBuffer, int cbInBuffer) { return Decompress_LZMA_MPK(pvOutBuffer, pcbOutBuffer, pvInBuffer, cbInBuffer); }
