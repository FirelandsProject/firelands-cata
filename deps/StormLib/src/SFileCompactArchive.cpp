        ...
        for (pFileEntry = ha->pFileTable; pFileEntry < pFileTableEnd; pFileEntry++, dwBlockIndex++)
        {
            if (pFileEntry->dwFlags & MPQ_FILE_EXISTS)
            {
                if (pFileEntry->szFileName != NULL && !IsPseudoFileName(pFileEntry->szFileName, NULL))
                {
                    ...
                    {
                        dwFileKey = DecryptFileKey(pFileEntry->szFileName, pFileEntry->ByteOffset, pFileEntry->dwFileSize, pFileEntry->dwFlags);
                    }
...
static int CopyNonMpqData(TFileStream* pSrcStream, TFileStream* pTrgStream, ULONGLONG& ByteOffset, ULONGLONG& ByteCount)
{
...
static int CopyMpqFileSectors(TMPQArchive* ha, TMPQFile* hf, TFileStream* pNewStream)
{
    ...
    {
        nError = WriteMpqDataMD5(pNewStream, ha->MpqPos + MpqFilePos, pFileEntry->dwCmpSize, ha->pHeader->dwRawChunkSize);
    }
