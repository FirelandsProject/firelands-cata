        ...
        for (pFileEntry = ha->pFileTable; pFileEntry < pFileTableEnd; pFileEntry++, dwBlockIndex++)
        {
            ...
            if (pFileEntry->dwFlags & MPQ_FILE_EXISTS)
            {
                ...
                if (pFileEntry->szFileName != NULL && !IsPseudoFileName(pFileEntry->szFileName, NULL))
                {
                    ...
                    pFileKeys[dwBlockIndex] = DecryptFileKey(pFileEntry->szFileName, pFileEntry->ByteOffset, pFileEntry->dwFileSize, pFileEntry->dwFlags);
                    continue;
                }
...
static DWORD CopyNonMpqData(TMPQArchive* ha, TFileStream* pSrcStream, TFileStream* pTrgStream, ULONGLONG& ByteOffset, ULONGLONG& ByteCount)
{
...
static DWORD CopyMpqFileSectors(TMPQArchive* ha, TMPQFile* hf, TFileStream* pNewStream,
    ULONGLONG MpqFilePos) // MPQ file position in the new archive
{
    ...
    {
        dwErrCode = WriteMpqDataMD5(pNewStream, ha->MpqPos + MpqFilePos, pFileEntry->dwCmpSize, ha->pHeader->dwRawChunkSize);
    }
