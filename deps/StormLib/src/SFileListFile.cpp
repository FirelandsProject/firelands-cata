...
static int WriteListFileLine(TMPQFile* hf, const char* szLine)
{
...
int SListFileSaveToMpq(TMPQArchive* ha)
{
    ...
    if (nFileNodes != 0)
    {
        ...
        nError = SFileAddFile_Init(ha, LISTFILE_NAME, 0, dwFileSize, LANG_NEUTRAL, ha->dwFileFlags1 | MPQ_FILE_REPLACEEXISTING, &hf);
        // Add all file names
        if (nError == ERROR_SUCCESS)
        {
    ...
    else
    {
        ...
        nError = SFileAddFile_Init(ha, LISTFILE_NAME, 0, dwFileSize, LANG_NEUTRAL, MPQ_FILE_ENCRYPTED | MPQ_FILE_COMPRESS | MPQ_FILE_REPLACEEXISTING, &hf);
