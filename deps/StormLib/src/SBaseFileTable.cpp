...
static void InitFileFlagArray(LPDWORD FlagArray) { memset(FlagArray, 0xCC, MAX_FLAG_INDEX * sizeof(DWORD)); }
...
TMPQBits* TMPQBits::Create(DWORD NumberOfBits, BYTE FillValue)
{
    TMPQBits* pBitArray;
...
void TMPQBits::GetBits(unsigned int nBitPosition, unsigned int nBitLength, void* pvBuffer, int nResultByteSize)
{
...
void TMPQBits::SetBits(unsigned int nBitPosition, unsigned int nBitLength, void* pvBuffer, int nResultByteSize)
{
    unsigned char* pbBuffer = (unsigned char*)pvBuffer;
...
void GetMPQBits(TMPQBits* pBits, unsigned int nBitPosition, unsigned int nBitLength, void* pvBuffer, int nResultByteSize) { pBits->GetBits(nBitPosition, nBitLength, pvBuffer, nResultByteSize); }

//-----------------------------------------------------------------------------
// Support for MPQ header

static bool VerifyTablePosition64(ULONGLONG MpqOffset, // Position of the MPQ header
...
static bool VerifyTableTandemPositions(ULONGLONG MpqOffset, // Position of the MPQ header
    ...
    ULONGLONG FileSize)                                     // Size of the entire file, in bytes
{
    return VerifyTablePosition64(MpqOffset, TableOffset1, TableSize1, FileSize) && VerifyTablePosition64(MpqOffset, TableOffset2, TableSize2, FileSize);
}
...
static ULONGLONG DetermineArchiveSize_V4(TMPQHeader* pHeader, ULONGLONG /* MpqOffset */, ULONGLONG /* FileSize */)
{
...
DWORD ConvertMpqHeaderToFormat4(TMPQArchive* ha, ULONGLONG ByteOffset, ULONGLONG FileSize, DWORD dwFlags, MTYPE MapType)
{
    ...
    case MPQ_FORMAT_VERSION_4:
        ...
        bHetBetOffsetOK = VerifyTableTandemPositions(ByteOffset, pHeader->HetTablePos64, pHeader->HetTableSize64, pHeader->BetTablePos64, pHeader->BetTableSize64, FileSize);

        // Is the "Hash&Block" table tandem OK?
        bHashBlockOffsetOK = VerifyTableTandemPositions(ByteOffset, pHeader->dwHashTablePos, pHeader->HashTableSize64, pHeader->dwBlockTablePos, pHeader->BlockTableSize64, FileSize);
...
static DWORD BuildFileTableFromBlockTable(TMPQArchive* ha, TMPQBlock* pBlockTable)
{
...
static TMPQHash* TranslateHashTable(TMPQArchive* ha, ULONGLONG* pcbTableSize)
{
    TMPQHash* pHashTable;
...
static USHORT* TranslateHiBlockTable(TMPQArchive* ha, ULONGLONG* pcbTableSize)
{
    TFileEntry* pFileEntry = ha->pFileTable;
...
static DWORD SaveMpqTable(TMPQArchive* ha, void* pMpqTable, ULONGLONG ByteOffset, size_t Size, unsigned char* md5, DWORD dwKey, bool bCompress)
{
...
static DWORD SaveExtTable(TMPQArchive* ha, TMPQExtHeader* pExtTable, ULONGLONG ByteOffset, DWORD dwTableSize, unsigned char* md5, DWORD dwKey, bool bCompress, LPDWORD pcbTotalSize)
{
    ULONGLONG FileOffset;
    ...
    {
        dwErrCode = WriteMemDataMD5(ha->pStream, FileOffset, pExtTable, dwTableSize, ha->pHeader->dwRawChunkSize, &cbTotalSize);
    }
...
static void CreateHetHeader(TMPQHetTable* pHetTable, TMPQHetHeader* pHetHeader)
{
    ...
    pHetHeader->ExtHdr.dwDataSize = pHetHeader->dwTableSize = sizeof(TMPQHetHeader) - sizeof(TMPQExtHeader) + pHetHeader->dwTotalCount + pHetHeader->dwIndexTableSize;
}
...
static DWORD InsertHetEntry(TMPQHetTable* pHetTable, ULONGLONG FileNameHash, DWORD dwFileIndex)
{
    ...
    for (;;)
    {
        ...
        if (pHetTable->pNameHashes[Index] == HET_ENTRY_FREE)
        {
            ...
            pHetTable->pBetIndexes->SetBits(pHetTable->dwIndexSizeTotal * Index, pHetTable->dwIndexSize, &dwFileIndex, 4);
            return ERROR_SUCCESS;
        }
...
static DWORD GetFileIndex_Het(TMPQArchive* ha, const char* szFileName)
{
    ...
    while (pHetTable->pNameHashes[Index] != HET_ENTRY_FREE)
    {
        ...
        if (pHetTable->pNameHashes[Index] == NameHash1)
        {
            ...
            // Get the file index
            pHetTable->pBetIndexes->GetBits(pHetTable->dwIndexSizeTotal * Index, pHetTable->dwIndexSize, &dwFileIndex, sizeof(DWORD));
...
static void CreateBetHeader(TMPQArchive* ha, TMPQBetHeader* pBetHeader)
{
    ...
    pBetHeader->dwTableEntrySize =
        pBetHeader->dwBitCount_FilePos + pBetHeader->dwBitCount_FileSize + pBetHeader->dwBitCount_CmpSize + pBetHeader->dwBitCount_FlagIndex + pBetHeader->dwBitCount_Unknown;
    ...
    pBetHeader->ExtHdr.dwDataSize = pBetHeader->dwTableSize =
        sizeof(TMPQBetHeader) - sizeof(TMPQExtHeader) + pBetHeader->dwFlagCount * sizeof(DWORD) + ((pBetHeader->dwTableEntrySize * pBetHeader->dwEntryCount) + 7) / 8 + pBetHeader->dwNameHashArraySize;
}
...
static TMPQBetTable* TranslateBetTable(TMPQArchive* ha, TMPQBetHeader* pBetHeader)
{
...
TMPQExtHeader* TranslateBetTable(TMPQArchive* ha, ULONGLONG* pcbBetTable)
{
    ...
    if (pbLinearTable != NULL)
    {
        ...
        if (pBitArray != NULL)
        {
            ...
            for (pFileEntry = ha->pFileTable; pFileEntry < pFileTableEnd; pFileEntry++)
            {
                ...
                pBitArray->SetBits(nBitOffset + BetHeader.dwBitIndex_FilePos, BetHeader.dwBitCount_FilePos, &pFileEntry->ByteOffset, 8);
                pBitArray->SetBits(nBitOffset + BetHeader.dwBitIndex_FileSize, BetHeader.dwBitCount_FileSize, &pFileEntry->dwFileSize, 4);
                pBitArray->SetBits(nBitOffset + BetHeader.dwBitIndex_CmpSize, BetHeader.dwBitCount_CmpSize, &pFileEntry->dwCmpSize, 4);

                // Save the flag index
                dwFlagIndex = GetFileFlagIndex(FlagArray, pFileEntry->dwFlags);
                pBitArray->SetBits(nBitOffset + BetHeader.dwBitIndex_FlagIndex, BetHeader.dwBitCount_FlagIndex, &dwFlagIndex, 4);
        ...
        if (pBitArray != NULL)
        {
            ...
            for (pFileEntry = ha->pFileTable; pFileEntry < pFileTableEnd; pFileEntry++)
            {
                ...
                pBitArray->SetBits(BetHeader.dwBitTotal_NameHash2 * dwFileIndex, BetHeader.dwBitCount_NameHash2, &pFileEntry->FileNameHash, 8);
    ...
    if (pBetTable != NULL)
    {
        ...
        for (i = 0; i < pHetTable->dwTotalCount; i++)
        {
            ...
            if (pHetTable->pNameHashes[i] != HET_ENTRY_FREE)
            {
                ...
                pHetTable->pBetIndexes->GetBits(pHetTable->dwIndexSizeTotal * i, pHetTable->dwIndexSize, &dwFileIndex, 4);
                // Overflow test
                if (dwFileIndex < pBetTable->dwEntryCount)
                {
                    ...
                    // Load the BET hash
                    pBetTable->pNameHashes->GetBits(pBetTable->dwBitTotal_NameHash2 * dwFileIndex, pBetTable->dwBitCount_NameHash2, &NameHash2, 8);
        ...
        for (i = 0; i < pBetTable->dwEntryCount; i++)
        {
            ...
            pBitArray->GetBits(dwBitPosition + pBetTable->dwBitIndex_FilePos, pBetTable->dwBitCount_FilePos, &pFileEntry->ByteOffset, 8);

            // Read the file size
            pBitArray->GetBits(dwBitPosition + pBetTable->dwBitIndex_FileSize, pBetTable->dwBitCount_FileSize, &pFileEntry->dwFileSize, 4);

            // Read the compressed size
            pBitArray->GetBits(dwBitPosition + pBetTable->dwBitIndex_CmpSize, pBetTable->dwBitCount_CmpSize, &pFileEntry->dwCmpSize, 4);

            // Read the flag index
            if (pBetTable->dwFlagCount != 0)
            {
                pBitArray->GetBits(dwBitPosition + pBetTable->dwBitIndex_FlagIndex, pBetTable->dwBitCount_FlagIndex, &dwFlagIndex, 4);
