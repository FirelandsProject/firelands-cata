...
static TBitArray* CreateBitArray(DWORD NumberOfBits, BYTE FillValue)
{
    TBitArray* pBitArray;
...
void TBitArray::GetBits(unsigned int nBitPosition, unsigned int nBitLength, void* pvBuffer, int nResultByteSize)
{
...
void TBitArray::SetBits(unsigned int nBitPosition, unsigned int nBitLength, void* pvBuffer, int nResultByteSize)
{
    unsigned char* pbBuffer = (unsigned char*)pvBuffer;
...
static TMPQBlock* TranslateBlockTable(TMPQArchive* ha, ULONGLONG* pcbTableSize, bool* pbNeedHiBlockTable)
{
...
TMPQExtTable* LoadExtTable(TMPQArchive* ha, ULONGLONG ByteOffset, size_t Size, DWORD dwSignature, DWORD dwKey)
{
...
// Used in MPQ Editor
void FreeMpqBuffer(void* pvBuffer) { STORM_FREE(pvBuffer); }

static int SaveMpqTable(TMPQArchive* ha, void* pMpqTable, ULONGLONG ByteOffset, size_t Size, unsigned char* md5, DWORD dwKey, bool bCompress)
{
...
static int SaveExtTable(TMPQArchive* ha, TMPQExtTable* pExtTable, ULONGLONG ByteOffset, DWORD dwTableSize, unsigned char* md5, DWORD dwKey, bool bCompress, LPDWORD pcbTotalSize)
{
    ULONGLONG FileOffset;
    ...
    {
        nError = WriteMemDataMD5(ha->pStream, FileOffset, pExtTable, dwTableSize, ha->pHeader->dwRawChunkSize, &cbTotalSize);
    }
...
static void CreateHetHeader(TMPQHetTable* pHetTable, PHET_TABLE_HEADER pHetHeader)
{
    // Fill the BET header
    ...
    pHetHeader->dwTableSize = sizeof(HET_TABLE_HEADER) + pHetHeader->dwHashTableSize + pHetHeader->dwIndexTableSize;
}
...
DWORD GetFileIndex_Het(TMPQArchive* ha, const char* szFileName)
{
    ...
    while (pHetTable->pHetHashes[Index] != HET_ENTRY_FREE)
    {
        ...
        if (pHetTable->pHetHashes[Index] == HetHash)
        {
            ...
            pHetTable->pBetIndexes->GetBits(pHetTable->dwIndexSizeTotal * Index, pHetTable->dwIndexSize, &dwFileIndex, 4);
...
DWORD AllocateHetEntry(TMPQArchive* ha, TFileEntry* pFileEntry)
{
    ...
    for (;;)
    {
        ...
        if (pHetTable->pHetHashes[Index] == HET_ENTRY_DELETED)
        {
            ...
            pHetTable->pBetIndexes->GetBits(pHetTable->dwIndexSizeTotal * Index, pHetTable->dwIndexSize, &dwBetIndex, 4);
    ...
    pHetTable->pBetIndexes->SetBits(pHetTable->dwIndexSizeTotal * FreeHetIndex, pHetTable->dwIndexSize, &dwFileIndex, 4);
    // Fill the file entry
...
static void CreateBetHeader(TMPQArchive* ha, PBET_TABLE_HEADER pBetHeader)
{
    ...
    pBetHeader->dwTableSize = sizeof(BET_TABLE_HEADER) + pBetHeader->dwFlagCount * sizeof(DWORD) + ((pBetHeader->dwTableEntrySize * pBetHeader->dwFileCount) + 7) / 8 + pBetHeader->dwBetHashArraySize;
}
...
static TMPQBetTable* TranslateBetTable(TMPQArchive* ha, TMPQExtTable* pExtTable)
{
...
TMPQExtTable* TranslateBetTable(TMPQArchive* ha, ULONGLONG* pcbBetTable)
{
    TMPQExtTable* pExtTable = NULL;
    BET_TABLE_HEADER BetHeader;
    ...
    BetTableSize = sizeof(BET_TABLE_HEADER) + BetHeader.dwFlagCount * sizeof(DWORD) + ((BetHeader.dwTableEntrySize * BetHeader.dwFileCount) + 7) / 8 + BetHeader.dwBetHashArraySize;
    ...
    if (pbLinearTable != NULL)
    {
        ...
        if (pBitArray != NULL)
        {
            ...
            for (i = 0; i < BetHeader.dwFileCount; i++)
            {
                ...
                pBitArray->SetBits(BetHeader.dwBetHashSizeTotal * i, BetHeader.dwBetHashSize, &FileNameHash, 8);
...
int RenameFileEntry(TMPQArchive* ha, TFileEntry* pFileEntry, const char* szNewFileName)
{
    ...
    if (ha->pHetTable != NULL)
    {
        ...
        pHetTable->pBetIndexes->SetBits(pHetTable->dwIndexSizeTotal * pFileEntry->dwHetIndex, pHetTable->dwIndexSize, &dwInvalidFileIndex, 4);
    }
...
static void FixBlockTableSize(TMPQArchive* ha, TMPQBlock* pBlockTable, DWORD dwClaimedSize)
{
    TMPQHeader* pHeader = ha->pHeader;
...
int BuildFileTable_Classic(TMPQArchive* ha, TFileEntry* pFileTable, ULONGLONG FileSize)
{
    TFileEntry* pFileEntry;
    TMPQHeader* pHeader = ha->pHeader;
    TMPQBlock* pBlockTable;
...
int BuildFileTable_HetBet(TMPQArchive* ha, TFileEntry* pFileTable)
{
    TMPQHetTable* pHetTable = ha->pHetTable;
    TMPQBetTable* pBetTable;
        ...
        for (i = 0; i < pHetTable->dwHashTableSize; i++)
        {
            ...
            if (pHetTable->pHetHashes[i] != 0)
            {
                ...
                if (dwFileIndex < pBetTable->dwFileCount)
                {
                    ...
                    // Load the BET hash
                    pBetTable->pBetHashes->GetBits(pBetTable->dwBetHashSizeTotal * dwFileIndex, pBetTable->dwBetHashSize, &pFileEntry->BetHash, 8);
                }
            }
        }
