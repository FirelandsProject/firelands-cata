    ...
    if (dwErrCode == ERROR_SUCCESS && dwSearchScope != SFILE_OPEN_CHECK_EXISTS)
    {
        ...
        if (hf != NULL)
        {
            ...
            if (bOpenByIndex == false)
            {
                ...
                {
                    hf->dwFileKey = DecryptFileKey(szFileName, pFileEntry->ByteOffset, pFileEntry->dwFileSize, pFileEntry->dwFlags);
                }
            }
        }
        else
...
bool WINAPI SFileHasFile(HANDLE hMpq, const char* szFileName) { return SFileOpenFileEx(hMpq, szFileName, SFILE_OPEN_CHECK_EXISTS, NULL); }
