...
void DumpHetAndBetTable(TMPQHetTable* pHetTable, TMPQBetTable* pBetTable)
{
    ...
    for (i = 0; i < pHetTable->dwHashTableSize; i++)
    {
        ...
        pHetTable->pBetIndexes->GetBits(i * pHetTable->dwIndexSizeTotal, pHetTable->dwIndexSize, &dwBetIndex, 4);

        if (dwBetIndex < pHetTable->dwMaxFileCount)
        {
            DWORD dwEntryIndex = pBetTable->dwTableEntrySize * dwBetIndex;

            pBetTable->pBetHashes->GetBits(dwBetIndex * pBetTable->dwBetHashSizeTotal, pBetTable->dwBetHashSize, &BetHash, 8);

            pBetTable->pFileTable->GetBits(dwEntryIndex + pBetTable->dwBitIndex_FilePos, pBetTable->dwBitCount_FilePos, &ByteOffset, 8);

            pBetTable->pFileTable->GetBits(dwEntryIndex + pBetTable->dwBitIndex_FileSize, pBetTable->dwBitCount_FileSize, &dwFileSize, 4);

            pBetTable->pFileTable->GetBits(dwEntryIndex + pBetTable->dwBitIndex_CmpSize, pBetTable->dwBitCount_CmpSize, &dwCmpSize, 4);

            pBetTable->pFileTable->GetBits(dwEntryIndex + pBetTable->dwBitIndex_FlagIndex, pBetTable->dwBitCount_FlagIndex, &dwFlagIndex, 4);

            dwFlags = pBetTable->pFileFlags[dwFlagIndex];
        }

        printf(" %04X    %02lX     %04X  %016llX %016llX %08X %08X  %04X  %08X\n", i, pHetTable->pHetHashes[i], dwBetIndex, BetHash, ByteOffset, dwFileSize, dwCmpSize, dwFlagIndex, dwFlags);
    }
    printf("-----------------------------------------------------------------------------------------\n");
}
