DELETE FROM `firelands_string` WHERE `entry` BETWEEN 820 AND 842;
...
UPDATE `firelands_string` SET `content_default`='* is vendor (%u)' WHERE `entry`=545;
UPDATE `firelands_string` SET `content_default`='* is trainer (%u)' WHERE `entry`=546;
