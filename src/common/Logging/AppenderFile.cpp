...
{
    if (extraArgs.empty())
        throw InvalidAppenderArgsException(Firelands::StringFormat("Log::CreateAppenderFromConfig: Missing file name for appender %s\n", name.c_str()));
...
void AppenderFile::_write(LogMessage const* message)
{
    ...
    if (_dynamicName)
    {
        char namebuf[FIRELANDS_PATH_MAX];
        snprintf(namebuf, FIRELANDS_PATH_MAX, _fileName.c_str(), message->param1.c_str());
