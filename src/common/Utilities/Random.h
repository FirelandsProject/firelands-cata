...
FC_COMMON_API int32 irand(int32 min, int32 max);

/* Return a random number in the range min..max (inclusive). */
FC_COMMON_API uint32 urand(uint32 min, uint32 max);
...
FC_COMMON_API uint32 urandms(uint32 min, uint32 max);

/* Return a random number in the range 0 .. UINT32_MAX. */
FC_COMMON_API uint32 rand32();
...
FC_COMMON_API Milliseconds randtime(Milliseconds min, Milliseconds max);

/* Return a random number in the range min..max */
FC_COMMON_API float frand(float min, float max);

/* Return a random double from 0.0 to 1.0 (exclusive). */
FC_COMMON_API double rand_norm();

/* Return a random double from 0.0 to 100.0 (exclusive). */
FC_COMMON_API double rand_chance();

/* Return a random number in the range 0..count (exclusive) with each value having a different chance of happening */
FC_COMMON_API uint32 urandweighted(size_t count, double const* chances);

/* Return true if a coin flip with the given chance of receiving tails occours*/
FC_COMMON_API bool coin_flip(float chance);
...
*/
class FC_COMMON_API RandomEngine
{
public:
