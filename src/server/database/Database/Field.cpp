...
uint8 Field::GetUInt8() const {
  ...
  if (data.raw) return *reinterpret_cast<uint8 const*>(data.value);
...
int8 Field::GetInt8() const {
  ...
  if (data.raw) return *reinterpret_cast<int8 const*>(data.value);
...
uint16 Field::GetUInt16() const {
  ...
  if (data.raw) return *reinterpret_cast<uint16 const*>(data.value);
...
int16 Field::GetInt16() const {
  ...
  if (data.raw) return *reinterpret_cast<int16 const*>(data.value);
...
uint32 Field::GetUInt32() const {
  ...
  if (data.raw) return *reinterpret_cast<uint32 const*>(data.value);
...
int32 Field::GetInt32() const {
  ...
  if (data.raw) return *reinterpret_cast<int32 const*>(data.value);
...
int64 Field::GetInt64() const {
  ...
  if (data.raw) return *reinterpret_cast<int64 const*>(data.value);
...
float Field::GetFloat() const {
  ...
  if (data.raw) return *reinterpret_cast<float const*>(data.value);
...
#ifdef FC_DEBUG
  if (!IsType(DatabaseFieldTypes::Double) &&
      !IsType(DatabaseFieldTypes::Decimal)) {
    LogWrongType(__FUNCTION__);
    return 0.0f;
  }
#endif
...
std::vector<uint8> Field::GetBinary() const {
  ...
  if (!data.value || !data.length) return result;
...
void Field::LogWrongType(char const* getter) const {
  LOG_WARN("sql.sql", "Warning: %s on %s field %s.%s (%s.%s) at index %u.",
           getter, meta->TypeName, meta->TableAlias, meta->Alias,
           meta->TableName, meta->Name, meta->Index);
}
