 ...
 public:
  ...
  std::vector<ChatCommand> GetCommands() const override {
    ...
    static std::vector<ChatCommand> ticketCommandTable = {
        ...
        {"escalatedlist", SEC_GAMEMASTER, true,
        ...
        {"response", SEC_GAMEMASTER, true, nullptr, "",
         ticketResponseCommandTable},
  ...
  static bool HandleGMTicketAssignToCommand(ChatHandler* handler,
    ...
    if (!normalizePlayerName(target)) return false;
    ...
    ticket->SetAssignedTo(
        targetGuid, AccountMgr::IsAdminAccount(
                        AccountMgr::GetSecurity(accountId, realm.Id.Realm)));
    ticket->SaveToDB(trans);
    sTicketMgr->UpdateLastChange();

    std::string msg = ticket->FormatMessageString(
        *handler, nullptr, target.c_str(), nullptr, nullptr, nullptr);
  ...
  static bool HandleGMTicketCloseByIdCommand(ChatHandler* handler,
    ...
    sTicketMgr->ResolveAndCloseTicket(
        ticket->GetId(), player ? player->GetGUID() : ObjectGuid(uint64(0)));
    sTicketMgr->UpdateLastChange();
  ...
  static bool HandleGMTicketCommentCommand(ChatHandler* handler,
    ...
    std::string msg = [&] {
      ...
      return ticket->FormatMessageString(
          *handler, nullptr,
          assignedName.empty() ? nullptr : assignedName.c_str(), nullptr,
          nullptr, nullptr);
    }();

    msg += handler->PGetParseString(
        LANG_COMMAND_TICKETLISTADDCOMMENT,
        player ? player->GetName().c_str() : "Console", comment);
    handler->SendGlobalGMSysMessage(msg.c_str());
  ...
  static bool HandleGMTicketUnAssignCommand(ChatHandler* handler,
    ...
    std::string assignedTo =
        ticket->GetAssignedToName();  // copy assignedto name because we need it
                                      // after the ticket has been unnassigned
    CharacterDatabaseTransaction trans = CharacterDatabaseTransaction(nullptr);
    ...
    std::string msg = ticket->FormatMessageString(
        *handler, nullptr, assignedTo.c_str(),
        handler->GetSession()
            ? handler->GetSession()->GetPlayer()->GetName().c_str()
            : "Console",
        nullptr, nullptr);
  ...
  static bool HandleGMTicketGetByIdCommand(ChatHandler* handler,
    ...
    handler->SendSysMessage(
        ticket->FormatMessageString(*handler, true).c_str());
    return true;
  }
  ...
  static bool HandleGMTicketGetByNameCommand(ChatHandler* handler,
    ...
    if (!normalizePlayerName(name)) return false;
  ...
  static bool _HandleGMTicketResponseAppendCommand(char const* args,
                                                   bool newLine,
                                                   ChatHandler* handler) {
    if (!*args) return false;
    ...
    CharacterDatabaseTransaction trans = CharacterDatabaseTransaction(nullptr);
    ticket->AppendResponse(response);
    if (newLine) ticket->AppendResponse("\n");
    ticket->SaveToDB(trans);
