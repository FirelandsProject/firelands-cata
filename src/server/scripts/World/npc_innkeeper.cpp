...
{
    explicit npc_innkeeper(Creature* creature) : ScriptedAI(creature) { }

    bool GossipHello(Player* player) override
    {
        if (IsHolidayActive(HOLIDAY_HALLOWS_END) && !player->HasAura(SPELL_TRICK_OR_TREATED))
        {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, 0, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
        }
        ...
        if (me->IsVendor())
        {
            AddGossipItemFor(player, GOSSIP_ICON_VENDOR, 2, GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRADE);
        }
