...
static bool IsFriendly(Creature* piece, Creature* target) { return piece->GetFaction() == target->GetFaction(); }
...
class npc_echo_of_medivh : public CreatureScript
{
  public:
    ...
    {
        npc_echo_of_medivhAI(Creature* creature) : ScriptedAI(creature), _summons(me) { _instance = creature->GetInstanceScript(); }
            ...
            for (uint8 row = 0; row < MAX_ROW; ++row)
            {
                for (uint8 col = 0; col < MAX_COL; ++col)
                {
                    ...
                    if (Creature* trigger = me->SummonCreature(
                            NPC_CHESS_MOVE_TRIGGER, (-11108.099609f + (3.49f * col) + (4.4f * row)), (-1872.910034f - (4.4f * col) + (3.45f * row)), 220.667f, 0, TEMPSUMMON_MANUAL_DESPAWN, 0))
                    {
        ...
        void HandleCheat()
        {
            ...
            case 1: // Fire
            {
                ...
                {
                    Firelands::Containers::RandomResize(targetList, 3);
                }
            ...
            case 2: // Buff
            {
                ...
                {
                    Firelands::Containers::RandomResize(targetList, resizeMax);
                }
    ...
    CreatureAI* GetAI(Creature* creature) const override { return new npc_echo_of_medivhAI(creature); }
};
...
class npc_chesspiece : public CreatureScript
{
  public:
    ...
    struct npc_chesspieceAI : public ScriptedAI
    {
        ...
        void MovementInform(uint32 movementType, uint32 /*pointId*/) override
        {
            ...
            if (!me->IsCharmed())
            {
                ...
                {
                    if (Creature* target = CAST_AI(npc_echo_of_medivh::npc_echo_of_medivhAI, medivh->AI())->GetHostileTargetForChangeFacing(me, _currentOrientation))
                    {
                        CastChangeFacing(me, target);
                    }
                }
        ...
        void JustDied(Unit* /*killer*/) override
        {
            if (Creature* medivh = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_ECHO_OF_MEDIVH)))
            {
                CAST_AI(npc_echo_of_medivh::npc_echo_of_medivhAI, medivh->AI())->HandlePieceJustDied(me);
            }
        ...
        Creature* GetEnemyPiece(float maxDistance, uint8 searchType = CHESS_PIECE_SEARCH_TYPE_CLOSEST, bool checkFront = true)
        {
            if (Creature* medivh = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_ECHO_OF_MEDIVH)))
            {
                return CAST_AI(npc_echo_of_medivh::npc_echo_of_medivhAI, medivh->AI())->GetPiece(me, searchType, maxDistance, true, 0, checkFront);
            }
        ...
        Creature* GetLowestHpFriendlyPiece(float maxDistance, uint32 minHPDiff)
        {
            if (Creature* medivh = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_ECHO_OF_MEDIVH)))
            {
                return CAST_AI(npc_echo_of_medivh::npc_echo_of_medivhAI, medivh->AI())->GetPiece(me, CHESS_PIECE_SEARCH_TYPE_RANDOM, maxDistance, false, minHPDiff, false);
            }
                ...
                if (!_teamControlledByRaid)
                {
                    if (_nextMoveTimer)
                    {
                        if (_nextMoveTimer <= diff)
                        {
                            if (Creature* medivh = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_ECHO_OF_MEDIVH)))
                            {
                                if (CAST_AI(npc_echo_of_medivh::npc_echo_of_medivhAI, medivh->AI())->HandlePieceMoveByAI(me, _currentOrientation))
                                {
                                    _nextMoveTimer = 0;
                                }
                                else
        ...
        void SpellHitTarget(Unit* target, SpellInfo const* spellInfo) override
        {
            ...
            case SPELL_MOVE_GENERIC:
                if (Creature* medivh = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_ECHO_OF_MEDIVH)))
                {
                    int8 result = CAST_AI(npc_echo_of_medivh::npc_echo_of_medivhAI, medivh->AI())->HandlePieceMove(me, target->GetGUID(), spellInfo->Id == SPELL_MOVE_GENERIC);
                    if (result != -1)
                    {
            ...
            case SPELL_CHANGE_FACING:
                if (Creature* medivh = ObjectAccessor::GetCreature(*me, _instance->GetGuidData(DATA_ECHO_OF_MEDIVH)))
                {
                    int8 result = CAST_AI(npc_echo_of_medivh::npc_echo_of_medivhAI, medivh->AI())->HandlePieceRotate(me, target->GetGUID());
                    if (result != -1)
                    {
                        DoCast(SPELL_MOVE_COOLDOWN);
                        _currentOrientation = KarazhanChessOrientationType(result);
    ...
    CreatureAI* GetAI(Creature* creature) const override { return new npc_chesspieceAI(creature); }
};
...
class npc_chess_move_trigger : public CreatureScript
{
  public:
    ...
    CreatureAI* GetAI(Creature* creature) const override { return new npc_chess_move_triggerAI(creature); }
};

// 30019 - Spell Control Piece
