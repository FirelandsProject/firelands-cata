    ...
    case SI_AREATRIGGER_A:
        if (player->GetTeam() == ALLIANCE && player->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            if (m_Gathered_A >= SI_MAX_RESOURCES)
            {
                ...
                sWorld->SendZoneText(OutdoorPvPSIBuffZones[0], sObjectMgr->GetFirelandsStringForDBCLocale(LANG_OPVP_SI_CAPTURE_A));
    ...
    case SI_AREATRIGGER_H:
        if (player->GetTeam() == HORDE && player->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            if (m_Gathered_H >= SI_MAX_RESOURCES)
            {
                ...
                /// @todo: confirm this text
                sWorld->SendZoneText(OutdoorPvPSIBuffZones[0], sObjectMgr->GetFirelandsStringForDBCLocale(LANG_OPVP_SI_CAPTURE_H));
            ...
            case ALLIANCE:
            {
                ...
                if (atEntry)
                {
                    ...
                    if (player->GetDistance(atEntry->Pos.X, atEntry->Pos.Y, atEntry->Pos.Z) > 5.0f + atEntry->Radius)
                    {
                        ...
                        if (!go->Create(map->GenerateLowGuid<HighGuid::GameObject>(), SI_SILITHYST_MOUND, map, *player, QuaternionData(), 255, GO_STATE_READY))
                        {
                            delete go;
                            return true;
                        }
