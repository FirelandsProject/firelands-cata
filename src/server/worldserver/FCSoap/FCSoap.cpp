...
void TCSoapThread(const std::string& host, uint16 port) {
  ...
  while (!World::IsStopped()) {
    ...
    LOG_DEBUG("network.soap", "Accepted connection from IP=%d.%d.%d.%d",
...
int ns1__executeCommand(soap* soap, char* command, char** result) {
  ...
  if (!command || !*command)
    return soap_sender_fault(soap, "Command can not be empty",
  ...
  // completed
  {
    // CliCommandHolder will be deleted from world, accessing after queueing is
    // NOT safe
    CliCommandHolder* cmd =
        new CliCommandHolder(&connection, command, &SOAPCommand::print,
...
struct Namespace namespaces[] = {
    {"SOAP-ENV", "http://schemas.xmlsoap.org/soap/envelope/", nullptr,
     nullptr},  // must be first
    {"SOAP-ENC", "http://schemas.xmlsoap.org/soap/encoding/", nullptr,
