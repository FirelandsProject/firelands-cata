...
char serviceDescription[] =
    "Firelands World of Warcraft emulator world service";
/*
...
public:
  FreezeDetector(Firelands::Asio::IoContext &ioContext, uint32 maxCoreStuckTime)
      : _timer(ioContext), _worldLoopCounter(0), _lastChangeMsTime(getMSTime()),
        _maxCoreStuckTimeInMs(maxCoreStuckTime) {}

  static void Start(std::shared_ptr<FreezeDetector> const &freezeDetector) {
    freezeDetector->_timer.expires_from_now(boost::posix_time::seconds(5));
    freezeDetector->_timer.async_wait(std::bind(
        &FreezeDetector::Handler, std::weak_ptr<FreezeDetector>(freezeDetector),
        std::placeholders::_1));
  }
...
extern int main(int argc, char **argv) {
  ...
  std::shared_ptr<boost::dll::shared_library> winmm(
      new boost::dll::shared_library(
          "winmm.dll", dllError, boost::dll::load_mode::search_system_folders),
  ...
  std::shared_ptr<Firelands::Asio::IoContext> ioContext =
      std::make_shared<Firelands::Asio::IoContext>();

  sLog->RegisterAppender<AppenderDB>();
  // If logs are supposed to be handled async then we need to pass the IoContext
  // into the Log singleton
  sLog->Initialize(sConfigMgr->GetBoolDefault("Log.Async.Enable", false)
                       ? ioContext.get()
                       : nullptr);

  Firelands::Banner::Show(
      "worldserver-daemon",
      [](char const *text) { LOG_INFO("server.worldserver", "%s", text); },
      []() {
        LOG_INFO("server.worldserver", "Using configuration file %s.",
                    sConfigMgr->GetFilename().c_str());
        LOG_INFO("server.worldserver", "Using SSL version: %s (library: %s)",
                    OPENSSL_VERSION_TEXT, SSLeay_version(SSLEAY_VERSION));
        LOG_INFO("server.worldserver", "Using Boost version: %i.%i.%i",
  ...
  SetProcessPriority("server.worldserver",
                     sConfigMgr->GetIntDefault(CONFIG_PROCESSOR_AFFINITY, 0),
  ...
  sMetric->Initialize(realm.Name, *ioContext, []() {
    FC_METRIC_VALUE("online_players", sWorld->GetPlayerCount());
  });
  ...
  if (sConfigMgr->GetBoolDefault("SOAP.Enabled", false)) {
    soapThread.reset(
        new std::thread(TCSoapThread,
                        sConfigMgr->GetStringDefault("SOAP.IP", "127.0.0.1"),
  ...
  if (!sWorldSocketMgr.StartWorldNetwork(*ioContext, worldListener, worldPort,
                                         networkThreads)) {
  ...
  LOG_INFO("server.worldserver", "%s (worldserver-daemon) ready...",
...
void ShutdownCLIThread(std::thread *cliThread) {
  if (cliThread != nullptr) {
    ...
    if (!CancelSynchronousIo(cliThread->native_handle())) {
      ...
      LOG_DEBUG(
          "server.worldserver",
          "Error cancelling I/O of CliThread, error code %u, detail: %s",
...
void WorldUpdateLoop() {
  uint32 minUpdateDiff =
      uint32(sConfigMgr->GetIntDefault("MinWorldUpdateTime", 1));
...
void FreezeDetector::Handler(std::weak_ptr<FreezeDetector> freezeDetectorRef,
  ...
  if (!error) {
    if (std::shared_ptr<FreezeDetector> freezeDetector =
      ...
      // possible freeze
      else if (getMSTimeDiff(freezeDetector->_lastChangeMsTime, curtime) >
               freezeDetector->_maxCoreStuckTimeInMs) {
        LOG_ERROR("server.worldserver",
      ...
      freezeDetector->_timer.async_wait(std::bind(
          &FreezeDetector::Handler, freezeDetectorRef, std::placeholders::_1));
    }
  }
}
...
bool LoadRealmInfo(Firelands::Asio::IoContext &ioContext) {
  ...
  if (!externalAddress) {
    LOG_ERROR("server.worldserver", "Could not resolve address %s",
                 fields[2].GetString().c_str());
    return false;
  }

  realm.ExternalAddress = Firelands::make_unique<boost::asio::ip::address>(
  ...
  realm.LocalAddress =
      Firelands::make_unique<boost::asio::ip::address>(localAddress->address());

  Optional<boost::asio::ip::tcp::endpoint> localSubmask =
      resolver.Resolve(boost::asio::ip::tcp::v4(), fields[4].GetString(), "");
  if (!localSubmask) {
    LOG_ERROR("server.worldserver", "Could not resolve address %s",
                 fields[4].GetString().c_str());
    return false;
  }

  realm.LocalSubnetMask =
      Firelands::make_unique<boost::asio::ip::address>(localSubmask->address());
...
bool StartDB() {
  ...
  QueryResult realmIdQuery = LoginDatabase.PQuery(
      "SELECT `Region`,`Battlegroup` FROM `realmlist` WHERE `id`=%u",
      realm.Id.Realm);
  if (!realmIdQuery) {
    LOG_ERROR("server.worldserver",
                 "Realm id %u not defined in realmlist table", realm.Id.Realm);
    return false;
  }
  ...
  LOG_INFO("server.worldserver",
              "Realm running as realm ID %u region %u battlegroup %u",
              realm.Id.Realm, uint32(realm.Id.Region), uint32(realm.Id.Site));
  ...
  WorldDatabase.PExecute(
      "UPDATE version SET core_version = '%s', core_revision = '%s'",
      GitRevision::GetFullVersion(), GitRevision::GetHash()); // One-time query

  sWorld->LoadDBVersion();

  LOG_INFO("server.worldserver", "Using World DB: %s",
              sWorld->GetDBVersion());
  return true;
}
...
void ClearOnlineAccounts() {
  ...
  LoginDatabase.DirectPExecute(
      "UPDATE account SET online = 0 WHERE online > 0 AND id IN (SELECT acctid "
...
variables_map GetConsoleArguments(int argc, char **argv, fs::path &configFile,
  ...
  all.add_options()("help,h", "print usage message")(
      ...
      value<fs::path>(&configFile)
          ->default_value(fs::absolute(_FIRELANDS_CORE_CONFIG)),
      "use <arg> as configuration file")("update-databases-only,u",
