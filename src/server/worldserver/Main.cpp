...
int main(int argc, char **argv)
{
    ...
    Firelands::Banner::Show("worldserver-daemon",
        ...
        []()
        {
            LOG_INFO("server.worldserver", "Using configuration file %s.", sConfigMgr->GetFilename());
    ...
    // Seed the OpenSSL's PRNG here. That way it won't auto-seed when calling BigNumber::SetRand and slow down the first world login
    BigNumber seed;
    seed.SetRand(16 * 8);
    ...
    sMetric->Initialize(realm.Name, *ioContext, []()
    {
        ...
        //FC_METRIC_VALUE("db_queue_login", uint64(LoginDatabase.QueueSize()));
        //FC_METRIC_VALUE("db_queue_character", uint64(CharacterDatabase.QueueSize()));
        //FC_METRIC_VALUE("db_queue_world", uint64(WorldDatabase.QueueSize()));
    });
    ...
    {
        soapThread.reset(new std::thread(TCSoapThread, sConfigMgr->GetStringDefault("SOAP.IP", "127.0.0.1"), uint16(sConfigMgr->GetIntDefault("SOAP.Port", 7878))), [](std::thread *thr)
        {
            thr->join();
            delete thr;
        });
    ...
    if (int32 coreStuckTime = sConfigMgr->GetIntDefault("MaxCoreStuckTime", 60))
    {
...
bool StartDB()
{
    ...
    // One-time query
    WorldDatabase.PExecute("UPDATE version SET core_version = '%s', core_revision = '%s'", GitRevision::GetFullVersion(), GitRevision::GetHash());
