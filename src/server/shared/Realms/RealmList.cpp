...
void RealmList::Initialize(Firelands::Asio::IoContext& ioContext, uint32 updateInterval)
{
...
void RealmList::UpdateRealm(Battlenet::RealmHandle const& id, uint32 build, std::string const& name, boost::asio::ip::address&& address, boost::asio::ip::address&& localAddr, boost::asio::ip::address&& localSubmask, uint16 port, uint8 icon, RealmFlags flag, uint8 timezone, AccountTypes allowedSecurityLevel, float population)
{
    ...
    {
        realm.ExternalAddress = Firelands::make_unique<boost::asio::ip::address>(std::move(address));
    }

    if (!realm.LocalAddress || *realm.LocalAddress != localAddr)
    {
        realm.LocalAddress = Firelands::make_unique<boost::asio::ip::address>(std::move(localAddr));
    }

    if (!realm.LocalSubnetMask || *realm.LocalSubnetMask != localSubmask)
    {
        realm.LocalSubnetMask = Firelands::make_unique<boost::asio::ip::address>(std::move(localSubmask));
    }
    ...
    if (result) {
        do {
            try {
                ...
                if (!externalAddress)
                {
                    LOG_ERROR("realmlist", "Could not resolve address %s", externalAddressString.c_str());
                    continue;
                }
                ...
                if (!localAddress)
                {
                    LOG_ERROR("realmlist", "Could not resolve address %s", localAddressString.c_str());
                    continue;
                }
                ...
                if (!localSubmask)
                {
                    LOG_ERROR("realmlist", "Could not resolve address %s", localSubmaskString.c_str());
                    continue;
                }
                ...
                UpdateRealm(id, build, name, externalAddress->address(), localAddress->address(), localSubmask->address(), port, icon, flag, timezone, (allowedSecurityLevel <= SEC_ADMINISTRATOR ? AccountTypes(allowedSecurityLevel) : SEC_ADMINISTRATOR), pop);
