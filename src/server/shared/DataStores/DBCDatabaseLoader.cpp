...
char* DBCDatabaseLoader::Load(uint32& records, char**& indexTable)
{
    ...
    std::unique_ptr<char[]> dataTable = Firelands::make_unique<char[]>(result->GetRowCount() * _recordSize);
    std::unique_ptr<uint32[]> newIndexes = Firelands::make_unique<uint32[]>(result->GetRowCount());
    uint32 newRecords = 0;
