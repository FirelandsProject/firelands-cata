...
class AsyncAcceptor
{
  public:
    ...
    AsyncAcceptor(Firelands::Asio::IoContext& ioContext, std::string const& bindIp, uint16 port)
        : _acceptor(ioContext), _endpoint(Firelands::Net::make_address(bindIp), port), _socket(ioContext), _closed(false), _socketFactory(std::bind(&AsyncAcceptor::DefeaultSocketFactory, this))
    {
    }

    template <class T> void AsyncAccept();

    template <AcceptCallback acceptCallback> void AsyncAcceptWithCallback()
    {
        tcp::socket* socket;
        uint32 threadIndex;
...
template <class T> void AsyncAcceptor::AsyncAccept()
{
