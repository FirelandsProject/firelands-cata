...
bool ConditionMgr::addToSpellArea(Condition* cond) const
{
    ...
    {
        LOG_ERROR("sql.sql", "%s invalid spell ID %u specified for area/zone ID %u.", cond->ToString().c_str(), cond->SourceGroup, cond->SourceEntry);
        return false;
    }

    LOG_ERROR("sql.sql", "%s SpellID %u does not have a entry for area/zone ID %u.", cond->ToString().c_str(), cond->SourceGroup, cond->SourceEntry);
    return false;
}
...
bool ConditionMgr::isSourceTypeValid(Condition* cond) const
{
    ...
    switch (cond->SourceType)
    {
        ...
        case CONDITION_SOURCE_TYPE_GRAVEYARD:
            ...
            {
                LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table, does not exist in WorldSafeLocs, ignoring.", cond->ToString().c_str());
                return false;
            }
            break;
        case CONDITION_SOURCE_TYPE_SPAWN:
...
bool ConditionMgr::isConditionTypeValid(Condition* cond) const
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_PHASEID:
        {
            ...
            {
                LOG_ERROR("sql.sql", "Phase condition has nonexistent phaseid in value1 (%u), skipped", cond->ConditionValue1);
                return false;
            }
            break;
        }
        ...
        {
            if (cond->ConditionValue2)
                LOG_ERROR("sql.sql", "Terrain swap condition has useless data in value2 (%u)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                LOG_ERROR("sql.sql", "Terrain swap condition has useless data in value3 (%u)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_TITLE:
        {
