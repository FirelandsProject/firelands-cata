...
void Guild::SendCommandResult(WorldSession* session, GuildCommandType type,
  ...
  // Note: SMSG_GUILD_COMMAND_RESULT and SMSG_GUILD_COMMAND_RESULT_2 do exactly
  // the same in the client, they just have different structures. There's no
  // particular reason why we use SMSG_GUILD_COMMAND_RESULT_2, this one is
  // processed inmediately as it is read from the client.
  ...
  LOG_DEBUG("guild",
            "SMSG_GUILD_COMMAND_RESULT [%s]: Type: %u, code: %u, param: %s",
...
void Guild::SendSaveEmblemResult(WorldSession* session,
  ...
  LOG_DEBUG("guild", "MSG_SAVE_GUILD_EMBLEM [%s] Code: %u",
            session->GetPlayerInfo().c_str(), errCode);
}
...
// If maximum number of events is reached, oldest event is removed from
// collection.
inline void Guild::LogHolder::AddEvent(CharacterDatabaseTransaction& trans,
                                       LogEntry* entry) {
...
void Guild::BankEventLogEntry::WritePacket(
  ...
  bool hasItem = m_eventType == GUILD_BANK_LOG_DEPOSIT_ITEM ||
                 m_eventType == GUILD_BANK_LOG_WITHDRAW_ITEM ||
                 m_eventType == GUILD_BANK_LOG_MOVE_ITEM ||
                 m_eventType == GUILD_BANK_LOG_MOVE_ITEM2;

  bool itemMoved = (m_eventType == GUILD_BANK_LOG_MOVE_ITEM ||
                    m_eventType == GUILD_BANK_LOG_MOVE_ITEM2);
  ...
  if (hasStack) bankLogEntry.Count = int32(m_itemStackCount);

  if (IsMoneyEvent()) bankLogEntry.Money = uint64(m_itemOrMoney);

  if (hasItem) bankLogEntry.ItemID = int32(m_itemOrMoney);

  if (itemMoved) bankLogEntry.OtherTab = int8(m_destTabId);
...
void Guild::NewsLogEntry::SaveToDB(CharacterDatabaseTransaction& trans) const {
  uint8 index = 0;
  CharacterDatabasePreparedStatement* stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_INS_GUILD_NEWS);
  stmt->setUInt32(index, m_guildId);
...
void Guild::NewsLogEntry::WritePacket(
    WorldPackets::Guild::GuildNews& newsPacket) const {
...
void Guild::RankInfo::CreateMissingTabsIfNeeded(
    uint8 tabs, CharacterDatabaseTransaction& trans,
    bool logOnCreate /* = false */) {
  for (uint8 i = 0; i < tabs; ++i) {
    GuildBankRightsAndSlots& rightsAndSlots = m_bankTabRightsAndSlots[i];
    if (rightsAndSlots.GetTabId() == i) continue;

    rightsAndSlots.SetTabId(i);
    if (m_rankId == GR_GUILDMASTER) rightsAndSlots.SetGuildMasterValues();

    if (logOnCreate)
      LOG_ERROR("guild",
                "Guild %u has broken Tab %u for rank %u. Created default tab.",
                m_guildId, i, m_rankId);
...
void Guild::RankInfo::SetBankMoneyPerDay(uint32 money) {
  if (m_bankMoneyPerDay == money) return;
...
// BankTab
Guild::BankTab::BankTab(ObjectGuid::LowType guildId, uint8 tabId)
...
bool Guild::BankTab::LoadItemFromDB(Field* fields) {
  ...
  if (!pItem->LoadFromDB(itemGuid, ObjectGuid::Empty, fields, itemEntry)) {
    LOG_ERROR("guild",
              "Item (GUID %u, id: %u) not found in item_instance, deleting "
              "from guild bank!",
              itemGuid, itemEntry);
...
void Guild::BankTab::Delete(CharacterDatabaseTransaction& trans,
  ...
  for (uint8 slotId = 0; slotId < GUILD_BANK_MAX_SLOTS; ++slotId) {
    if (Item* pItem = m_items[slotId]) {
      ...
      if (removeItemsFromDB) pItem->DeleteFromDB(trans);
      delete pItem;
      pItem = nullptr;
    }
  }
...
void Guild::BankTab::SetInfo(std::string const& name, std::string const& icon) {
  if (m_name == name && m_icon == icon) return;
...
bool Guild::BankTab::SetItem(CharacterDatabaseTransaction& trans, uint8 slotId,
                             Item* item) {
  if (slotId >= GUILD_BANK_MAX_SLOTS) return false;
...
void Guild::BankTab::SendText(Guild const* guild, WorldSession* session) const {
  ...
  if (session) {
    LOG_DEBUG("guild",
              "SMSG_GUILD_BANK_QUERY_TEXT_RESULT [%s]: Tabid: %u, Text: %s",
              session->GetPlayerInfo().c_str(), m_tabId, m_text.c_str());
    session->SendPacket(textQuery.Write());
  } else {
    LOG_DEBUG(
        "guild",
        "SMSG_GUILD_BANK_QUERY_TEXT_RESULT [Broadcast]: Tabid: %u, Text: %s",
        m_tabId, m_text.c_str());
    guild->BroadcastPacket(textQuery.Write());
  }
}

// Member
Guild::Member::Member(ObjectGuid::LowType guildId, ObjectGuid guid,
                      uint8 rankId)
    : m_guildId(guildId),
      m_guid(guid),
      m_zoneId(0),
...
void Guild::Member::SetStats(std::string const& name, uint8 level, uint8 _class,
                             uint32 zoneId, uint32 accountId,
                             uint32 achievementPoints, uint32 reputation) {
  m_name = name;
  m_level = level;
  m_class = _class;
  m_zoneId = zoneId;
...
void Guild::Member::SetPublicNote(std::string const& publicNote) {
  if (m_publicNote == publicNote) return;
...
void Guild::Member::SetOfficerNote(std::string const& officerNote) {
  if (m_officerNote == officerNote) return;
...
void Guild::Member::ChangeRank(CharacterDatabaseTransaction& trans,
  ...
  if (Player* player = FindConnectedPlayer()) player->SetGuildRank(newRank);
...
void Guild::Member::AddActivity(uint64 activity) {
  ...
  CharacterDatabasePreparedStatement* stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_MEMBER_ACTIVITY);
...
void Guild::Member::LoadProfessionDataFromDB(ObjectGuid guid) {
  CharacterDatabasePreparedStatement* stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_SKILLS);
  ...
  if (result) {
    do {
      ...
      CharacterDatabasePreparedStatement* stmt2 =
          CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHARACTER_SPELL);
      ...
      if (result2) {
        do {
          ...
          for (GuildProfession prof :
               sGuildMgr->GetGuildProfessionData(skill)) {
...
bool Guild::Member::CheckStats() const {
  if (m_level < 1) {
    LOG_ERROR("guild",
              "%s has a broken data in field `characters`.`level`, deleting "
              "him from guild!",
...
void Guild::Member::UpdateBankTabWithdrawValue(
    CharacterDatabaseTransaction& trans, uint8 tabId, uint32 amount) {
...
void Guild::Member::UpdateBankMoneyWithdrawValue(
    CharacterDatabaseTransaction& trans, uint64 amount) {
...
void Guild::Member::ResetValues(bool weekly /* = false*/) {
  ...
  if (weekly) ResetWeekActivityAndReputation();
}

void Guild::Member::AddReputation(uint32 rep, Player* player) {
  rep = std::min(rep, sWorld->getIntConfig(CONFIG_GUILD_WEEKLY_REP_CAP) -
                          m_weekReputation);

  player->GetReputationMgr().ModifyReputation(
      sFactionStore.LookupEntry(FACTION_GUILD), rep);
...
// MoveItemData
Guild::MoveItemData::MoveItemData(Guild* guild, Player* player, uint8 container,
                                  uint8 slotId)
    : m_pGuild(guild),
      m_pPlayer(player),
...
bool Guild::MoveItemData::CheckItem(uint32& splitedAmount) {
  ASSERT(m_pItem);
  if (splitedAmount > m_pItem->GetCount()) return false;
  if (splitedAmount == m_pItem->GetCount()) splitedAmount = 0;
  return true;
}
...
bool Guild::MoveItemData::CanStore(Item* pItem, bool swap, bool sendError) {
  ...
  if (sendError && msg != EQUIP_ERR_OK) m_pPlayer->SendEquipError(msg, pItem);
...
void Guild::MoveItemData::LogAction(MoveItemData* pFrom) const {
  ...
  sScriptMgr->OnGuildItemMove(m_pGuild, m_pPlayer, pFrom->GetItem(),
                              pFrom->IsBank(), pFrom->GetContainer(),
                              pFrom->GetSlotId(), IsBank(), GetContainer(),
                              GetSlotId());
}
...
void Guild::PlayerMoveItemData::LogBankEvent(
    CharacterDatabaseTransaction& trans, MoveItemData* pFrom,
    uint32 count) const {
  ASSERT(pFrom);
  // Bank -> Char
  m_pGuild->_LogBankEvent(
      trans, GUILD_BANK_LOG_WITHDRAW_ITEM, pFrom->GetContainer(),
      m_pPlayer->GetGUID().GetCounter(), pFrom->GetItem()->GetEntry(), count);
}
...
bool Guild::BankMoveItemData::HasStoreRights(MoveItemData* pOther) const {
  ...
  if (pOther->IsBank() && pOther->GetContainer() == m_container) return true;
...
void Guild::BankMoveItemData::RemoveItem(CharacterDatabaseTransaction& trans,
  ...
  // Decrease amount of player's remaining items (if item is moved to different
  // tab or to player)
  if (!pOther->IsBank() || pOther->GetContainer() != m_container)
    m_pGuild->_UpdateMemberWithdrawSlots(trans, m_pPlayer->GetGUID(),
                                         m_container);
}
...
void Guild::BankMoveItemData::LogAction(MoveItemData* pFrom) const {
  ...
  if (!pFrom->IsBank() &&
      m_pPlayer->IsGameMaster())  /// @todo Move this to scripts
  {
    sLog->outCommand(m_pPlayer->GetSession()->GetAccountId(),
                     "GM %s (Guid: %u) (Account: %u) deposit item: %s (Entry: "
                     "%d Count: %u) to guild bank named: %s (Guild ID: %u)",
                     m_pPlayer->GetName().c_str(),
                     m_pPlayer->GetGUID().GetCounter(),
                     m_pPlayer->GetSession()->GetAccountId(),
                     pFrom->GetItem()->GetTemplate()->GetDefaultLocaleName(),
...
Item* Guild::BankMoveItemData::_StoreItem(CharacterDatabaseTransaction& trans,
  ...
  if (pItem && pTab->SetItem(trans, slotId, pItem)) return pItem;
...
void Guild::BankMoveItemData::CanStoreItemInTab(Item* pItem, uint8 skipSlotId,
  ...
  for (uint8 slotId = 0; (slotId < GUILD_BANK_MAX_SLOTS) && (count > 0);
    ...
    // specified)
    if (slotId == skipSlotId) continue;

    Item* pItemDest = m_pGuild->_GetItem(m_container, slotId);
    if (pItemDest == pItem) pItemDest = nullptr;

    // If merge skip empty, if not merge skip non-empty
    if ((pItemDest != nullptr) != merge) continue;
...
InventoryResult Guild::BankMoveItemData::CanStore(Item* pItem, bool swap) {
  ...
  if (pItem->IsSoulBound()) return EQUIP_ERR_DROP_BOUND_ITEM;
  ...
  if (m_slotId != NULL_SLOT) {
    ...
    if ((pItemDest == pItem) || swap) pItemDest = nullptr;
...
bool Guild::Create(Player* pLeader, std::string const& name) {
  ...
  if (sGuildMgr->GetGuildByName(name)) return false;

  WorldSession* pLeaderSession = pLeader->GetSession();
  if (!pLeaderSession) return false;
  ...
  LOG_DEBUG("guild", "GUILD: creating guild [%s] for leader %s (%u)",
            name.c_str(), pLeader->GetName().c_str(),
...
void Guild::SaveToDB() {
  ...
  CharacterDatabasePreparedStatement* stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_EXPERIENCE);
...
void Guild::UpdateMemberData(Player* player, uint8 dataid, uint32 value) {
  if (Member* member = GetMember(player->GetGUID())) {
    switch (dataid) {
      ...
      default:
        LOG_ERROR("guild",
                  "Guild::UpdateMemberData: Called with incorrect DATAID %u "
                  "(value %u)",
                  dataid, value);
        return;
    }
...
bool Guild::SetName(std::string const& name) {
  if (m_name == name || name.empty() || name.length() > 24 ||
      sObjectMgr->IsReservedName(name) || !ObjectMgr::IsValidCharterName(name))
    return false;
...
void Guild::HandleRoster(WorldSession* session) {
  ...
  for (std::pair<uint32 const, Guild::Member*> const& memberPair : m_members) {
    ...
    memberData.PersonalAchievementPoints =
        int32(member->GetAchievementPoints());
    memberData.GuildReputation = int32(member->GetTotalReputation());
    memberData.LastSave = member->GetInactiveDays();
    memberData.GuildRepToCap =
        sWorld->getIntConfig(CONFIG_GUILD_WEEKLY_REP_CAP) -
        member->GetWeekReputation();
  ...
  LOG_DEBUG("guild", "SMSG_GUILD_ROSTER [%s]",
...
void Guild::SendQueryResponse(WorldSession* session) {
  ...
  LOG_DEBUG("guild", "SMSG_QUERY_GUILD_INFO_RESPONSE [%s]",
            session->GetPlayerInfo().c_str());
}
...
void Guild::SendGuildRankInfo(WorldSession* session) const {
  ...
  for (uint8 i = 0; i < _GetRanksSize(); i++) {
    ...
    for (uint8 j = 0; j < GUILD_BANK_MAX_TABS; ++j) {
      ...
      rankData.TabWithdrawItemLimit[j] =
          uint32(rankInfo->GetBankTabSlotsPerDay(j));
    }
...
void Guild::HandleSetAchievementTracking(
    WorldSession* session, std::set<uint32> const& achievementIds) {
  Player* player = session->GetPlayer();

  if (Member* member = GetMember(player->GetGUID())) {
    std::set<uint32> criteriaIds;

    for (std::set<uint32>::iterator achievementId = achievementIds.begin();
         achievementId != achievementIds.end(); ++achievementId) {
    ...
    m_achievementMgr->SendAllTrackedCriterias(player,
                                              member->GetTrackedCriteriaIds());
  }
}

void Guild::HandleSetMOTD(WorldSession* session, std::string const& motd) {
  if (m_motd == motd) return;
...
void Guild::HandleSetNewGuildMaster(WorldSession* session,
  ...
  if (isSelfPromote) {
    ...
    if (!newGuildMaster) return;

    if (!newGuildMaster->IsRankNotLower(GR_MEMBER) ||
        uint32(oldGuildMaster->GetInactiveDays()) <
            GUILD_MASTER_DETHRONE_INACTIVE_DAYS) {
      SendCommandResult(session, GUILD_COMMAND_CHANGE_LEADER,
  ...
  _BroadcastEvent(GE_LEADER_CHANGED, ObjectGuid::Empty,
                  player->GetName().c_str(), newGuildMaster->GetName().c_str());
  CharacterDatabase.CommitTransaction(trans);
}
...
void Guild::HandleSetBankTabInfo(WorldSession* session, uint8 tabId,
  ...
  if (!tab) {
    LOG_ERROR("guild",
              "Guild::HandleSetBankTabInfo: Player %s trying to change bank "
              "tab info from unexisting tab %d.",
              session->GetPlayerInfo().c_str(), tabId);
    return;
  }

  tab->SetInfo(name, icon);
  _BroadcastEvent(GE_BANK_TAB_UPDATED, ObjectGuid::Empty,
                  std::to_string(tabId).c_str(), name.c_str(), icon.c_str());
}

void Guild::HandleSetMemberNote(WorldSession* session, std::string const& note,
                                ObjectGuid guid, bool officer) {
  // Player must have rights to set public/officer note
  if (!_HasRankRight(session->GetPlayer(),
                     officer ? GR_RIGHT_EOFFNOTE : GR_RIGHT_EPNOTE))
    SendCommandResult(session, GUILD_COMMAND_PUBLIC_NOTE,
                      ERR_GUILD_PERMISSIONS);
...
void Guild::HandleSetRankInfo(
    WorldSession* session, uint8 rankId, std::string const& name, uint32 rights,
    uint32 moneyPerDay, GuildBankRightsAndSlotsVec const& rightsAndSlots) {
  // Only leader can modify ranks
  if (!_IsLeader(session->GetPlayer()))
    SendCommandResult(session, GUILD_COMMAND_CHANGE_RANK,
                      ERR_GUILD_PERMISSIONS);
  else if (RankInfo* rankInfo = GetRankInfo(rankId)) {
    LOG_DEBUG("guild", "Changed RankName to '%s', rights to 0x%08X",
              name.c_str(), rights);
...
void Guild::HandleBuyBankTab(WorldSession* session, uint8 tabId) {
  ...
  if (_GetPurchasedTabsSize() >= GUILD_BANK_MAX_TABS) return;

  if (tabId != _GetPurchasedTabsSize()) return;

  // Do not get money for bank tabs that the GM bought, we had to buy them
  // already. This is just a speedup check, GetGuildBankTabPrice will return 0.
  if (tabId < GUILD_BANK_MAX_TABS - 2)  // 7th tab is actually the 6th
  {
    ...
    if (!player->HasEnoughMoney(
            uint64(tabCost)))  // Should not happen, this is checked by client
      return;
  ...
  UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BUY_GUILD_BANK_SLOTS,
                            _GetPurchasedTabsSize(), 0, 0, nullptr, player);
...
void Guild::HandleInviteMember(WorldSession* session, std::string const& name) {
  ...
  if (!pInvitee) {
    SendCommandResult(session, GUILD_COMMAND_INVITE,
  ...
  if (pInvitee->GetSocial()->HasIgnore(player->GetGUID())) return;
  ...
  {
      SendCommandResult(session, GUILD_COMMAND_INVITE, ERR_ALREADY_IN_GUILD_S,
  name); return;
  }*/
  ...
  SendCommandResult(session, GUILD_COMMAND_INVITE, ERR_GUILD_COMMAND_SUCCESS,
                    name);

  LOG_DEBUG("guild", "Player %s invited %s to join his Guild",
            player->GetName().c_str(), name.c_str());

  pInvitee->SetGuildIdInvited(m_id);
  _LogEvent(GUILD_EVENT_LOG_INVITE_PLAYER, player->GetGUID().GetCounter(),
            pInvitee->GetGUID().GetCounter());
...
void Guild::HandleLeaveMember(WorldSession* session) {
  ...
  if (_IsLeader(player)) {
    if (m_members.size() > 1)
      ...
      SendCommandResult(session, GUILD_COMMAND_QUIT,
    ...
    SendCommandResult(session, GUILD_COMMAND_QUIT, ERR_GUILD_COMMAND_SUCCESS,
                      m_name);
  }

  sCalendarMgr->RemovePlayerGuildEventsAndSignups(player->GetGUID(), GetId());

  if (disband) delete this;
}
  ...
  if (!_HasRankRight(player, GR_RIGHT_REMOVE))
    ...
    // Do not allow to remove player with the same rank or higher
      ...
      if (!memberMe || member->IsRankNotLower(memberMe->GetRankId()))
        SendCommandResult(session, GUILD_COMMAND_REMOVE,
        ...
        _LogEvent(GUILD_EVENT_LOG_UNINVITE_PLAYER,
                  player->GetGUID().GetCounter(), guid.GetCounter());
        _BroadcastEvent(GE_REMOVED, ObjectGuid::Empty, name.c_str(),
                        player->GetName().c_str());
        SendCommandResult(session, GUILD_COMMAND_REMOVE,
                          ERR_GUILD_COMMAND_SUCCESS, name);
      }
    }
  }
}

void Guild::HandleUpdateMemberRank(WorldSession* session, ObjectGuid guid,
                                   bool demote) {
  Player* player = session->GetPlayer();
  ...
  // Promoted player must be a member of guild
    ...
    if (demote) {
      ...
      // member->GetRankId() + 1 is the highest rank that current player can
      // promote to
    ...
    _LogEvent(
        demote ? GUILD_EVENT_LOG_DEMOTE_PLAYER : GUILD_EVENT_LOG_PROMOTE_PLAYER,
        player->GetGUID().GetCounter(), member->GetGUID().GetCounter(),
        newRankId);
...
void Guild::HandleSetMemberRank(WorldSession* session, ObjectGuid targetGuid,
                                ObjectGuid setterGuid, uint32 rank) {
  Player* player = session->GetPlayer();
...
void Guild::HandleAddNewRank(WorldSession* session, std::string const& name) {
  ...
  if (size >= GUILD_RANKS_MAX_COUNT) return;
  ...
  if (_IsLeader(session->GetPlayer())) {
    ...
    if (_CreateRank(trans, name, GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK))
      _BroadcastEvent(GE_RANK_UPDATED, ObjectGuid::Empty,
                      std::to_string(size).c_str(), name.c_str());
  }
}
...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint64 amount,
  ...
  _LogBankEvent(trans,
                cashFlow ? GUILD_BANK_LOG_CASH_FLOW_DEPOSIT
                         : GUILD_BANK_LOG_DEPOSIT_MONEY,
                uint8(0), player->GetGUID().GetCounter(), amount);
  CharacterDatabase.CommitTransaction(trans);

  std::string aux =
      ByteArrayToHexStr(reinterpret_cast<uint8*>(&m_bankMoney), 8, true);
  _BroadcastEvent(GE_BANK_MONEY_SET, player->GetGUID(), aux.c_str());

  if (player->IsGameMaster()) {
    sLog->outCommand(player->GetSession()->GetAccountId(),
                     "GM %s (Account: %u) deposit money (Amount: " UI64FMTD
                     ") to guild bank (Guild ID %u)",
                     player->GetName().c_str(),
                     player->GetSession()->GetAccountId(), amount, m_id);
  }
}

bool Guild::HandleMemberWithdrawMoney(WorldSession* session, uint64 amount,
                                      bool repair) {
  ...
  if (!_HasRankRight(
          player, repair ? GR_RIGHT_WITHDRAW_REPAIR : GR_RIGHT_WITHDRAW_GOLD))
    return false;

  if (_GetMemberRemainingMoney(member) <
      int64(amount))  // Check if we have enough slot/money today
    return false;
  ...
  if (!repair) {
    if (!player->ModifyMoney(amount)) return false;
  ...
  _LogBankEvent(
      trans,
      repair ? GUILD_BANK_LOG_REPAIR_MONEY : GUILD_BANK_LOG_WITHDRAW_MONEY,
      uint8(0), player->GetGUID().GetCounter(), amount);
  CharacterDatabase.CommitTransaction(trans);
  ...
  if (repair)
    UpdateAchievementCriteria(
        ACHIEVEMENT_CRITERIA_TYPE_SPENT_GOLD_GUILD_REPAIRS, amount, 0, 0,
        nullptr, player);
  return true;
}
...
void Guild::HandleGuildPartyRequest(WorldSession* session) {
  ...
  if (!IsMember(player->GetGUID())) return;
  ...
  if (Group* group = player->GetGroup()) {
    ...
    NeededGuildPartyMembers = group->GetNeededMembersOfSameGuild(
        (player->InArena() && player->GetBattleground()->isRated())
            ? player->GetBattleground()->GetArenaType()
            : 0,
        player->GetMap());
  }
  ...
  session->SendPacket(&data);
  LOG_DEBUG("guild", "SMSG_GUILD_PARTY_STATE_RESPONSE [%s]",
            session->GetPlayerInfo().c_str());
}
...
void Guild::SendEventLog(WorldSession* session) const {
  ...
  for (GuildLog::const_iterator itr = logs->begin(); itr != logs->end();
       ++itr) {
  ...
  LOG_DEBUG("guild", "SMSG_GUILD_EVENT_LOG_QUERY_RESULT [%s]",
            session->GetPlayerInfo().c_str());
}
...
void Guild::SendNewsUpdate(WorldSession* session) {
  ...
  for (GuildLog::const_iterator itr = logs->begin(); itr != logs->end();
       ++itr) {
    // Skip news log entries that are older than a week. A cleanup of db and log
    // data will be executed on daily guild reset time.
    if ((*itr)->GetTimestamp() < weeklyMinTime) continue;
  ...
  session->SendPacket(packet.Write());
  LOG_DEBUG("guild", "SMSG_GUILD_NEWS_UPDATE [%s]",
            session->GetPlayerInfo().c_str());
}
...
void Guild::SendBankLog(WorldSession* session, uint8 tabId) const {
  ...
  if (tabId < _GetPurchasedTabsSize() || tabId == GUILD_BANK_MAX_TABS) {
    ...
    // If we have a weekly bonus money value, we can safely assume that Cash
    // Flow has been unlocked.
    ...
    LOG_DEBUG("guild", "SMSG_GUILD_BANK_LOG_QUERY_RESULT [%s] TabId: %u",
              session->GetPlayerInfo().c_str(), tabId);
  }
}

void Guild::SendBankTabText(WorldSession* session, uint8 tabId) const {
  if (BankTab const* tab = GetBankTab(tabId)) tab->SendText(this, session);
}
...
void Guild::SendPermissions(WorldSession* session) const {
  ...
  session->SendPacket(&data);
  LOG_DEBUG("guild", "SMSG_GUILD_PERMISSIONS_QUERY_RESULTS [%s] Rank: %u",
            session->GetPlayerInfo().c_str(), rankId);
}
...
void Guild::SendMoneyInfo(WorldSession* session) const {
  ...
  data << int64(amount);
  session->SendPacket(&data);
  LOG_DEBUG("guild", "SMSG_GUILD_BANK_MONEY_WITHDRAWN [%s] Money: " SI64FMTD,
            session->GetPlayerInfo().c_str(), amount);
}
...
void Guild::SendLoginInfo(WorldSession* session) {
  ...
  */
  ...
  LOG_DEBUG("guild", "SMSG_GUILD_EVENT [%s] MOTD",
            session->GetPlayerInfo().c_str());

  SendGuildRankInfo(session);
  _BroadcastEvent(GE_SIGNED_ON, player->GetGUID(), player->GetName().c_str());

  // Send to self separately, player is not in world yet and is not found by
  // _BroadcastEvent
  ...
  WorldPackets::Guild::GuildMemberDailyReset
      packet;  // tells the client to request bank withdrawal limit
  session->SendPacket(packet.Write());

  if (!sWorld->getBoolConfig(CONFIG_GUILD_LEVELING_ENABLED)) return;

  for (uint32 i = 0; i < sGuildPerkSpellsStore.GetNumRows(); ++i)
    if (GuildPerkSpellsEntry const* entry =
            sGuildPerkSpellsStore.LookupEntry(i))
...
void Guild::SendMemberUpdateNote(std::string const& note, ObjectGuid guid,
                                 bool isPublic) const {
...
bool Guild::LoadFromDB(Field* fields) {
  ...
  if (purchasedTabs > GUILD_BANK_MAX_TABS) purchasedTabs = GUILD_BANK_MAX_TABS;

  m_bankTabs.resize(purchasedTabs);
  for (uint8 i = 0; i < purchasedTabs; ++i)
    m_bankTabs[i] = new BankTab(m_id, i);

  _currChallengeCount[GUILD_CHALLENGE_TYPE_DUNGEON] =
      uint8(fields[16].GetUInt32());
  _currChallengeCount[GUILD_CHALLENGE_TYPE_RAID] =
      uint8(fields[17].GetUInt32());
  _currChallengeCount[GUILD_CHALLENGE_TYPE_RATED_BG] =
      uint8(fields[18].GetUInt32());
...
void Guild::LoadBankRightFromDB(Field* fields) {
  ...
  GuildBankRightsAndSlots rightsAndSlots(
      fields[1].GetUInt8(), fields[3].GetUInt8(), fields[4].GetUInt32());
  // rankId
  ...
  if (dbTabId < _GetPurchasedTabsSize() || isMoneyTab) {
    ...
    if (pLog->CanInsert()) {
      ...
      GuildBankEventLogTypes eventType =
          GuildBankEventLogTypes(fields[3].GetUInt8());
      if (BankEventLogEntry::IsMoneyEvent(eventType)) {
        if (!isMoneyTab) {
          LOG_ERROR("guild",
                    "GuildBankEventLog ERROR: MoneyEvent(LogGuid: %u, Guild: "
                    "%u) does not belong to money tab (%u), ignoring...",
                    guid, m_id, dbTabId);
          return false;
        }
      } else if (isMoneyTab) {
        LOG_ERROR("guild",
                  "GuildBankEventLog ERROR: non-money event (LogGuid: %u, "
                  "Guild: %u) belongs to money tab, ignoring...",
                  guid, m_id);
        return false;
      }
      pLog->LoadEvent(
          new BankEventLogEntry(m_id,                           // guild id
                                guid,                           // guid
...
void Guild::LoadBankTabFromDB(Field* fields) {
  ...
  if (tabId >= _GetPurchasedTabsSize())
    LOG_ERROR("guild", "Invalid tab (tabId: %u) in guild bank, skipped.",
              tabId);
  else
...
bool Guild::Validate() {
  ...
  // count is 2 and max is 10.
  bool broken_ranks = false;
  uint8 ranks = _GetRanksSize();

  CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction();
  if (ranks < GUILD_RANKS_MIN_COUNT || ranks > GUILD_RANKS_MAX_COUNT) {
    LOG_ERROR("guild", "Guild %u has invalid number of ranks, creating new...",
              m_id);
    broken_ranks = true;
  } else {
    for (uint8 rankId = 0; rankId < ranks; ++rankId) {
      ...
      if (rankInfo->GetId() != rankId) {
        ...
        rankInfo->CreateMissingTabsIfNeeded(_GetPurchasedTabsSize(), trans,
                                            true);
    }
  }
...
void Guild::BroadcastToGuild(WorldSession* session, bool officerOnly,
                             std::string const& msg, uint32 language) const {
  if (session && session->GetPlayer() &&
      _HasRankRight(session->GetPlayer(), officerOnly ? GR_RIGHT_OFFCHATSPEAK
                                                      : GR_RIGHT_GCHATSPEAK)) {
    WorldPacket data;
    ChatHandler::BuildChatPacket(
        data, officerOnly ? CHAT_MSG_OFFICER : CHAT_MSG_GUILD,
        Language(language), session->GetPlayer(), nullptr, msg);
    for (auto itr = m_members.begin(); itr != m_members.end(); ++itr)
      if (Player* player = itr->second->FindConnectedPlayer())
        if (player->GetSession() &&
            _HasRankRight(player, officerOnly ? GR_RIGHT_OFFCHATLISTEN
...
void Guild::BroadcastAddonToGuild(WorldSession* session, bool officerOnly,
  ...
  if (session && session->GetPlayer() &&
    ...
    WorldPacket data;
    ChatHandler::BuildChatPacket(
        data, officerOnly ? CHAT_MSG_OFFICER : CHAT_MSG_GUILD, LANG_ADDON,
        session->GetPlayer(), nullptr, msg, 0, "", DEFAULT_LOCALE, prefix);
...
void Guild::BroadcastPacketIfTrackingAchievement(WorldPacket const* packet,
                                                 uint32 criteriaId) const {
  for (Members::const_iterator itr = m_members.begin(); itr != m_members.end();
       ++itr)
...
void Guild::MassInviteToEvent(WorldSession* session, uint32 minLevel,
                              uint32 maxLevel, uint32 minRank) {
  uint32 count = 0;
...
void Guild::_DeleteMemberFromDB(CharacterDatabaseTransaction& trans,
                                ObjectGuid::LowType lowguid) const {
  CharacterDatabasePreparedStatement* stmt =
...
// Members handling
bool Guild::AddMember(CharacterDatabaseTransaction& trans, ObjectGuid guid,
                      uint8 rankId) {
  Player* player = ObjectAccessor::FindConnectedPlayer(guid);
  // Player cannot be in guild
  if (player) {
    if (player->GetGuildId() != 0) return false;
  ...
  if (rankId == GUILD_RANK_NONE) rankId = _GetLowestRankId();
  ...
  CharacterDatabasePreparedStatement* stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_SEL_OLD_GUILD_DATA);
  stmt->setUInt32(0, lowguid);
  ...
  if (player) {
    ...
    // Player must exist
    CharacterDatabasePreparedStatement* stmt2 =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_DATA_FOR_GUILD);
    stmt2->setUInt32(0, lowguid);
    if (PreparedQueryResult result = CharacterDatabase.Query(stmt2)) {
      Field* fields = result->Fetch();
      name = fields[0].GetString();
      member->SetStats(name, fields[1].GetUInt8(), fields[2].GetUInt8(),
                       fields[3].GetUInt16(), fields[4].GetUInt32(),
                       fields[5].GetUInt32(),
  ...
  if (player)
    if (Group* group = player->GetGroup()) group->UpdateGuildFor(guid, m_id);
...
void Guild::DeleteMember(CharacterDatabaseTransaction& trans, ObjectGuid guid,
  ...
  if (m_leaderGuid == guid && !isDisbanding) {
    ...
    if (!newLeader) {
      Disband();
      if (canDeleteGuild) delete this;
      return;
    }

    _SetLeader(trans, newLeader);

    // If leader does not exist (at guild loading with deleted leader) do not
    // send broadcasts
    if (oldLeader) {
      _BroadcastEvent(GE_LEADER_CHANGED, ObjectGuid::Empty,
                      oldLeader->GetName().c_str(),
  ...
  if (Member* member = GetMember(guid)) {
    if (!isDisbanding) {
      CharacterDatabasePreparedStatement* stmt =
          CharacterDatabase.GetPreparedStatement(CHAR_INS_LEAVE_GUILD_DATA);
      stmt->setUInt32(0, lowguid);
      stmt->setUInt32(1, m_id);
  ...
  if (player) {
    ...
    // Update GuildId in group
    if (Group* group = player->GetGroup()) group->UpdateGuildFor(guid, 0);
  } else
    sCharacterCache->UpdateCharacterGuildId(guid, 0);

  _DeleteMemberFromDB(trans, lowguid);
  if (!isDisbanding) _UpdateAccountsNumber();
}

bool Guild::ChangeMemberRank(CharacterDatabaseTransaction& trans,
                             ObjectGuid guid, uint8 newRank) {
  if (newRank <=
...
uint64 Guild::GetMemberAvailableMoneyForRepairItems(ObjectGuid guid) const {
  ...
  return std::min(m_bankMoney,
                  static_cast<uint64>(_GetMemberRemainingMoney(member)));
}
...
void Guild::SwapItems(Player* player, uint8 tabId, uint8 slotId,
  ...
  if (tabId == destTabId && slotId == destSlotId) return;
...
void Guild::SwapItemsWithInventory(Player* player, bool toChar, uint8 tabId,
                                   uint8 slotId, uint8 playerBag,
                                   uint8 playerSlotId, uint32 splitedAmount) {
...
void Guild::_CreateLogHolders() {
  m_eventLog =
      new LogHolder(sWorld->getIntConfig(CONFIG_GUILD_EVENT_LOG_COUNT));
  m_newsLog = new LogHolder(sWorld->getIntConfig(CONFIG_GUILD_NEWS_LOG_COUNT));
  for (uint8 tabId = 0; tabId <= GUILD_BANK_MAX_TABS; ++tabId)
    m_bankEventLog[tabId] =
        new LogHolder(sWorld->getIntConfig(CONFIG_GUILD_BANK_EVENT_LOG_COUNT));
}
...
void Guild::_CreateDefaultGuildRanks(CharacterDatabaseTransaction& trans,
  ...
  _CreateRank(trans, sObjectMgr->GetFirelandsString(LANG_GUILD_MASTER, loc),
              GR_RIGHT_ALL);
  _CreateRank(trans, sObjectMgr->GetFirelandsString(LANG_GUILD_OFFICER, loc),
              GR_RIGHT_ALL);
  _CreateRank(trans, sObjectMgr->GetFirelandsString(LANG_GUILD_VETERAN, loc),
              GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
  _CreateRank(trans, sObjectMgr->GetFirelandsString(LANG_GUILD_MEMBER, loc),
              GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
  _CreateRank(trans, sObjectMgr->GetFirelandsString(LANG_GUILD_INITIATE, loc),
              GR_RIGHT_GCHATLISTEN | GR_RIGHT_GCHATSPEAK);
}

bool Guild::_CreateRank(CharacterDatabaseTransaction& trans,
                        std::string const& name, uint32 rights) {
  uint8 newRankId = _GetRanksSize();
  if (newRankId >= GUILD_RANKS_MAX_COUNT) return false;
  ...
  if (!isInTransaction) trans = CharacterDatabase.BeginTransaction();

  info.CreateMissingTabsIfNeeded(_GetPurchasedTabsSize(), trans);
  info.SaveToDB(trans);

  if (!isInTransaction) CharacterDatabase.CommitTransaction(trans);
...
bool Guild::_IsLeader(Player* player) const {
  if (player->GetGUID() == m_leaderGuid) return true;
  if (Member const* member = GetMember(player->GetGUID()))
...
bool Guild::_ModifyBankMoney(CharacterDatabaseTransaction& trans, uint64 amount,
                             bool add, bool fromCashFlow /*= false*/) {
  if (add) {
    m_bankMoney += amount;
    if (fromCashFlow) _weeklyBonusMoney += amount;
  } else {
    // Check if there is enough money in bank.
    if (m_bankMoney < amount) return false;
    m_bankMoney -= amount;
  }
...
void Guild::_SetRankBankTabRightsAndSlots(
  ...
  if (rightsAndSlots.GetTabId() >= _GetPurchasedTabsSize()) return;
...
inline int32 Guild::_GetRankBankTabSlotsPerDay(uint8 rankId,
                                               uint8 tabId) const {
  if (tabId < _GetPurchasedTabsSize())
    if (RankInfo const* rankInfo = GetRankInfo(rankId))
  ...
  if (member) {
    ...
    if ((_GetRankBankTabRights(rankId, tabId) & GUILD_BANK_RIGHT_VIEW_TAB) !=
      ...
      int32 remaining = _GetRankBankTabSlotsPerDay(rankId, tabId) -
                        member->GetBankTabWithdrawValue(tabId);
...
inline int64 Guild::_GetMemberRemainingMoney(Member const* member) const {
  if (member) {
    uint8 rankId = member->GetRankId();
    if (rankId == GR_GUILDMASTER) return std::numeric_limits<int64>::max();

    if ((_GetRankRights(rankId) &
         (GR_RIGHT_WITHDRAW_REPAIR | GR_RIGHT_WITHDRAW_GOLD)) != 0) {
      int64 remaining = (int64(_GetRankBankMoneyPerDay(rankId)) * GOLD) -
                        member->GetBankMoneyWithdrawValue();
      if (remaining > 0) return remaining;
    }
  }
  return 0;
...
inline bool Guild::_MemberHasTabRights(ObjectGuid guid, uint8 tabId,
                                       uint32 rights) const {
  if (Member const* member = GetMember(guid)) {
    // Leader always has full rights
    if (member->IsRank(GR_GUILDMASTER) || m_leaderGuid == guid) return true;
...
inline void Guild::_LogEvent(GuildEventLogTypes eventType,
  ...
  CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction();
  m_eventLog->AddEvent(
      trans, new EventLogEntry(m_id, m_eventLog->GetNextGUID(), eventType,
...
void Guild::_LogBankEvent(CharacterDatabaseTransaction& trans,
                          ...
                          ObjectGuid::LowType lowguid, uint64 itemOrMoney,
                          uint16 itemStackCount, uint8 destTabId) {
  if (tabId > GUILD_BANK_MAX_TABS) return;

  // not logging moves within the same tab
  if (eventType == GUILD_BANK_LOG_MOVE_ITEM && tabId == destTabId) return;
  ...
  LogHolder* pLog = m_bankEventLog[tabId];
  pLog->AddEvent(trans, new BankEventLogEntry(
                            m_id, pLog->GetNextGUID(), eventType, dbTabId,
...
inline Item* Guild::_GetItem(uint8 tabId, uint8 slotId) const {
  if (const BankTab* tab = GetBankTab(tabId)) return tab->GetItem(slotId);
  return nullptr;
}

inline void Guild::_RemoveItem(CharacterDatabaseTransaction& trans, uint8 tabId,
                               uint8 slotId) {
  if (BankTab* pTab = GetBankTab(tabId)) pTab->SetItem(trans, slotId, nullptr);
}
...
void Guild::_MoveItems(MoveItemData* pSrc, MoveItemData* pDest,
  ...
  {
      LOG_FATAL("guild", "Guild::SwapItems: Player %s(GUIDLow: %u) tried to move
  item %u from tab %u slot %u to tab %u slot %u, but item %u has a stack of
  zero!", player->GetName(), player->GetGUID().GetCounter(),
  pItemSrc->GetEntry(), tabId, slotId, destTabId, destSlotId,
  pItemSrc->GetEntry());
      //return; // Commented out for now, uncomment when it's verified that this
  causes a crash!!
  }
  // */
  ...
  if (splitedAmount) {
    ...
    if (!pSrc->CloneItem(splitedAmount)) return;  // Item could not be cloned
  ...
  } else  // 6. No split
  {
    ...
    if (!_DoItemsMove(pSrc, pDest, false))  // Item could not be merged
    {
      ...
      if (!pDest->InitItem()) return;

      // 6.2.2. Check rights to store item in source (opposite direction)
      if (!pSrc->HasStoreRights(pDest))
        return;  // Player has no rights to store item in source (opposite
                 // direction)

      if (!pDest->HasWithdrawRights(pSrc))
        return;  // Player has no rights to withdraw item from destination
                 // (opposite direction)
...
void Guild::_SendBankContentUpdate(uint8 tabId, SlotIds slots) const {
  if (BankTab const* tab = GetBankTab(tabId)) {
    ...
    for (SlotIds::const_iterator itr = slots.begin(); itr != slots.end();
         ++itr) {
      ...
      itemInfo.Item.RandomPropertiesID =
          int32(tabItem ? tabItem->GetItemRandomPropertyId() : 0);
      itemInfo.Item.RandomPropertiesSeed =
          int32(tabItem ? tabItem->GetItemSuffixFactor() : 0);
      itemInfo.EnchantmentID =
          int32(tabItem ? tabItem->GetEnchantmentId(
                              EnchantmentSlot(PERM_ENCHANTMENT_SLOT))
                        : 0);
      itemInfo.OnUseEnchantmentID =
          int32(tabItem ? tabItem->GetEnchantmentId(
                              EnchantmentSlot(BONUS_ENCHANTMENT_SLOT))
                        : 0);
      itemInfo.ReforgeEnchantmentID =
          int32(tabItem ? tabItem->GetEnchantmentId(
                              EnchantmentSlot(REFORGE_ENCHANTMENT_SLOT))
                        : 0);
      itemInfo.Flags =
          int32(tabItem ? tabItem->GetInt32Value(ITEM_FIELD_FLAGS) : 0);
      itemInfo.Locked = false;

      if (tabItem) {
        uint8 slotIndex = 0;
        for (uint8 enchSlot = SOCK_ENCHANTMENT_SLOT;
             enchSlot <= SOCK_ENCHANTMENT_SLOT_3; enchSlot++) {
          if (uint32 enchantId =
                  tabItem->GetEnchantmentId(EnchantmentSlot(enchSlot))) {
      ...
      if (_MemberHasTabRights(itr->second->GetGUID(), tabId,
        ...
        if (Player* player = itr->second->FindPlayer()) {
          packet.WithdrawalsRemaining =
              _GetMemberRemainingSlots(itr->second, tabId);
...
void Guild::_BroadcastEvent(GuildEvents guildEvent, ObjectGuid guid,
                            ...
                            char const* param3) const {
  WorldPackets::Guild::GuildEvent packet;
  packet.EventType = guildEvent;
  if (param1) packet.Param.push_back(param1);

  if (param2) packet.Param.push_back(param2);

  if (param3) packet.Param.push_back(param3);

  if (guid) packet.GUID = guid;

  BroadcastPacket(packet.Write());

  if (sLog->ShouldLog("guild", LOG_LEVEL_DEBUG))
    LOG_DEBUG("guild", "SMSG_GUILD_EVENT [Broadcast] Event: %s (%u)",
              _GetGuildEventString(guildEvent).c_str(), guildEvent);
}

void Guild::SendBankList(WorldSession* session, uint8 tabId,
  ...
  // ItemInfo
  uint32 itemCount = 0;
  if (fullUpdate && _MemberHasTabRights(session->GetPlayer()->GetGUID(), tabId,
                                        GUILD_BANK_RIGHT_VIEW_TAB))
    if (BankTab const* tab = GetBankTab(tabId))
      for (uint8 slotId = 0; slotId < GUILD_BANK_MAX_SLOTS; ++slotId)
        if (tab->GetItem(slotId)) ++itemCount;
    ...
    if (BankTab const* tab = GetBankTab(tabId)) {
      for (uint8 slotId = 0; slotId < GUILD_BANK_MAX_SLOTS; ++slotId) {
        if (Item* tabItem = tab->GetItem(slotId)) {
          ...
          itemInfo.Item.RandomPropertiesID =
              int32(tabItem->GetItemRandomPropertyId());
          itemInfo.Item.RandomPropertiesSeed =
              int32(tabItem->GetItemSuffixFactor());
          itemInfo.Count = int32(tabItem->GetCount());
          itemInfo.Charges = int32(abs(tabItem->GetSpellCharges()));
          itemInfo.EnchantmentID =
              tabItem->GetEnchantmentId(EnchantmentSlot(PERM_ENCHANTMENT_SLOT));
          itemInfo.OnUseEnchantmentID = tabItem->GetEnchantmentId(
              EnchantmentSlot(BONUS_ENCHANTMENT_SLOT));
          itemInfo.ReforgeEnchantmentID = tabItem->GetEnchantmentId(
              EnchantmentSlot(REFORGE_ENCHANTMENT_SLOT));
          ...
          for (uint8 enchSlot = SOCK_ENCHANTMENT_SLOT;
            ...
            if (uint32 enchantId =
                    tabItem->GetEnchantmentId(EnchantmentSlot(enchSlot))) {
              WorldPackets::Guild::GuildBankItemInfo::GuildBankSocketEnchant
                  gem;
              gem.SocketIndex = slotIndex;
              gem.SocketEnchantID = enchantId;
              itemInfo.SocketEnchant.push_back(gem);
...
void Guild::SendGuildRanksUpdate(ObjectGuid setterGuid, ObjectGuid targetGuid,
                                 uint32 rank) {
  Member* member = GetMember(targetGuid);
  ASSERT(member);
  ...
  LOG_DEBUG(
      "network",
      "SMSG_GUILD_RANKS_UPDATE [Broadcast] Target: %s, Issuer: %s, RankId: %u",
...
void Guild::CompleteChallenge(uint8 challengeType, Player* source) {
  ...
  uint64 gold = GetLevel() >= sWorld->getIntConfig(CONFIG_GUILD_MAX_LEVEL)
                    ? _challengeGoldMaxLevel[challengeType]
                    : _challengeGold[challengeType];
  uint32 xp = GetLevel() >= sWorld->getIntConfig(CONFIG_GUILD_MAX_LEVEL)
                  ? _challengeXp[challengeType]
                  : 0;
  ...
  UpdateAchievementCriteria(
      ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_GUILD_CHALLENGE_TYPE, challengeType, 0,
      0, nullptr, source);
  UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_GUILD_CHALLENGE,
                            0, 0, 0, nullptr, source);
  ...
  if (money / 10000 != gold) money = std::numeric_limits<uint64>::max();

  _ModifyBankMoney(trans, money, true, false);

  CharacterDatabasePreparedStatement* stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_CHALLENGE);
...
void Guild::GiveReputation(uint32 rep, Player* source) {
  ...
  if (Member* member = GetMember(source->GetGUID())) {
    ...
    SendGuildReputationWeeklyCap(source->GetSession(),
                                 member->GetWeekReputation());
  }
}
...
void Guild::GiveXP(uint32 xp, Player* source, bool rewardedByChallenge) {
  ...
  if (GetLevel() < GUILD_EXPERIENCE_UNCAPPED_LEVEL)
    xp = std::min(xp, sWorld->getIntConfig(CONFIG_GUILD_DAILY_XP_CAP) -
                          uint32(_todayExperience));
  ...
  if (Member* member = GetMember(source->GetGUID())) member->AddActivity(xp);
  ...
  // Ding, mon!
  while (GetExperience() >= sGuildMgr->GetXPForGuildLevel(GetLevel()) &&
         GetLevel() < sWorld->getIntConfig(CONFIG_GUILD_MAX_LEVEL)) {
    ...
    UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_REACH_GUILD_LEVEL,
                              GetLevel(), 0, 0, nullptr, source);
...
void Guild::SendGuildXP(WorldSession* session /*= nullptr*/) const {
  ...
  packet.GuildRemainingXP =
      uint64(sGuildMgr->GetXPForGuildLevel(GetLevel()) - GetExperience());
...
void Guild::SendGuildReputationWeeklyCap(WorldSession* session,
  ...
  uint32 cap = std::max<uint32>(
      0, sWorld->getIntConfig(CONFIG_GUILD_WEEKLY_REP_CAP) - reputation);
...
void Guild::ResetTimes(bool weekly) {
  ...
  if (weekly) {
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_RESET_CHALLENGE);
  ...
  for (Members::const_iterator itr = m_members.begin(); itr != m_members.end();
    ...
    if (Player* player = itr->second->FindPlayer()) {
      ...
      WorldPacket data(SMSG_GUILD_MEMBER_DAILY_RESET,
                       0);  // tells the client to request bank withdrawal limit
      player->SendDirectMessage(&data);
    }
  }
}

void Guild::AddGuildNews(uint8 type, ObjectGuid guid, uint32 flags,
                         uint32 value) {
  NewsLogEntry* news = new NewsLogEntry(m_id, m_newsLog->GetNextGUID(),
                                        GuildNews(type), guid, flags, value);
...
void Guild::UpdateAchievementCriteria(AchievementCriteriaTypes type,
                                      uint64 miscValue1, uint64 miscValue2,
                                      uint64 miscValue3, Unit* unit,
                                      Player* player, GameObject* go) {
  m_achievementMgr->UpdateAchievementCriteria(type, miscValue1, miscValue2,
                                              miscValue3, unit, player, go);
}

void Guild::HandleNewsSetSticky(WorldSession* session, uint32 newsId,
                                bool sticky) {
  GuildLog* logs = m_newsLog->GetGuildLog();
  GuildLog::iterator itr = logs->begin();
  while (itr != logs->end() && (*itr)->GetGUID() != newsId) ++itr;

  if (itr == logs->end()) {
    LOG_DEBUG(
        "guild",
        "HandleNewsSetSticky: [%s] requested unknown newsId %u - Sticky: %u",
...
void Guild::SendKnownRecipes(Player const* player) {
  ...
  std::unordered_map<uint32, std::array<uint8, GUILD_RECIPES_COUNT>>
      uniqueBitsMap;
...
void Guild::SendMembersForRecipe(Player const* player, uint32 skillLineId,
                                 uint32 spellId, uint32 uniqueBit) {
...
void Guild::SendRecipesOfMember(Player const* player, uint32 skillLineId,
...
void Guild::InitializeGuildChallengeRewards() {
  // Initializing all possible fields so we wont end up with uninitialized
  // fields in case of bad database data
  for (uint8 i = 0; i < MAX_GUILD_CHALLENGE_TYPES; i++) {
...
void Guild::ClearExpiredNews() {
  ...
  if (GuildLog* logs = m_newsLog->GetGuildLog())
    for (GuildLog::const_iterator itr = logs->begin(); itr != logs->end();
         ++itr)
      if ((*itr)->GetTimestamp() < minTime) itr = logs->erase(itr);
}

Guild::LogEntry::LogEntry(ObjectGuid::LowType guildId, uint32 guid)
    : m_guildId(guildId),
      m_guid(guid),
