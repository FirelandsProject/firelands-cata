...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket& recvData) {
  ...
  if (_player->PlayerTalkClass->GetInteractionData().SourceGuid != guid) return;
  ...
  if (guid.IsCreatureOrVehicle()) {
    ...
    LOG_DEBUG("network",
              "WORLD: HandleGossipSelectOptionOpcode - unsupported %s.",
              guid.ToString().c_str());
    return;
  }
  ...
  if ((unit && unit->GetScriptId() != unit->LastUsedScriptID) ||
    ...
    LOG_DEBUG("network",
              "WORLD: HandleGossipSelectOptionOpcode - Script reloaded while "
              "in use, ignoring and set new scipt id");
  ...
  if (!code.empty()) {
    if (unit) {
      if (!unit->AI()->GossipSelectCode(_player, menuId, gossipListId,
                                        code.c_str()))
        _player->OnGossipSelect(unit, gossipListId, menuId);
    } else {
      if (!go->AI()->GossipSelectCode(_player, menuId, gossipListId,
                                      code.c_str()))
...
void WorldSession::HandleWhoOpcode(
    WorldPackets::Who::WhoRequestPkt& whoRequest) {
  ...
  if (whoRequest.Request.Areas.size() > 10) return;

  // user entered strings count, client limit=4 (checked on 2.0.10)
  // can't be received from real client or broken packet
  if (request.Words.size() > 4) return;

  std::vector<std::wstring> wWords;
  wWords.resize(request.Words.size());
  for (size_t i = 0; i < request.Words.size(); ++i) {
    LOG_DEBUG("network", "WorldSession::HandleWhoOpcode: Word: %s",
              request.Words[i].Word.c_str());

    // user entered string, it used as universal search pattern(guild+player
    // name)?
    if (!Utf8toWStr(request.Words[i].Word, wWords[i])) continue;
  ...
  if (!(Utf8toWStr(request.Name, wPlayerName) &&
        Utf8toWStr(request.Guild, wGuildName)))
    return;

  wstrToLower(wPlayerName);
  wstrToLower(wGuildName);

  // client send in case not set max level value 100 but Firelands supports 255
  ...
  for (WhoListPlayerInfo const& target : whoList) {
    // player can see member of other team only if has
    // CONFIG_ALLOW_TWO_SIDE_WHO_LIST
    if (target.GetTeam() != team && !allowTwoSideWhoList) continue;

    // player can see MODERATOR, GAME MASTER, ADMINISTRATOR only if has
    // The corresponding gm level
    if (target.GetSecurity() > AccountTypes(gmLevelInWhoList)) continue;
    ...
    if (lvl < request.MinLevel || lvl > request.MaxLevel) continue;

    // check if class matches classmask
    if (request.ClassFilter >= 0 &&
        !(request.ClassFilter & (1 << target.GetClass())))
      continue;

    // check if race matches racemask
    if (request.RaceFilter >= 0 &&
        (request.RaceFilter & (1 << target.GetRace())))
      continue;

    if (!whoRequest.Request.Areas.empty()) {
      if (std::find(
              whoRequest.Request.Areas.begin(), whoRequest.Request.Areas.end(),
              int32(target.GetZoneId())) == whoRequest.Request.Areas.end())
        continue;
    }

    std::wstring const& wTargetName = target.GetWidePlayerName();
    if (!(wPlayerName.empty() ||
          wTargetName.find(wPlayerName) != std::wstring::npos))
      continue;

    std::wstring const& wTargetGuildName = target.GetWideGuildName();

    if (!wGuildName.empty() &&
        wTargetGuildName.find(wGuildName) == std::wstring::npos)
      continue;
    ...
    if (!whoEntry.PlayerData.Initialize(target.GetGuid(), nullptr)) continue;
    ...
    if (response.Response.Entries.size() >=
        sWorld->getIntConfig(CONFIG_MAX_WHO))
      break;
  }
...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket& /*recvData*/) {
  ...
  if (ObjectGuid lguid = GetPlayer()->GetLootGUID()) DoLootRelease(lguid);

  bool instantLogout =
      (GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING) &&
       !GetPlayer()->IsInCombat()) ||
      GetPlayer()->IsInFlight() ||
      uint32(GetSecurity()) >= sWorld->getIntConfig(CONFIG_INSTANT_LOGOUT);

  bool canLogoutInCombat =
      GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_RESTING);
  ...
  if (GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_IN_PVP)) {
    ...
    if (!GetPlayer()->pvpInfo.IsHostile && GetPlayer()->IsPvP())
      GetPlayer()->pvpInfo.EndTimer =
          GameTime::GetGameTime();  // start toggle-off
  }
...
void WorldSession::HandleRequestCemeteryList(WorldPacket& /*recvPacket*/) {
  ...
  for (auto it = range.first; it != range.second && graveyardIds.size() < 16;
       ++it)  // client max
  {
  ...
  if (graveyardIds.empty()) {
    LOG_DEBUG("network",
              "No graveyards found for zone %u for player %u (team %u) in "
  ...
  for (uint32 id : graveyardIds) packet.CemeteryID.push_back(id);
...
void WorldSession::HandleReclaimCorpseOpcode(WorldPacket& recvData) {
  ...
  if (_player->InArena()) return;

  // body not released yet
  if (!_player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_GHOST)) return;
  ...
  if (!corpse->IsWithinDistInMap(_player, CORPSE_RECLAIM_RADIUS, true)) return;
...
void WorldSession::HandleResurrectResponseOpcode(WorldPacket& recvData) {
  ...
  if (!player->IsResurrectRequestedBy(guid)) return;
  ...
  if (Player* resurrectingPlayer = ObjectAccessor::GetPlayer(*player, guid)) {
    if (InstanceScript* instance = resurrectingPlayer->GetInstanceScript()) {
      if (instance->IsEncounterInProgress() &&
          resurrectingPlayer->GetMap()->IsRaid()) {
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recvData) {
  ...
  if (player->IsInFlight()) {
    LOG_DEBUG("network",
              "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) in flight, "
              "ignore Area Trigger ID:%u",
  ...
  if (!atEntry) {
    LOG_DEBUG("network",
              "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) send unknown "
              "(by DBC) Area Trigger ID:%u",
              player->GetName().c_str(), player->GetGUID().GetCounter(),
              triggerId);
    return;
  }
  ...
  if (!player->IsInAreaTriggerRadius(atEntry)) {
    LOG_DEBUG("network",
              "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far, ignore "
              "Area Trigger ID: %u",
              player->GetName().c_str(), player->GetGUID().GetCounter(),
              triggerId);
    return;
  ...
  if (player->isDebugAreaTriggers)
    ChatHandler(player->GetSession())
        .PSendSysMessage(LANG_DEBUG_AREATRIGGER_REACHED, triggerId);

  if (sScriptMgr->OnAreaTrigger(player, atEntry)) return;
  ...
  if (sObjectMgr->IsTavernAreaTrigger(triggerId)) {
    ...
    if (sWorld->IsFFAPvPRealm())
      player->RemoveByteFlag(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PVP_FLAG,
  ...
  if (OutdoorPvP* pvp = player->GetOutdoorPvP())
    if (pvp->HandleAreaTrigger(_player, triggerId)) return;
  ...
  if (player->GetMapId() != at->target_mapId) {
    if (Map::EnterState denyReason =
      ...
      bool reviveAtTrigger = false;  // should we revive the player if he is
                                     // trying to enter the correct instance?
      switch (denyReason) {
        case Map::CANNOT_ENTER_NO_ENTRY:
          LOG_DEBUG("maps",
                    "MAP: Player '%s' attempted to enter map with id %d which "
                    "has no entry",
                    player->GetName().c_str(), at->target_mapId);
          break;
        case Map::CANNOT_ENTER_UNINSTANCED_DUNGEON:
          LOG_DEBUG("maps",
                    "MAP: Player '%s' attempted to enter dungeon map %d but no "
                    "instance template was found",
                    player->GetName().c_str(), at->target_mapId);
          break;
        case Map::CANNOT_ENTER_DIFFICULTY_UNAVAILABLE:
          LOG_DEBUG("maps",
                    "MAP: Player '%s' attempted to enter instance map %d but "
                    "the requested difficulty was not found",
                    player->GetName().c_str(), at->target_mapId);
          if (MapEntry const* entry = sMapStore.LookupEntry(at->target_mapId))
            player->SendTransferAborted(entry->ID, TRANSFER_ABORT_DIFFICULTY,
                                        player->GetDifficulty(entry->IsRaid()));
          break;
        case Map::CANNOT_ENTER_NOT_IN_RAID: {
          LOG_DEBUG("maps",
                    "MAP: Player '%s' must be in a raid group to enter "
                    "instance map %d",
                    player->GetName().c_str(), at->target_mapId);
        ...
        case Map::CANNOT_ENTER_CORPSE_IN_DIFFERENT_INSTANCE: {
          ...
          LOG_DEBUG("maps",
                    "MAP: Player '%s' does not have a corpse in instance map "
                    "%d and cannot enter",
                    player->GetName().c_str(), at->target_mapId);
          break;
        }
        case Map::CANNOT_ENTER_INSTANCE_BIND_MISMATCH:
          if (MapEntry const* entry = sMapStore.LookupEntry(at->target_mapId)) {
            ...
            LOG_DEBUG("maps",
                      "MAP: Player '%s' cannot enter instance map '%s' because "
                      "their permanent bind is incompatible with their group's",
                      player->GetName().c_str(), mapName);
            player->SendTransferAborted(
                entry->ID, TRANSFER_ABORT_LOCKED_TO_DIFFERENT_INSTANCE,
                player->GetDifficulty(entry->IsRaid()));
          }
          reviveAtTrigger = true;
          break;
        case Map::CANNOT_ENTER_TOO_MANY_INSTANCES:
          ...
          LOG_DEBUG("maps",
                    "MAP: Player '%s' cannot enter instance map %d because he "
                    "has exceeded the maximum number of instances per hour.",
                    player->GetName().c_str(), at->target_mapId);
          reviveAtTrigger = true;
          break;
      ...
      if (reviveAtTrigger)  // check if the player is touching the areatrigger
                            // leading to the map his corpse is on
  ...
  if (!teleported)
    player->TeleportTo(at->target_mapId, at->target_X, at->target_Y,
...
void WorldSession::HandleUpdateAccountData(WorldPacket& recvData) {
  ...
  LOG_DEBUG("network", "UAD: type %u, time %u, decompressedSize %u", type,
            timestamp, decompressedSize);

  if (type > NUM_ACCOUNT_DATA_TYPES) return;
  ...
  if (decompressedSize > 0xFFFF) {
    ...
    LOG_ERROR("network", "UAD: Account data packet too big, size %u",
              decompressedSize);
    return;
  }
  ...
  if (uncompress(dest.contents(), &realSize,
                 recvData.contents() + recvData.rpos(),
                 recvData.size() - recvData.rpos()) != Z_OK) {
    recvData.rfinish();  // unnneded warning spam in this case
...
void WorldSession::HandleRequestAccountData(WorldPacket& recvData) {
  ...
  if (size && compress(dest.contents(), &destSize,
                       (uint8 const*)adata->Data.c_str(), size) != Z_OK) {
...
int32 WorldSession::HandleEnableNagleAlgorithm() {
  // Instructs the server we wish to receive few amounts of large packets
  // (SMSG_MULTIPLE_PACKETS?) instead of large amount of small packets
  return 0;
}
...
void WorldSession::HandleSetActionButtonOpcode(WorldPacket& recvData) {
  ...
  LOG_DEBUG("network", "CMSG_SET_ACTION_BUTTON Button: %u Data: %u", button,
            packetData);

  if (!packetData)
    GetPlayer()->removeActionButton(button);
  else
    GetPlayer()->addActionButton(button, ACTION_BUTTON_ACTION(packetData),
                                 ACTION_BUTTON_TYPE(packetData));
}

void WorldSession::HandleCompleteMovie(
    WorldPackets::Misc::CompleteMovie& /*packet*/) {
...
void WorldSession::HandleCompleteCinematic(
    WorldPackets::Misc::CompleteCinematic& /*packet*/) {
  // If player has sight bound to visual waypoint NPC we should remove it
  GetPlayer()->GetCinematicMgr()->EndCinematic();
}

void WorldSession::HandleNextCinematicCamera(
...
void WorldSession::HandleSetActionBarToggles(WorldPacket& recvData) {
  ...
  GetPlayer()->SetByteValue(PLAYER_FIELD_BYTES,
...
void WorldSession::HandlePlayedTime(WorldPacket& recvData) {
  ...
  packet.TriggerEvent =
      bool(TriggerScriptEvent);  // 0 - will not show in chat frame
  SendPacket(packet.Write());
}
...
void WorldSession::HandleInspectOpcode(WorldPacket& recvData) {
  ...
  if (!player) {
    LOG_DEBUG("network", "CMSG_INSPECT: No player found from %s",
              guid.ToString().c_str());
    return;
  }

  if (!GetPlayer()->IsWithinDistInMap(player, INSPECT_DISTANCE, false)) return;

  if (GetPlayer()->IsValidAttackTarget(player)) return;

  uint32 talent_points = 41;
  WorldPacket data(SMSG_INSPECT_TALENT,
                   8 + 4 + 1 + 1 + talent_points + 8 + 4 + 8 + 4);
  data << player->GetGUID();

  if (GetPlayer()->CanBeGameMaster() ||
      sWorld->getIntConfig(CONFIG_TALENTS_INSPECTING) +
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& recvData) {
  ...
  LOG_DEBUG("network",
            "CMSG_WORLD_TELEPORT: Player = %s, Time = %u, map = %u, x = %f, y "
            "= %f, z = %f, o = %f",
            GetPlayer()->GetName().c_str(), time, mapid, PositionX, PositionY,
...
void WorldSession::HandleWhoisOpcode(WorldPacket& recvData) {
  ...
  if (acc.empty()) acc = "Unknown";
  std::string email = fields[1].GetString();
  if (email.empty()) email = "Unknown";
  std::string lastip = fields[2].GetString();
  if (lastip.empty()) lastip = "Unknown";
...
void WorldSession::HandleComplainOpcode(WorldPacket& recvData) {
  ...
  switch (spam_type) {
    ...
    case 1:
      ...
      recvData >> unk4;         // time
      recvData >> description;  // spam description string (messagetype, channel
                                // name, player name, message)
      break;
  }

  // NOTE: all chat messages from this spammer automatically ignored by spam
  // reporter until logout in case chat spam. if it's mail spam - ALL mails from
  // this spammer automatically removed by client

  // Complaint Received message
  WorldPacket data(SMSG_COMPLAIN_RESULT, 2);
  data << uint8(0);  // value 1 resets CGChat::m_complaintsSystemStatus in
                     // client. (unused?)
  data << uint8(0);  // value 0xC generates a "CalendarError" in client.
  SendPacket(&data);

  LOG_DEBUG("network",
            "REPORT SPAM: type %u, %s, unk1 %u, unk2 %u, unk3 %u, unk4 %u, "
            "message %s",
            spam_type, spammer_guid.ToString().c_str(), unk1, unk2, unk3, unk4,
            description.c_str());
}
...
void WorldSession::HandleFarSightOpcode(WorldPacket& recvData) {
  ...
  if (apply) {
    LOG_DEBUG("network", "Added FarSight %s to player %u",
              _player->GetGuidValue(PLAYER_FARSIGHT).ToString().c_str(),
              _player->GetGUID().GetCounter());
    if (WorldObject* target = _player->GetViewpoint())
      _player->SetSeer(target);
    else
      LOG_DEBUG("network", "Player %s (%s) requests non-existing seer %s",
                _player->GetName().c_str(),
                _player->GetGUID().ToString().c_str(),
                _player->GetGuidValue(PLAYER_FARSIGHT).ToString().c_str());
  } else {
    LOG_DEBUG("network", "Player %u set vision to self",
              _player->GetGUID().GetCounter());
...
void WorldSession::HandleSetTitleOpcode(WorldPacket& recvData) {
  ...
  if (title > 0 && title < MAX_TITLE_INDEX) {
    if (!GetPlayer()->HasTitle(title)) return;
  } else
    title = 0;
...
void WorldSession::HandleSetDungeonDifficultyOpcode(WorldPacket& recvData) {
  ...
  if (mode >= MAX_DUNGEON_DIFFICULTY) {
    LOG_DEBUG("network",
              "WorldSession::HandleSetDungeonDifficultyOpcode: player %d sent "
              "an invalid instance mode %d!",
  ...
  if (map && map->IsDungeon()) {
    LOG_DEBUG(
        "network",
        "WorldSession::HandleSetDungeonDifficultyOpcode: player (Name: %s, "
        "GUID: %u) tried to reset the instance while player is inside!",
  ...
  if (group) {
    if (group->IsLeader(_player->GetGUID())) {
      for (GroupReference* itr = group->GetFirstMember(); itr != nullptr;
        ...
        if (!groupGuy) continue;

        if (!groupGuy->IsInMap(groupGuy)) return;

        if (groupGuy->GetMap()->IsNonRaidDungeon()) {
          LOG_DEBUG("network",
                    "WorldSession::HandleSetDungeonDifficultyOpcode: player %d "
                    "tried to reset the instance while group member (Name: %s, "
                    "GUID: %u) is inside!",
                    _player->GetGUID().GetCounter(),
                    groupGuy->GetName().c_str(),
                    groupGuy->GetGUID().GetCounter());
          return;
        }
      }
...
void WorldSession::HandleSetRaidDifficultyOpcode(WorldPacket& recvData) {
  ...
  if (mode >= MAX_RAID_DIFFICULTY) {
    LOG_ERROR("network",
              "WorldSession::HandleSetRaidDifficultyOpcode: player %d sent an "
  ...
  if (map && map->IsDungeon()) {
    LOG_DEBUG("network",
              "WorldSession::HandleSetRaidDifficultyOpcode: player %d tried to "
              "reset the instance while inside!",
              _player->GetGUID().GetCounter());
    return;
  }

  if (Difficulty(mode) == _player->GetRaidDifficulty()) return;
    ...
    if (group->IsLeader(_player->GetGUID())) {
      for (GroupReference* itr = group->GetFirstMember(); itr != nullptr;
        ...
        if (groupGuy->GetMap()->IsRaid()) {
          LOG_DEBUG("network",
                    "WorldSession::HandleSetRaidDifficultyOpcode: player %d "
...
void WorldSession::HandleQueryInspectAchievements(WorldPacket& recvData) {
  ...
  LOG_DEBUG("network",
            "CMSG_QUERY_INSPECT_ACHIEVEMENTS [%s] Inspected Player [%s]",
            _player->GetGUID().ToString().c_str(), guid.ToString().c_str());
  Player* player = ObjectAccessor::GetPlayer(*_player, guid);
  if (!player) return;
...
void WorldSession::HandleAreaSpiritHealerQueryOpcode(WorldPacket& recvData) {
  ...
  if (bg) sBattlegroundMgr->SendAreaSpiritHealerQueryOpcode(_player, bg, guid);
...
void WorldSession::HandleAreaSpiritHealerQueueOpcode(WorldPacket& recvData) {
  ...
  if (bg) bg->AddPlayerToResurrectQueue(guid, _player->GetGUID());
...
void WorldSession::HandleInstanceLockResponse(WorldPacket& recvPacket) {
  ...
  if (!_player->HasPendingBind()) {
    LOG_INFO("network",
             "InstanceLockResponse: Player %s (guid %u) tried to bind "
...
void WorldSession::HandleObjectUpdateFailedOpcode(WorldPacket& recvPacket) {
  ...
  LOG_ERROR("network",
            "Object update failed for object " UI64FMTD
            " (%s) for player %s (%u)",
            uint64(guid), obj ? obj->GetName().c_str() : "object-not-found",
            GetPlayerName().c_str(), GetGUIDLow());

  // If create object failed for current player then client will be stuck on
  // loading screen
...
void WorldSession::HandleSaveCUFProfiles(WorldPacket& recvPacket) {
  ...
  if (count > MAX_CUF_PROFILES) {
    LOG_ERROR("entities.player",
              "HandleSaveCUFProfiles - %s tried to save more than %i CUF "
              "profiles. Hacking attempt?",
  ...
  for (uint8 i = 0; i < count; ++i) {
    ...
    profiles[i]->BoolOptions.set(CUF_AUTO_ACTIVATE_SPEC_2,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_AUTO_ACTIVATE_10_PLAYERS,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_UNK_157, recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_DISPLAY_HEAL_PREDICTION,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_AUTO_ACTIVATE_SPEC_1,
                                 recvPacket.ReadBit());
    ...
    profiles[i]->BoolOptions.set(CUF_AUTO_ACTIVATE_15_PLAYERS,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_AUTO_ACTIVATE_40_PLAYERS,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_DISPLAY_PETS, recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_AUTO_ACTIVATE_5_PLAYERS,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_DISPLAY_ONLY_DISPELLABLE_DEBUFFS,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_AUTO_ACTIVATE_2_PLAYERS,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_UNK_156, recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_DISPLAY_NON_BOSS_DEBUFFS,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_DISPLAY_MAIN_TANK_AND_ASSIST,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_DISPLAY_AGGRO_HIGHLIGHT,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_AUTO_ACTIVATE_3_PLAYERS,
                                 recvPacket.ReadBit());
    ...
    profiles[i]->BoolOptions.set(CUF_DISPLAY_HORIZONTAL_GROUPS,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_AUTO_ACTIVATE_25_PLAYERS,
                                 recvPacket.ReadBit());
    profiles[i]->BoolOptions.set(CUF_KEEP_GROUPS_TOGETHER,
                                 recvPacket.ReadBit());
  }
...
void WorldSession::HandleRequestResearchHistory(
    WorldPackets::Archaeology::RequestResearchHistory& /*packet*/) {
  _player->NotifyRequestResearchHistory();
}

void WorldSession::HandleOpeningCinematic(
    WorldPackets::Misc::OpeningCinematic& /*packet*/) {
  // Only players that has not yet gained any experience can use this
  if (_player->GetUInt32Value(PLAYER_XP)) return;

  if (ChrClassesEntry const* classEntry =
          sChrClassesStore.LookupEntry(_player->getClass())) {
    if (classEntry->CinematicSequenceID)
      _player->SendCinematicStart(classEntry->CinematicSequenceID);
    else if (ChrRacesEntry const* raceEntry =
                 sChrRacesStore.LookupEntry(_player->getRace()))
