...
void WorldSession::HandleAuctionSellItem(WorldPacket& recvData)
{
    ...
    {
        LOG_DEBUG("network", "WORLD: HandleAuctionSellItem - Player %s (GUID %u) attempted to sell item with higher price than max gold amount.", _player->GetName().c_str(), _player->GetGUID().GetCounter());
        SendAuctionCommandResult(nullptr, AUCTION_SELL_ITEM, ERR_AUCTION_DATABASE_ERROR);
        return;
    }
    ...
    if (itemsCount == 1 && item->GetCount() == count[0])
    {
        ...
        LOG_INFO("network", "CMSG_AUCTION_SELL_ITEM: Player %s (guid %d) is selling item %s entry %u (guid %d) with count %u with initial bid " UI64FMTD " with buyout " UI64FMTD " and with time %u (in sec) in auctionhouse %u",
...
void WorldSession::HandleAuctionRemoveItem(WorldPacket& recvData)
{
    ...
    if (auction && auction->owner == player->GetGUID().GetCounter())
    {
        ...
        {
            LOG_ERROR("network", "Auction id: %u got non existing item (item guid : %u)!", auction->Id, auction->itemGUIDLow);
    ...
    else
    {
        ...
        LOG_INFO("entities.player.cheat", "CHEATER : %u, he tried to cancel auction (id: %u) of another player, or auction is nullptr", player->GetGUID().GetCounter(), auctionId);
        return;
    }
