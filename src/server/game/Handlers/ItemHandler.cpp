...
void WorldSession::HandleBuyItemOpcode(WorldPacket& recvData)
{
    ...
    if (itemType == ITEM_VENDOR_TYPE_ITEM)
    {
        ...
        LOG_DEBUG("network", "WORLD: received wrong itemType (%u) in HandleBuyItemOpcode", itemType);
}
...
void WorldSession::HandleBuyBankSlotOpcode(WorldPacket& recvPacket)
{
    ...
    if (!CanUseBank(guid))
    {
        ...
        SendPacket(&data);
        LOG_DEBUG("network", "WORLD: HandleBuyBankSlotOpcode - %s not found or you can't interact with him.", guid.ToString().c_str());
        return;
    }
...
void WorldSession::HandleTransmogrifyItems(WorldPackets::Item::TransmogrifyItems& transmogrifyItems)
{
    ...
    {
        LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify more items than possible. Amount: (%u)", player->GetGUID().GetCounter(), player->GetName().c_str(), uint32(transmogrifyItems.Items.size()));
        return;
    }
    ...
    for (WorldPackets::Item::TransmogrifyItem& item : transmogrifyItems.Items)
    {
        ...
        {
            LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify %s with a wrong slot (%u) when transmogrifying items.", player->GetGUID().GetCounter(), player->GetName().c_str(), item.SrcItemGUID.ToString().c_str(), item.Slot);
            return;
        }
        ...
        if (!targetItem)
        {
            LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify an invalid item in a valid slot (slot: %u).", player->GetGUID().GetCounter(), player->GetName().c_str(), item.Slot);
            return;
        }
        ...
        if (item.ItemID)
        {
            ...
            if (!proto)
            {
                LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify to an invalid item (entry: %u).", player->GetGUID().GetCounter(), player->GetName().c_str(), item.ItemID);
                return;
            }
            ...
            if (!sourceItem)
            {
                LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify with an invalid item (%s).", player->GetGUID().GetCounter(), player->GetName().c_str(), item.SrcItemGUID.ToString().c_str());
                return;
            }
            ...
            {
                LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify with an invalid entry (entry: %u) for %s.", player->GetGUID().GetCounter(), player->GetName().c_str(), item.ItemID, item.SrcItemGUID.ToString().c_str());
                return;
            }
            ...
            {
                LOG_DEBUG("network", "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) failed CanTransmogrifyItemWithItem (%u with %u).", player->GetGUID().GetCounter(), player->GetName().c_str(), targetItem->GetEntry(), sourceItem->GetEntry());
                return;
            }
...
void WorldSession::HandleReforgeItemOpcode(WorldPacket& recvData)
{
    ...
    {
        LOG_DEBUG("network", "WORLD: HandleReforgeItemOpcode - %s not found or player can't interact with it.", guid.ToString().c_str());
    ...
    if (!item)
    {
        LOG_DEBUG("network", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to reforge an invalid/non-existant item.", player->GetGUID().GetCounter(), player->GetName().c_str());
        SendReforgeResult(false);
        return;
    }
    ...
    if (!reforgeEntry)
    {
        ...
        {
            LOG_ERROR("network", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to remove reforge from non-reforged item (Entry: %u)", player->GetGUID().GetCounter(), player->GetName().c_str(), item->GetEntry());
            SendReforgeResult(false);
            return;
        }
    ...
    if (item->GetEnchantmentId(REFORGE_ENCHANTMENT_SLOT))
    {
        LOG_ERROR("network", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to reforge an already reforged item (Entry: %u)", player->GetGUID().GetCounter(), player->GetName().c_str(), item->GetEntry());
        SendReforgeResult(false);
        return;
    }
    ...
    if (!stats)
    {
        LOG_DEBUG("network", "WORLD: HandleReforgeItemOpcode - Player (Guid: %u Name: %s) tried to reforge an item with invalid reforge entry (%u).", player->GetGUID().GetCounter(), player->GetName().c_str(), reforgeEntry);
        SendReforgeResult(false);
        return;
    }
