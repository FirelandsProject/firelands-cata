...
void WorldSession::HandleDismissControlledVehicle(WorldPacket &recvData)
{
    ...
    {
        LOG_ERROR("network", "Player %s tried to dismiss a controlled vehicle (%s) that he has no control over. Possible cheater or malformed packet.",
...
void WorldSession::HandleChangeSeatsOnControlledVehicle(WorldPacket& recvData)
{
    ...
    {
        recvData.rfinish();                                // prevent warnings spam
        LOG_ERROR("network", "HandleChangeSeatsOnControlledVehicle, Opcode: %u, Player %u tried to switch seats but current seatflags %u don't permit that.",
...
void WorldSession::HandleEjectPassenger(WorldPacket& data)
{
    ...
    if (guid.IsUnit())
    {
        ...
        if (seat->IsEjectable())
            ...
            LOG_ERROR("network", "Player %u attempted to eject %s from non-ejectable seat.", GetPlayer()->GetGUID().GetCounter(), guid.ToString().c_str());
    }
    else
    ...
    if (Vehicle* vehicle = GetPlayer()->GetVehicle())
    {
        if (VehicleSeatEntry const* seat = vehicle->GetSeatForPassenger(GetPlayer()))
        {
            if (seat->CanEnterOrExit())
            {
                ...
                LOG_ERROR("network", "Player %u tried to exit vehicle, but seatflags %u (ID: %u) don't permit that.",
