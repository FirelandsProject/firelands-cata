...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData) {
  ...
  switch (recvData.GetOpcode()) {
    ...
    case CMSG_MESSAGECHAT_RAID_WARNING:
      ...
      break;
    default:
      LOG_ERROR("network", "HandleMessagechatOpcode : Unknown chat opcode (%u)",
                recvData.GetOpcode());
      recvData.hexlike();
      return;
  }
  ...
  if (type != CHAT_MSG_EMOTE && type != CHAT_MSG_AFK && type != CHAT_MSG_DND) {
    ...
    if (lang == LANG_UNIVERSAL) {
      LOG_INFO("entities.player.cheat",
               "CMSG_MESSAGECHAT: Possible hacking-attempt: %s tried to send a "
               "message in universal language",
               GetPlayerInfo().c_str());
    ...
    if (langDesc->skill_id != 0 && !sender->HasSkill(langDesc->skill_id)) {
      ...
      Unit::AuraEffectList const& langAuras =
    ...
    if (lang == LANG_ADDON) {
      ...
      switch (type) {
        ...
        default:
          LOG_ERROR("network",
                    "Player %s (GUID: %u) sent a chatmessage with an invalid "
    ...
    // LANG_ADDON should not be changed nor be affected by flood control
      ...
      // send in universal language if player in .gm on mode (ignore spell
      // effects)
      if (sender->IsGameMaster())
        ...
        if (!ModLangAuras.empty()) lang = ModLangAuras.front()->GetMiscValue();
      }

      if (!sender->CanSpeak()) {
        std::string timeStr =
            secsToTimeString(m_muteTime - GameTime::GetGameTime());
        SendNotification(GetFirelandsString(LANG_WAIT_BEFORE_SPEAKING),
                         timeStr.c_str());
  ...
  if (sender->HasAura(1852) && type != CHAT_MSG_WHISPER) {
    SendNotification(GetFirelandsString(LANG_GM_SILENCE),
  ...
  if (!ignoreChecks) {
    ...
    if (lang == LANG_ADDON) {
      if (AddonChannelCommandHandler(this).ParseCommands(msg.c_str())) return;
    }
    if (lang != LANG_ADDON) {
      if (ChatHandler(this).ParseCommands(msg.c_str())) return;
  ...
  switch (type) {
    case CHAT_MSG_SAY: {
      ...
      if (sender->getLevel() <
        ...
        SendNotification(GetFirelandsString(LANG_SAY_REQ),
                         sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
        return;
      }
    ...
    case CHAT_MSG_EMOTE: {
      ...
      if (sender->getLevel() <
          sWorld->getIntConfig(CONFIG_CHAT_EMOTE_LEVEL_REQ)) {
        SendNotification(GetFirelandsString(LANG_SAY_REQ),
                         sWorld->getIntConfig(CONFIG_CHAT_EMOTE_LEVEL_REQ));
        return;
      }
    ...
    case CHAT_MSG_YELL: {
      ...
      if (sender->getLevel() <
          sWorld->getIntConfig(CONFIG_CHAT_YELL_LEVEL_REQ)) {
        SendNotification(GetFirelandsString(LANG_SAY_REQ),
                         sWorld->getIntConfig(CONFIG_CHAT_YELL_LEVEL_REQ));
        return;
      }
    ...
    case CHAT_MSG_WHISPER: {
      ...
      if (!receiver || (lang != LANG_ADDON && !receiver->isAcceptWhispers() &&
      ...
      if (!sender->IsGameMaster() &&
          ...
          !receiver->IsInWhisperWhiteList(sender->GetGUID())) {
        SendNotification(GetFirelandsString(LANG_WHISPER_REQ),
                         sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
        return;
      }
      ...
      // If player is a Gamemaster and doesn't accept whisper, we auto-whitelist
      // every player that the Gamemaster is talking to We also do that if a
      // player is under the required level for whispers.
      if (receiver->getLevel() <
              sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ) ||
          (!sender->isAcceptWhispers() &&
    ...
    case CHAT_MSG_PARTY:
    case CHAT_MSG_PARTY_LEADER: {
      // if player is in battleground, he cannot say to battleground members by
      // /p
      ...
      if (group->IsLeader(GetPlayer()->GetGUID())) type = CHAT_MSG_PARTY_LEADER;

      sScriptMgr->OnPlayerChat(GetPlayer(), type, lang, msg, group);

      WorldPacket data;
      ChatHandler::BuildChatPacket(data, ChatMsg(type), Language(lang), sender,
                                   nullptr, msg);
    ...
    case CHAT_MSG_RAID_LEADER: {
      ...
      if (group->IsLeader(GetPlayer()->GetGUID())) type = CHAT_MSG_RAID_LEADER;
    ...
    case CHAT_MSG_RAID_WARNING: {
      ...
      if (!group ||
          !(group->isRaidGroup() ||
            sWorld->getBoolConfig(CONFIG_CHAT_PARTY_RAID_WARNINGS)) ||
      ...
      // code is ok
      ChatHandler::BuildChatPacket(data, CHAT_MSG_RAID_WARNING, Language(lang),
                                   sender, nullptr, msg);
      group->BroadcastPacket(&data, false);
      break;
    }
    ...
    case CHAT_MSG_CHANNEL: {
      if (AccountMgr::IsPlayerAccount(GetSecurity())) {
        if (sender->getLevel() <
          ...
          SendNotification(GetFirelandsString(LANG_CHANNEL_REQ),
                           sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
          return;
        }
      }
      ...
      if (Channel* chn =
              ChannelMgr::GetChannelForPlayerByNamePart(channel, sender)) {
    ...
    case CHAT_MSG_AFK: {
      if (!sender->IsInCombat()) {
        ...
        {
          sender->autoReplyMsg =
              msg.empty() ? GetFirelandsString(LANG_PLAYER_AFK_DEFAULT) : msg;
    ...
    case CHAT_MSG_DND: {
      ...
      {
        sender->autoReplyMsg =
            msg.empty() ? GetFirelandsString(LANG_PLAYER_DND_DEFAULT) : msg;
    ...
    default:
      LOG_ERROR("network", "CHAT: unknown message type %u, lang: %u", type,
                lang);
      break;
  }
}
...
void WorldSession::HandleAddonMessagechatOpcode(WorldPacket& recvData) {
  ...
  switch (recvData.GetOpcode()) {
    ...
    default:
      LOG_ERROR("network",
                "HandleAddonMessagechatOpcode: Unknown addon chat opcode (%u)",
                recvData.GetOpcode());
      recvData.hexlike();
      return;
  }
  ...
  switch (type) {
    case CHAT_MSG_BATTLEGROUND: {
      ...
      WorldPacket data;
      ChatHandler::BuildChatPacket(data, type, LANG_ADDON, sender, nullptr,
                                   message, 0U, "", DEFAULT_LOCALE, prefix);
    ...
    case CHAT_MSG_OFFICER: {
      if (sender->GetGuildId())
        if (Guild* guild = sGuildMgr->GetGuildById(sender->GetGuildId()))
          guild->BroadcastAddonToGuild(this, type == CHAT_MSG_OFFICER, message,
                                       prefix);
      break;
    }
    case CHAT_MSG_WHISPER: {
      if (!normalizePlayerName(targetName)) break;
    ...
    case CHAT_MSG_RAID: {
      Group* group = sender->GetGroup();
      if (!group || group->isBGGroup()) break;

      WorldPacket data;
      ChatHandler::BuildChatPacket(data, type, LANG_ADDON, sender, nullptr,
                                   message, 0U, "", DEFAULT_LOCALE, prefix);
      group->BroadcastAddonMessagePacket(
          &data, prefix, true, -1, group->GetMemberGroup(sender->GetGUID()));
      break;
    }
    default: {
      LOG_ERROR("misc",
...
void WorldSession::HandleEmoteOpcode(WorldPacket& recvData) {
  ...
  if (emote != EMOTE_ONESHOT_NONE && emote != EMOTE_ONESHOT_WAVE) return;
...
class EmoteChatBuilder {
 public:
  EmoteChatBuilder(Player const& player, uint32 text_emote, uint32 emote_num,
                   Unit const* target)
      : i_player(player),
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket& recvData) {
  ...
  switch (emote_anim) {
    ...
    default:
      ...
      if (GetPlayer()->HasUnitState(UNIT_STATE_DIED)) break;
  ...
  CellCoord p = Firelands::ComputeCellCoord(GetPlayer()->GetPositionX(),
                                            GetPlayer()->GetPositionY());

  Cell cell(p);
  cell.SetNoCreate();

  Firelands::EmoteChatBuilder emote_builder(*GetPlayer(), text_emote, emoteNum,
                                            unit);
  Firelands::LocalizedPacketDo<Firelands::EmoteChatBuilder> emote_do(
      emote_builder);
  Firelands::PlayerDistWorker<
      Firelands::LocalizedPacketDo<Firelands::EmoteChatBuilder> >
      emote_worker(GetPlayer(),
                   sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE),
                   emote_do);
  TypeContainerVisitor<Firelands::PlayerDistWorker<Firelands::LocalizedPacketDo<
                           Firelands::EmoteChatBuilder> >,
                       WorldTypeMapContainer>
      message(emote_worker);
  cell.Visit(p, message, *GetPlayer()->GetMap(), *GetPlayer(),
             sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE));

  GetPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_DO_EMOTE,
                                         text_emote, 0, 0, unit);
...
void WorldSession::HandleChatIgnoredOpcode(WorldPacket& recvData) {
  ...
  WorldPacket data;
  ChatHandler::BuildChatPacket(data, CHAT_MSG_IGNORED, LANG_UNIVERSAL, _player,
                               _player, GetPlayer()->GetName());
