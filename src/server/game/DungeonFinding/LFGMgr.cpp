...
void LFGMgr::_SaveToDB(ObjectGuid guid, uint32 db_guid) {
  ...
  CharacterDatabasePreparedStatement* stmt =
      CharacterDatabase.GetPreparedStatement(CHAR_DEL_LFG_DATA);
...
void LFGMgr::LoadRewards() {
  ...
  QueryResult result = WorldDatabase.Query(
      "SELECT dungeonId, maxLevel, firstQuestId, otherQuestId, "
      "shortageQuestId, completionsPerPeriod, dailyReset FROM "
      "lfg_dungeon_rewards ORDER BY dungeonId, maxLevel ASC");

  if (!result) {
    LOG_ERROR("server.loading",
              ">> Loaded 0 lfg dungeon rewards. DB table `lfg_dungeon_rewards` "
              "is empty!");
    return;
  }
  ...
  do {
    ...
    if (!GetLFGDungeonEntry(dungeonId)) {
      LOG_ERROR(
          "sql.sql",
          "Dungeon %u specified in table `lfg_dungeon_rewards` does not exist!",
          dungeonId);
      continue;
    }
    ...
    if (!maxLevel || maxLevel > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)) {
      LOG_ERROR("sql.sql",
                "Level %u specified for dungeon %u in table "
    ...
    if (!firstQuestId || !sObjectMgr->GetQuestTemplate(firstQuestId)) {
      LOG_ERROR("sql.sql",
                "First quest %u specified for dungeon %u in table "
    ...
    if (otherQuestId && !sObjectMgr->GetQuestTemplate(otherQuestId)) {
      LOG_ERROR("sql.sql",
                "Other quest %u specified for dungeon %u in table "
                "`lfg_dungeon_rewards` does not exist!",
    ...
    if (shortageQuestId && !sObjectMgr->GetQuestTemplate(shortageQuestId)) {
      LOG_ERROR("sql.sql",
                "Shortage quest %u specified for dungeon %u in table "
                "`lfg_dungeon_rewards` does not exist!",
    ...
    RewardMapStore.insert(LfgRewardContainer::value_type(
        dungeonId,
        new LfgReward(maxLevel, firstQuestId, otherQuestId, shortageQuestId,
                      completionsPerPeriod, dailyReset)));
    ++count;
  } while (result->NextRow());

  LOG_INFO("server.loading", ">> Loaded %u lfg dungeon rewards in %u ms", count,
...
void LFGMgr::LoadLFGDungeons(bool reload /* = false */) {
  ...
  for (uint32 i = 0; i < sLFGDungeonStore.GetNumRows(); ++i) {
    ...
    switch (dungeon->TypeID) {
      ...
      case LFG_TYPE_RAID:
        LfgDungeonStore.insert(LFGDungeonContainer::value_type(
            dungeon->ID, LFGDungeonData(dungeon)));
        break;
  ...
  QueryResult result = WorldDatabase.Query(
      ...
      "requiredItemLevel FROM lfg_dungeon_template");
  if (!result) {
    LOG_ERROR("server.loading",
              ">> Loaded 0 lfg dungeon templates. DB table "
  ...
  do {
    ...
    if (dungeonItr == LfgDungeonStore.end()) {
      LOG_ERROR("sql.sql",
                "table `lfg_dungeon_template` contains coordinates for wrong "
                "dungeon %u",
                dungeonId);
      continue;
    }
  ...
  for (LFGDungeonContainer::iterator itr = LfgDungeonStore.begin();
    ...
    if (dungeon.type == LFG_TYPE_RANDOM)
      AddDungeonsFromGroupingMap(CachedDungeonMapStore, dungeon.group,
                                 dungeon.id);
    else
...
void LFGMgr::Update(uint32 diff) {
  ...
  for (LfgRoleCheckContainer::iterator it = RoleChecksStore.begin();
    ...
    for (LfgRolesMap::const_iterator itRoles = roleCheck.roles.begin();
         itRoles != roleCheck.roles.end(); ++itRoles) {
  ...
  for (LfgPlayerBootContainer::iterator it = BootsStore.begin();
    ...
    if (boot.cancelTime < currTime) {
      ...
      for (LfgAnswerContainer::const_iterator itVotes = boot.votes.begin();
           itVotes != boot.votes.end(); ++itVotes) {
  ...
  for (LfgQueueContainer::iterator it = QueuesStore.begin();
       it != QueuesStore.end(); ++it)
    if (uint8 newProposals = it->second.FindGroups())
      LOG_DEBUG("lfg.update", "Found %u new groups in queue %u", newProposals,
                it->first);
        ...
        if (ObjectGuid gguid = GetGroup(guid)) {
          ...
          SendLfgUpdateStatus(
              ...
              LfgUpdateData(LFG_UPDATETYPE_PROPOSAL_BEGIN,
                            GetSelectedDungeons(guid), GetComment(guid)),
              true);
        } else
          SendLfgUpdateStatus(
              guid,
...
void LFGMgr::JoinLfg(Player* player, uint8 roles, LfgDungeonSet& dungeons,
  ...
  uint32 rDungeonId = 0;
  bool isContinue =
      grp && grp->isLFGGroup() && GetState(gguid) != LFG_STATE_FINISHED_DUNGEON;
  ...
  else if (player->HasAura(FREEZE_DEBUFF))
    joinData.result = LFG_JOIN_NOT_MEET_REQS;
    ...
    if (grp->GetMembersCount() > MAXGROUPSIZE)
      ...
      for (GroupReference* itr = grp->GetFirstMember();
        ...
        if (Player* plrg = itr->GetSource()) {
          ...
          else if (plrg->InBattleground() || plrg->InArena() ||
  ...
  if (joinData.result == LFG_JOIN_OK) {
    ...
    if (joinData.result == LFG_JOIN_OK) {
      ...
      if (dungeons.empty())
        joinData.result =
            grp ? LFG_JOIN_INTERNAL_ERROR : LFG_JOIN_NOT_MEET_REQS;
    }
  }
  ...
  if (joinData.result != LFG_JOIN_OK) {
    LOG_DEBUG("lfg.join",
              "%s joining with %u members. Result: %u, Dungeons: %s",
              guid.ToString().c_str(), grp ? grp->GetMembersCount() : 1,
              joinData.result, ConcatenateDungeons(dungeons).c_str());
  ...
  } else  // Add player to queue
  {
    ...
    player->GetSession()->SendLfgUpdateStatus(
        LfgUpdateData(LFG_UPDATETYPE_JOIN_QUEUE_INITIAL, dungeons, comment),
        false);
    SetState(guid, isRaid ? LFG_STATE_RAIDBROWSER : LFG_STATE_QUEUED);
    player->GetSession()->SendLfgUpdateStatus(
        LfgUpdateData(isRaid ? LFG_UPDATETYPE_JOIN_RAIDBROWSER
  ...
  LOG_DEBUG("lfg.join", "%s joined (%s), Members: %s. Dungeons (%u): %s",
            guid.ToString().c_str(), grp ? "group" : "player",
            debugNames.c_str(), uint32(dungeons.size()),
...
void LFGMgr::LeaveLfg(ObjectGuid guid, bool disconnected) {
  ...
  LOG_DEBUG("lfg.leave", "%s left (%s)", guid.ToString().c_str(),
            guid == gguid ? "group" : "player");
  ...
  switch (state) {
    ...
    case LFG_STATE_RAIDBROWSER: {
      ...
      if (gguid) {
        ...
        // Set the new state to LFG_STATE_DUNGEON/LFG_STATE_FINISHED_DUNGEON if
        // the group is already in a dungeon This is required in case a LFG
        // group vote-kicks a player in a dungeon, queues, then leaves the queue
        // (maybe to queue later again)
        if (Group* group = sGroupMgr->GetGroupByGUID(gguid.GetCounter()))
          if (group->isLFGGroup() && GetDungeon(gguid) &&
              (oldState == LFG_STATE_DUNGEON ||
               oldState == LFG_STATE_FINISHED_DUNGEON))
            newState = oldState;
        ...
        for (GuidSet::const_iterator it = players.begin(); it != players.end();
             ++it) {
...
void LFGMgr::UpdateRoleCheck(ObjectGuid gguid,
  ...
  if (!guid)
    ...
    if (itRoles == roleCheck.roles.end()) {
      ...
      roleCheck.state = (dungeon && CheckGroupRoles(check_roles, dungeon))
  ...
  LfgJoinResultData joinData =
      LfgJoinResultData(LFG_JOIN_ROLE_CHECK_FAILED, roleCheck.state);
  for (LfgRolesMap::const_iterator it = roleCheck.roles.begin();
       it != roleCheck.roles.end(); ++it) {
    ...
    switch (roleCheck.state) {
      ...
      case LFG_ROLECHECK_FINISHED:
        ...
        SendLfgUpdateStatus(pguid,
                            LfgUpdateData(LFG_UPDATETYPE_ADDED_TO_QUEUE,
                                          dungeons, GetComment(pguid)),
                            true);
        break;
      default:
        if (roleCheck.leader == pguid) SendLfgJoinResult(pguid, joinData);
        SendLfgUpdateStatus(
            pguid, LfgUpdateData(LFG_UPDATETYPE_ROLECHECK_FAILED), true);
  ...
  if (roleCheck.state == LFG_ROLECHECK_FINISHED) {
    ...
    LFGQueue& queue = GetQueue(gguid);
    queue.AddQueueData(gguid, GameTime::GetGameTime(), roleCheck.dungeons,
                       roleCheck.roles);
    RoleChecksStore.erase(itRoleCheck);
...
void LFGMgr::GetCompatibleDungeons(LfgDungeonSet& dungeons,
                                   ...
                                   LfgLockPartyMap& lockMap, bool isContinue) {
  lockMap.clear();

  std::map<uint32, uint32> lockedDungeons;

  for (GuidSet::const_iterator it = players.begin();
       it != players.end() && !dungeons.empty(); ++it) {
    ObjectGuid guid = (*it);
    ...
    for (LfgLockMap::const_iterator it2 = cachedLockMap.begin();
      ...
      if (itDungeon != dungeons.end()) {
        ...
        // Don't remove the dungeon if team members are trying to continue a
        // locked instance
        if (it2->second.lockStatus == LFG_LOCKSTATUS_RAID_LOCKED &&
            isContinue) {
          LFGDungeonData const* dungeon = GetLFGDungeon(dungeonId);
          ASSERT(dungeon);
          ASSERT(player);
          if (InstancePlayerBind* playerBind = player->GetBoundInstance(
                  dungeon->map, Difficulty(dungeon->difficulty))) {
...
void LFGMgr::MakeNewGroup(LfgProposal const& proposal) {
  ...
  for (LfgProposalPlayerContainer::const_iterator it = proposal.players.begin();
       it != proposal.players.end(); ++it) {
  ...
  Group* grp = proposal.group
                   ...
                   : nullptr;
  for (GuidList::const_iterator it = players.begin(); it != players.end();
       ++it) {
    ObjectGuid pguid = (*it);
    ...
    for (GuidList::const_iterator itr = players.begin(); itr != players.end();
         ++itr) {
  ...
  // Teleport Player
  for (GuidList::const_iterator it = playersToTeleport.begin();
       it != playersToTeleport.end(); ++it)
...
void LFGMgr::UpdateProposal(uint32 proposalId, ObjectGuid guid, bool accept) {
  ...
  LOG_DEBUG("lfg.proposal.update", "%s, Proposal %u, Selection: %u",
            guid.ToString().c_str(), proposalId, accept);
  if (!accept) {
  ...
  LfgUpdateData updateData = LfgUpdateData(
      LFG_UPDATETYPE_GROUP_FOUND, GetSelectedDungeons(guid), GetComment(guid));
  for (LfgProposalPlayerContainer::const_iterator it = proposal.players.begin();
       it != proposal.players.end(); ++it) {
    ObjectGuid pguid = it->first;
  ...
  for (GuidList::const_iterator it = proposal.queues.begin();
       it != proposal.queues.end(); ++it)
...
void LFGMgr::RemoveProposal(LfgProposalContainer::iterator itProposal,
  ...
  LOG_DEBUG("lfg.proposal.remove", "Proposal %u, state FAILED, UpdateType %u",
            itProposal->first, type);
  ...
  for (LfgProposalPlayerContainer::const_iterator it = proposal.players.begin();
                         ...
                         // didn't accept
    {
      ...
      if (it->second.accept == LFG_ANSWER_DENY) {
        ...
        LOG_DEBUG("lfg.proposal.remove",
                  "%s didn't accept. Removing from queue and compatible cache",
                  guid.ToString().c_str());
      } else {
        updateData.updateType = LFG_UPDATETYPE_REMOVED_FROM_QUEUE;
        LOG_DEBUG("lfg.proposal.remove",
                  "%s in same group that someone that didn't accept. Removing "
                  "from queue and compatible cache",
                  guid.ToString().c_str());
      }

      RestoreState(guid,
                   "Proposal Fail (didn't accepted or in group with someone "
                   "that didn't accept");
      if (gguid != guid) {
        RestoreState(it->second.group,
      ...
      LOG_DEBUG("lfg.proposal.remove", "Readding %s to queue.",
                guid.ToString().c_str());
  ...
  // Remove players/groups from queue
  for (GuidSet::const_iterator it = toRemove.begin(); it != toRemove.end();
       ++it) {
    ObjectGuid guid = *it;
    queue.RemoveFromQueue(guid);
...
*/
void LFGMgr::InitBoot(ObjectGuid gguid, ObjectGuid kicker, ObjectGuid victim,
  ...
  // Set votes
  for (GuidSet::const_iterator itr = players.begin(); itr != players.end();
       ++itr) {
    ObjectGuid guid = (*itr);
...
void LFGMgr::UpdateBoot(ObjectGuid guid, bool accept) {
  ...
  if (agreeNum < (raidFinderGroup ? LFR_GROUP_KICK_VOTES_NEEDED
                                  : LFG_GROUP_KICK_VOTES_NEEDED) &&
      (votesNum - agreeNum) < (raidFinderGroup ? LFR_GROUP_KICK_VOTES_NEEDED
                                               : LFG_GROUP_KICK_VOTES_NEEDED))
    return;
  ...
  if (agreeNum ==
      (raidFinderGroup
           ? LFR_GROUP_KICK_VOTES_NEEDED
           : LFG_GROUP_KICK_VOTES_NEEDED))  // Vote passed - Kick player
  {
    if (Group* group = sGroupMgr->GetGroupByGUID(gguid.GetCounter()))
...
/**
   ...
   false)
*/
void LFGMgr::TeleportPlayer(Player* player, bool out,
                            bool fromOpcode /*= false*/,
                            bool saveEntryPoint /*= true*/) {
  LFGDungeonData const* dungeon = nullptr;
  Group* group = player->GetGroup();
  ...
  if (!dungeon) {
    LOG_DEBUG("lfg.teleport",
              "Player %s not in group/lfggroup or dungeon not found!",
  ...
  if (out) {
    LOG_DEBUG(
        "lfg.teleport",
        "Player %s is being teleported out. Current Map %u - Expected Map %u",
  ...
  else if (player->GetMapId() !=
           uint32(dungeon->map))  // Do not teleport players in dungeon to the
                                  // entrance
  {
    ...
    if (!fromOpcode && saveEntryPoint) {
      ...
      for (GroupReference* itr = group->GetFirstMember(); itr != nullptr;
        ...
        if (plrg && plrg != player &&
            plrg->GetMapId() == uint32(dungeon->map)) {
          mapid = plrg->GetMapId();
          x = plrg->GetPositionX();
          y = plrg->GetPositionY();
          z = plrg->GetPositionZ();
  ...
  LOG_DEBUG("lfg.teleport",
            ...
            player->GetName().c_str(), dungeon->map, dungeon->x, dungeon->y,
            dungeon->z, error);
}
...
void LFGMgr::FinishDungeon(ObjectGuid gguid, const uint32 dungeonId,
  ...
  if (gDungeonId != dungeonId) {
    LOG_DEBUG("lfg.dungeon.finish",
              "Group %s finished dungeon %u but queued for %u",
  ...
  if (GetState(gguid) ==
      LFG_STATE_FINISHED_DUNGEON)  // Shouldn't happen. Do not reward multiple
                                   // times
  {
    LOG_DEBUG("lfg.dungeon.finish", "Group: %s already rewarded",
              gguid.ToString().c_str());
    return;
  }
  ...
  for (GuidSet::const_iterator it = players.begin(); it != players.end();
    ...
    if (GetState(guid) == LFG_STATE_FINISHED_DUNGEON) {
      LOG_DEBUG("lfg.dungeon.finish", "Group: %s, Player: %s already rewarded",
    ...
    SendLfgUpdateStatus(
        guid,
        LfgUpdateData(LFG_UPDATETYPE_DUNGEON_FINISHED,
                      GetSelectedDungeons(guid), GetComment(guid)),
        true);
    ...
    if (!dungeon || (dungeon->type != LFG_TYPE_RANDOM && !dungeon->seasonal)) {
      LOG_DEBUG("lfg.dungeon.finish",
                "Group: %s, Player: %s dungeon %u is not random or seasonal",
    ...
    if (!player) {
      LOG_DEBUG("lfg.dungeon.finish",
                "Group: %s, Player: %s not found in world",
                gguid.ToString().c_str(), guid.ToString().c_str());
      continue;
    }
    ...
    if (player->FindMap() != currMap) {
      LOG_DEBUG("lfg.dungeon.finish",
                "Group: %s, Player: %s is in a different map",
                gguid.ToString().c_str(), guid.ToString().c_str());
      continue;
    }
    ...
    if (player->GetMapId() != mapId) {
      LOG_DEBUG("lfg.dungeon.finish",
                "Group: %s, Player: %s is in map %u and should be in %u to get "
                "reward",
                gguid.ToString().c_str(), guid.ToString().c_str(),
                player->GetMapId(), mapId);
      continue;
    }
    ...
    if (dungeon->difficulty == DUNGEON_DIFFICULTY_HEROIC)
      player->UpdateAchievementCriteria(
          ACHIEVEMENT_CRITERIA_TYPE_USE_LFD_TO_GROUP_WITH_PLAYERS, 1);
    ...
    // CanRewardQuest to check currency caps, SatisfyFirstLFGReward to check
    // weekly/daily reward caps for first quest
    if (player->CanRewardQuest(quest, false) &&
        player->SatisfyFirstLFGReward(rDungeonId,
                                      reward->completionsPerPeriod)) {
    ...
    // Give rewards
    LOG_DEBUG("lfg.dungeon.finish",
              "Group: %s, Player: %s done dungeon %u, %s previously done.",
              gguid.ToString().c_str(), guid.ToString().c_str(),
              GetDungeon(gguid), !firstReward ? " " : " not");
    LfgPlayerRewardData data =
        LfgPlayerRewardData(dungeon->Entry(), GetDungeon(gguid, false),
                            !firstReward, quest, shortageQuest);
...
LfgState LFGMgr::GetState(ObjectGuid guid) {
  ...
  if (guid.IsGroup()) {
    ...
    LOG_TRACE("lfg.data.group.state.get", "Group: %s, State: %s",
              guid.ToString().c_str(), GetStateString(state).c_str());
  } else {
    state = PlayersStore[guid].GetState();
    LOG_TRACE("lfg.data.player.state.get", "Player: %s, State: %s",
              guid.ToString().c_str(), GetStateString(state).c_str());
  }
...
LfgState LFGMgr::GetOldState(ObjectGuid guid) {
  ...
  if (guid.IsGroup()) {
    ...
    LOG_TRACE("lfg.data.group.oldstate.get", "Group: %s, Old state: %u",
              guid.ToString().c_str(), state);
  } else {
    state = PlayersStore[guid].GetOldState();
    LOG_TRACE("lfg.data.player.oldstate.get", "Player: %s, Old state: %u",
              guid.ToString().c_str(), state);
  }
...
bool LFGMgr::IsVoteKickActive(ObjectGuid gguid) {
  ...
  LOG_TRACE("lfg.data.group.votekick.get", "Group: %s, Active: %d",
            gguid.ToString().c_str(), active);
...
uint32 LFGMgr::GetDungeon(ObjectGuid guid, bool asId /*= true */) {
  ...
  LOG_TRACE("lfg.data.group.dungeon.get", "Group: %s, asId: %u, Dungeon: %u",
            guid.ToString().c_str(), asId, dungeon);
  return dungeon;
}
...
uint32 LFGMgr::GetDungeonMapId(ObjectGuid guid) {
  ...
  LOG_TRACE("lfg.data.group.dungeon.map",
            "Group: %s, MapId: %u (DungeonId: %u)", guid.ToString().c_str(),
            mapId, dungeonId);

  return mapId;
}

uint32 LFGMgr::GetDungeonIdForDifficulty(uint32 dungeonId,
...
uint8 LFGMgr::GetRoles(ObjectGuid guid) {
  ...
  LOG_TRACE("lfg.data.player.role.get", "Player: %s, Role: %u",
            guid.ToString().c_str(), roles);
  return roles;
}

const std::string& LFGMgr::GetComment(ObjectGuid guid) {
  LOG_TRACE("lfg.data.player.comment.get", "Player: %s, Comment: %s",
            guid.ToString().c_str(), PlayersStore[guid].GetComment().c_str());
  return PlayersStore[guid].GetComment();
}

LfgDungeonSet const& LFGMgr::GetSelectedDungeons(ObjectGuid guid) {
  LOG_TRACE(
      "lfg.data.player.dungeons.selected.get",
      "Player: %s, Selected Dungeons: %s", guid.ToString().c_str(),
      ConcatenateDungeons(PlayersStore[guid].GetSelectedDungeons()).c_str());
  return PlayersStore[guid].GetSelectedDungeons();
}

LfgLockMap const LFGMgr::GetLockedDungeons(ObjectGuid guid) {
  LOG_TRACE("lfg.data.player.dungeons.locked.get",
            "Player: %s, LockedDungeons.", guid.ToString().c_str());
  LfgLockMap lock;
  Player* player = ObjectAccessor::FindConnectedPlayer(guid);
  if (!player) {
    LOG_WARN("lfg.data.player.dungeons.locked.get",
             "Player: %s not ingame while retrieving his LockedDungeons.",
             guid.ToString().c_str());
    return lock;
  }
  ...
  for (LfgDungeonSet::const_iterator it = dungeons.begin();
    ...
    else if (DisableMgr::IsDisabledFor(DISABLE_TYPE_LFG_MAP, dungeon->map,
                                       player))
      lockStatus = LFG_LOCKSTATUS_RAID_LOCKED;
    else if (dungeon->difficulty > DUNGEON_DIFFICULTY_NORMAL &&
             player->GetBoundInstance(dungeon->map,
    ...
    (LFG_LOCKSTATUS_RAID_LOCKED) lockData = LFG_LOCKSTATUS_TOO_HIGH_GEAR_SCORE;
    ...
    */
    ...
    if (lockStatus)
      lock[dungeon->Entry()] =
          LfgLockInfoData(lockStatus, dungeon->requiredItemLevel,
...
uint8 LFGMgr::GetKicksLeft(ObjectGuid guid) {
  ...
  LOG_TRACE("lfg.data.group.kickleft.get", "Group: %s, Kicks left: %u",
            guid.ToString().c_str(), kicks);
  return kicks;
}
...
void LFGMgr::RestoreState(ObjectGuid guid, char const* debugMsg) {
  if (guid.IsGroup()) {
    ...
    LOG_TRACE("lfg.data.group.state.restore",
              "Group: %s (%s), State: %s, Old state: %s",
              guid.ToString().c_str(), debugMsg,
              GetStateString(data.GetState()).c_str(),
              GetStateString(data.GetOldState()).c_str());

    data.RestoreState();
  } else {
    LfgPlayerData& data = PlayersStore[guid];
    LOG_TRACE("lfg.data.player.state.restore",
              "Player: %s (%s), State: %s, Old state: %s",
              guid.ToString().c_str(), debugMsg,
              GetStateString(data.GetState()).c_str(),
              GetStateString(data.GetOldState()).c_str());
...
void LFGMgr::SetState(ObjectGuid guid, LfgState state) {
  if (guid.IsGroup()) {
    LfgGroupData& data = GroupsStore[guid];
    LOG_TRACE("lfg.data.group.state.set",
              "Group: %s, New state: %s, Previous: %s, Old state: %s",
              guid.ToString().c_str(), GetStateString(state).c_str(),
              GetStateString(data.GetState()).c_str(),
              GetStateString(data.GetOldState()).c_str());
    ...
    LfgPlayerData& data = PlayersStore[guid];
    LOG_TRACE("lfg.data.player.state.set",
              "Player: %s, New state: %s, Previous: %s, OldState: %s",
              guid.ToString().c_str(), GetStateString(state).c_str(),
              GetStateString(data.GetState()).c_str(),
              GetStateString(data.GetOldState()).c_str());
...
void LFGMgr::SetVoteKick(ObjectGuid gguid, bool active) {
  ...
  LOG_TRACE("lfg.data.group.votekick.set",
            "Group: %s, New state: %d, Previous: %d", gguid.ToString().c_str(),
...
void LFGMgr::SetDungeon(ObjectGuid guid, uint32 dungeon) {
  LOG_TRACE("lfg.data.group.dungeon.set", "Group: %s, Dungeon: %u",
            guid.ToString().c_str(), dungeon);
  GroupsStore[guid].SetDungeon(dungeon);
}

void LFGMgr::SetRoles(ObjectGuid guid, uint8 roles) {
  LOG_TRACE("lfg.data.player.role.set", "Player: %s, Roles: %u",
            guid.ToString().c_str(), roles);
  PlayersStore[guid].SetRoles(roles);
}

void LFGMgr::SetComment(ObjectGuid guid, std::string const& comment) {
  LOG_TRACE("lfg.data.player.comment.set", "Player: %s, Comment: %s",
            guid.ToString().c_str(), comment.c_str());
  PlayersStore[guid].SetComment(comment);
}

void LFGMgr::SetSelectedDungeons(ObjectGuid guid,
                                 LfgDungeonSet const& dungeons) {
  LOG_TRACE("lfg.data.player.dungeon.selected.set", "Player: %s, Dungeons: %s",
            guid.ToString().c_str(), ConcatenateDungeons(dungeons).c_str());
  PlayersStore[guid].SetSelectedDungeons(dungeons);
}

void LFGMgr::DecreaseKicksLeft(ObjectGuid guid) {
  GroupsStore[guid].DecreaseKicksLeft();
  LOG_TRACE("lfg.data.group.kicksleft.decrease", "Group: %s, Kicks: %u",
            guid.ToString().c_str(), GroupsStore[guid].GetKicksLeft());
}

void LFGMgr::SetTicket(ObjectGuid guid,
...
void LFGMgr::RemoveGroupData(ObjectGuid guid) {
  ...
  for (ObjectGuid playerGUID : players) {
    ...
    if (state != LFG_STATE_PROPOSAL) {
      ...
      SendLfgUpdateStatus(
          playerGUID, LfgUpdateData(LFG_UPDATETYPE_REMOVED_FROM_QUEUE), true);
    }
  }
...
uint8 LFGMgr::GetTeam(ObjectGuid guid) {
  ...
  LOG_TRACE("lfg.data.player.team.get", "Player: %s, Team: %u",
            guid.ToString().c_str(), team);
  return team;
}
...
void LFGMgr::SendLfgUpdateStatus(ObjectGuid guid, LfgUpdateData const& data,
                                 bool party) {
  if (Player* player = ObjectAccessor::FindConnectedPlayer(guid))
...
void LFGMgr::SendLfgUpdateProposal(ObjectGuid guid,
                                   LfgProposal const& proposal) {
  if (Player* player = ObjectAccessor::FindConnectedPlayer(guid))
    player->GetSession()->SendLfgUpdateProposal(proposal);
}

void LFGMgr::SendLfgQueueStatus(ObjectGuid guid,
...
LfgUpdateData LFGMgr::GetLfgStatus(ObjectGuid guid) {
  ...
  return LfgUpdateData(LFG_UPDATETYPE_UPDATE_STATUS, playerData.GetState(),
...
std::string LFGMgr::DumpQueueInfo(bool full) {
  ...
  for (LfgQueueContainer::const_iterator itr = QueuesStore.begin();
       itr != QueuesStore.end(); ++itr) {
...
bool LFGMgr::inLfgDungeonMap(ObjectGuid guid, uint32 map,
                             Difficulty difficulty) {
...
LfgDungeonSet LFGMgr::GetRandomAndSeasonalDungeons(uint8 level,
                                                   uint8 expansion) {
...
void LFGMgr::AddDungeonsFromGroupingMap(LfgCachedDungeonContainer& container,
                                        uint32 groupId, uint32 dungeonId) {
  ...
  for (auto itr : sLFGDungeonsGroupingMapStore) {
    if (itr->Random_lfgDungeonsID == dungeonId) {
      if (container[groupId].find(itr->LfgDungeonsID) ==
          container[groupId].end())
