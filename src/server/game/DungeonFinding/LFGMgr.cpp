...
void LFGMgr::LoadRewards()
{
    ...
    do
    {
        ...
        {
            LOG_ERROR("sql.sql", "Shortage quest %u specified for dungeon %u in table `lfg_dungeon_rewards` does not exist!", shortageQuestId, dungeonId);
            shortageQuestId = 0;
        }
...
void LFGMgr::FinishDungeon(ObjectGuid gguid, const uint32 dungeonId, Map const* currMap)
{
    ...
    for (GuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        // Give rewards
        LOG_DEBUG("lfg.dungeon.finish", "Group: %s, Player: %s done dungeon %u, %s previously done.", gguid.ToString().c_str(), guid.ToString().c_str(), GetDungeon(gguid), !firstReward ? " " : " not");
