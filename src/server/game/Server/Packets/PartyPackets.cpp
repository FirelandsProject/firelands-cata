...
void WorldPackets::Party::PartyMemberState::Initialize(Player const* player) {
  ...
  if (GetOpcode() == SMSG_PARTY_MEMBER_FULL_STATE) {
    ChangeMask = GROUP_UPDATE_FLAG_STATUS | GROUP_UPDATE_FLAG_CUR_HP |
                 GROUP_UPDATE_FLAG_MAX_HP | GROUP_UPDATE_FLAG_CUR_POWER |
                 GROUP_UPDATE_FLAG_MAX_POWER | GROUP_UPDATE_FLAG_LEVEL |
                 GROUP_UPDATE_FLAG_ZONE | GROUP_UPDATE_FLAG_WMO_GROUP_ID |
                 GROUP_UPDATE_FLAG_POSITION | GROUP_UPDATE_FLAG_AURAS |
                 GROUP_UPDATE_FLAG_PET_AURAS | GROUP_UPDATE_FLAG_PHASE |
                 GROUP_UPDATE_FLAG_UNK400000 | GROUP_UPDATE_FLAG_UNK800000 |
                 GROUP_UPDATE_FLAG_UNK1000000 | GROUP_UPDATE_FLAG_UNK2000000 |
                 GROUP_UPDATE_FLAG_UNK4000000 | GROUP_UPDATE_FLAG_UNK8000000 |
                 GROUP_UPDATE_FLAG_UNK10000000 | GROUP_UPDATE_FLAG_UNK20000000 |
                 GROUP_UPDATE_FLAG_UNK40000000;
    ...
    if (player->GetPet())
      ChangeMask |=
          GROUP_UPDATE_FLAG_PET_GUID | GROUP_UPDATE_FLAG_PET_NAME |
    ...
    if (player->GetVehicle()) ChangeMask |= GROUP_UPDATE_FLAG_VEHICLE_SEAT;
  } else {
  ...
  if (player->IsPvP()) MemberStats.Status |= MEMBER_STATUS_PVP;
  ...
  if (player->IsFFAPvP()) MemberStats.Status |= MEMBER_STATUS_PVP_FFA;

  if (player->isAFK()) MemberStats.Status |= MEMBER_STATUS_AFK;

  if (player->isDND()) MemberStats.Status |= MEMBER_STATUS_DND;
  ...
  if (player->GetVehicle() && player->GetVehicle()->GetVehicleInfo())
    MemberStats.VehicleSeat =
        player->GetVehicle()
            ->GetVehicleInfo()
            ->SeatID[player->m_movementInfo.transport.seat];

  // Auras
  MemberStats.AuraMask = player->GetAuraUpdateMaskForRaid();
  MemberStats.AuraCount = player->GetAppliedAuras().size();
  for (uint8 i = 0; i < MAX_AURAS; ++i) {
    if (!(MemberStats.AuraMask & (uint64(1) << i))) continue;
  ...
  // Phases
  PhasingHandler::FillPartyMemberPhase(&MemberStats.Phases,
                                       player->GetPhaseShift());
  ...
  if (player->GetPet()) {
    ...
    for (uint8 i = 0; i < MAX_AURAS; ++i) {
      if (!(MemberStats.PetStats->AuraMask & (uint64(1) << i))) continue;
...
ByteBuffer& operator<<(
  ...
  for (uint16 phaseId : phases.List) data << uint16(phaseId);
...
ByteBuffer& operator<<(ByteBuffer& data,
  ...
  for (int32 points : aura.Points) data << int32(points);
...
WorldPacket const* WorldPackets::Party::PartyMemberState::Write() {
  ...
  if (ChangeMask & GROUP_UPDATE_FLAG_PET_AURAS) {
    _worldPacket << uint8(_worldPacket.GetOpcode() ==
                          SMSG_PARTY_MEMBER_FULL_STATE);
    _worldPacket << uint64(
        MemberStats.PetStats.has_value() ? MemberStats.PetStats->AuraMask : 0);
    _worldPacket << uint32(MemberStats.PetStats.has_value() ? MAX_AURAS : 0);

    if (MemberStats.PetStats.has_value())
      for (WorldPackets::Party::PartyMemberAuraStates const& aura :
           MemberStats.PetStats->Auras)
        _worldPacket << aura;
  }
  ...
  if (ChangeMask & GROUP_UPDATE_FLAG_PHASE) _worldPacket << MemberStats.Phases;
...
WorldPacket const* WorldPackets::Party::PartyInvite::Write() {
  ...
  for (uint32 lfgSlot : LfgSlots) _worldPacket << uint32(lfgSlot);
...
ByteBuffer& operator<<(
    ByteBuffer& data,
    WorldPackets::Party::PartyPlayerInfo const& partyPlayerInfo) {
...
ByteBuffer& operator<<(ByteBuffer& data,
                       WorldPackets::Party::PartyLFGInfo const& lfgInfo) {
...
WorldPacket const* WorldPackets::Party::PartyUpdate::Write() {
  ...
  if (PartyFlags & GROUP_FLAG_LFG) _worldPacket << LfgInfo;
...
WorldPackets::Party::SummonRaidMemberValidateFailed::
    SummonRaidMemberValidateFailed(
        ...
        SummonRaidMemberValidateReasonCode reasonCode)
    : ServerPacket(SMSG_SUMMON_RAID_MEMBER_VALIDATE_FAILED,
                   4 + sizeof(SummonRaidMemberValidateReason)) {
  Members.emplace_back(memberGuid, AsUnderlyingType(reasonCode));
}

WorldPacket const*
WorldPackets::Party::SummonRaidMemberValidateFailed::Write() {
