...
WorldSession::WorldSession(uint32 id, std::string &&name,
                           ...
                           bool isARecruiter)
    : m_muteTime(mute_time), m_timeOutTime(0), AntiDOS(this), m_GUIDLow(0),
      _player(nullptr), _security(sec), _accountId(id),
      _accountName(std::move(name)), m_accountExpansion(expansion),
      m_expansion(
          std::min<uint8>(expansion, sWorld->getIntConfig(CONFIG_EXPANSION))),
      _warden(nullptr), _logoutTime(0), m_inQueue(false), m_playerLogout(false),
      m_playerRecentlyLogout(false), m_playerSave(false),
      m_sessionDbcLocale(sWorld->GetAvailableDbcLocale(locale)),
      m_sessionDbLocaleIndex(locale), m_latency(0),
      m_TutorialsChanged(TUTORIALS_FLAG_NONE), _filterAddonMessages(false),
      recruiterId(recruiter), isRecruiter(isARecruiter), _RBACData(nullptr),
      expireTime(60000), // 1 min after socket loss, session is deleted
      forceExit(false), m_currentBankerGUID(), _timeSyncClockDeltaQueue(6),
...
bool WorldSession::PlayerDisconnected() const {
  return !(m_Socket[CONNECTION_TYPE_REALM] &&
           m_Socket[CONNECTION_TYPE_REALM]->IsOpen() &&
           m_Socket[CONNECTION_TYPE_INSTANCE] &&
           m_Socket[CONNECTION_TYPE_INSTANCE]->IsOpen());
}
...
void WorldSession::SendPacket(WorldPacket const *packet,
  ...
  if (packet->GetConnection() != CONNECTION_TYPE_DEFAULT) {
    if (packet->GetConnection() != CONNECTION_TYPE_INSTANCE &&
        IsInstanceOnlyOpcode(packet->GetOpcode())) {
      LOG_ERROR("network.opcode",
                   "Prevented sending of instance only opcode %u with "
                   "connection type %u to %s",
                   packet->GetOpcode(), uint32(packet->GetConnection()),
                   GetPlayerInfo().c_str());
      return;
    }
  ...
  if (!m_Socket[conIdx]) {
    LOG_ERROR(
        "network.opcode",
        "Prevented sending of %s to non existent socket %u to %s",
        GetOpcodeNameForLogging(static_cast<OpcodeServer>(packet->GetOpcode()))
            .c_str(),
        uint32(conIdx), GetPlayerInfo().c_str());
    return;
  }
  ...
  if (!forced) {
    ...
    if (!handler || handler->Status == STATUS_UNHANDLED) {
      LOG_ERROR("network.opcode",
                   "Prevented sending disabled opcode %s to %s",
...
bool WorldSession::Update(uint32 diff, PacketFilter &updater) {
    ...
    try {
      ...
      case STATUS_LOGGEDIN:
        if (!_player) {
          ...
          //! sent while the server does not recognize the client to be in world
          //! yet. We will re-add the packets to the bottom of the queue and
          //! process them later.
          ...
          processedPackets =
              MAX_PROCESSED_PACKETS_IN_SAME_WORLDSESSION_UPDATE; // break out of
                                                                 // packet
                                                                 // processing
                                                                 // loop
      ...
      case STATUS_LOGGEDIN_OR_RECENTLY_LOGGOUT:
        if (!_player && !m_playerRecentlyLogout &&
            !m_playerLogout) // There's a short delay between _player = null and
                             // m_playerRecentlyLogout = true during logout
          LogUnexpectedOpcode(
  ...
  if (updater.ProcessUnsafe()) {
    ...
    if ((m_Socket[0] && !m_Socket[0]->IsOpen()) ||
        (m_Socket[1] && !m_Socket[1]->IsOpen())) {
...
void WorldSession::LogoutPlayer(bool save) {
  ...
  if (_player) {
    ...
    LOG_INFO(
        ...
        GetAccountId(), GetRemoteAddress().c_str(), _player->GetName().c_str(),
        _player->GetGUID().GetCounter(), _player->getLevel());
...
void WorldSession::Handle_NULL(WorldPacket &null) {
  LOG_ERROR(
      "network.opcode", "Received unhandled opcode %s from %s",
...
void WorldSession::SendConnectToInstance(
    WorldPackets::Auth::ConnectToSerial serial) {
  boost::system::error_code ignored_error;
  boost::asio::ip::tcp::endpoint instanceAddress = realm.GetAddressForClient(
      Firelands::Net::make_address(GetRemoteAddress(), ignored_error));
...
void WorldSession::ReadAddonsInfo(ByteBuffer &data) {
  ...
  if (size > 0xFFFFF) {
    LOG_DEBUG("addon",
                 "WorldSession::ReadAddonsInfo: AddOnInfo too big, size %u",
                 size);
    return;
  }
...
void WorldSession::SendAddonsInfo() {
  ...
  if (!bannedAddons->empty() &&
      bannedAddons->back().Timestamp <
          lastBannedAddOnTimestamp) // cheating attempt OR connecting to a realm
                                    // with different configured banned addons,
                                    // send everything
...
void WorldSession::InvalidateRBACData() {
  LOG_DEBUG("rbac",
               ...
               "%s, realmId: %d]",
               _RBACData->GetId(), _RBACData->GetName().c_str(),
               realm.Id.Realm);
  delete _RBACData;
  _RBACData = nullptr;
}
...
bool WorldSession::DosProtection::EvaluateOpcode(WorldPacket &p,
  ...
  case POLICY_BAN: {
    ...
    sWorld->BanAccount(bm, nameOrIp, duration, "DOS (Packet Flooding/Spoofing",
                       "Server: AutoDOS");
    LOG_WARN("network",
...
WorldSession::DosProtection::GetMaxPacketCounterAllowed(uint16 opcode) const {
  ...
  case CMSG_CALENDAR_GET_CALENDAR: //   0               1.5       medium upload
                                   //   bandwidth usage
