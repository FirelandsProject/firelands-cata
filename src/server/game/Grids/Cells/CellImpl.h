...
#ifndef FIRELANDS_CELLIMPL_H
#define FIRELANDS_CELLIMPL_H
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = Firelands::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = Firelands::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = Firelands::ComputeCellCoord(x + radius, y + radius).normalize();
...
{
    CellCoord p(Firelands::ComputeCellCoord(center_obj->GetPositionX(), center_obj->GetPositionY()));
    Cell cell(p);
    if (dont_load)
        cell.SetNoCreate();
