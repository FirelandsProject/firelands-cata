...
enum WorldBoolConfigs {
  ...
  CONFIG_UI_QUESTLEVELS_IN_DIALOGS,  // Should we add quest levels to the title
                                     // in the NPC dialogs?
...
struct FC_GAME_API CliCommandHolder {
  ...
  CliCommandHolder(void* callbackArg, char const* command, Print zprint,
...
class FC_GAME_API World {
 public:
  ...
  void SendServerMessage(ServerMessageType type, char const* text = "",
  ...
  void SendGlobalGMMessage(WorldPacket const* packet,
                           WorldSession* self = nullptr, uint32 team = 0);
  bool SendZoneMessage(uint32 zone, WorldPacket const* packet,
                       WorldSession* self = nullptr, uint32 team = 0);
  void SendZoneText(uint32 zone, char const* text, WorldSession* self = nullptr,
                    uint32 team = 0);
  ...
  void setBoolConfig(WorldBoolConfigs index, bool value) {
    if (index < BOOL_CONFIG_VALUE_COUNT) m_bool_configs[index] = value;
  }
  ...
  void setFloatConfig(WorldFloatConfigs index, float value) {
    if (index < FLOAT_CONFIG_VALUE_COUNT) m_float_configs[index] = value;
  }
  ...
  void setIntConfig(WorldIntConfigs index, uint32 value) {
    if (index < INT_CONFIG_VALUE_COUNT) m_int_configs[index] = value;
  }
  ...
  BanReturn BanAccount(BanMode mode, std::string const& nameOrIP,
                       std::string const& duration, std::string const& reason,
                       std::string const& author);
  BanReturn BanAccount(BanMode mode, std::string const& nameOrIP,
                       uint32 duration_secs, std::string const& reason,
                       std::string const& author);
  bool RemoveBanAccount(BanMode mode, std::string const& nameOrIP);
  BanReturn BanCharacter(std::string const& name, std::string const& duration,
