...
void Firelands::BroadcastTextBuilder::operator()(WorldPacket& data, LocaleConstant locale) const
{
...
size_t Firelands::BroadcastTextBuilder::operator()(WorldPacket* data, LocaleConstant locale) const
{
    BroadcastText const* bct = sObjectMgr->GetBroadcastText(_textId);
...
void Firelands::CustomChatTextBuilder::operator()(WorldPacket& data, LocaleConstant locale) const
{
    ChatHandler::BuildChatPacket(data, _msgType, _language, _source, _target, _text, 0, "", locale);
}

void Firelands::FirelandsStringChatBuilder::operator()(WorldPacket& data, LocaleConstant locale) const
{
    char const* text = sObjectMgr->GetFirelandsString(_textId, locale);
