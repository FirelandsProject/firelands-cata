...
pEffect SpellEffects[TOTAL_SPELL_EFFECTS] = {
    ...
    &Spell::EffectUnused,  // 20 SPELL_EFFECT_DODGE                    one
                           // spell: Dodge
    &Spell::EffectUnused,  // 21 SPELL_EFFECT_EVADE                    one
                           // spell: Evade (DND)
    ...
    &Spell::EffectUnused,        // 26 SPELL_EFFECT_DEFENSE                  one
                                 // spell: Defense
    ...
    &Spell::EffectUnused,     // 37 SPELL_EFFECT_SPELL_DEFENSE            one
                              // spell: SPELLDEFENSE (DND)
    ...
    &Spell::
        EffectTeleUnitsFaceCaster,  // 43
                                    // SPELL_EFFECT_TELEPORT_UNITS_FACE_CASTER
    &Spell::EffectLearnSkill,       // 44 SPELL_EFFECT_SKILL_STEP
    &Spell::EffectPlayMovie,        // 45 SPELL_EFFECT_PLAY_MOVIE
    &Spell::EffectUnused,  // 46 SPELL_EFFECT_SPAWN clientside, unit appears as
                           // if it was just spawned
    &Spell::EffectTradeSkill,   // 47 SPELL_EFFECT_TRADE_SKILL
    &Spell::EffectUnused,       // 48 SPELL_EFFECT_STEALTH                  one
                                // spell: Base Stealth
    &Spell::EffectUnused,       // 49 SPELL_EFFECT_DETECT                   one
                                // spell: Detect
    ...
    &Spell::EffectCreateRandomItem,  // 59 SPELL_EFFECT_CREATE_RANDOM_ITEM
                                     // create item base at spell specific loot
    ...
    &Spell::EffectUnused,        // 81 SPELL_EFFECT_CREATE_HOUSE             one
                                 // spell: Create House (TEST)
    ...
    &Spell::EffectKillCreditPersonal,  // 90 SPELL_EFFECT_KILL_CREDIT Kill
                                       // credit but only for single person
    &Spell::EffectUnused,  // 91 SPELL_EFFECT_THREAT_ALL               one
                           // spell: zzOLDBrainwash
    ...
    &Spell::EffectDurabilityDamagePCT,  // 115
    ...
    &Spell::EffectModifyThreatPercent,  // 125
                                        // SPELL_EFFECT_MODIFY_THREAT_PERCENT
    &Spell::EffectStealBeneficialBuff,  // 126
                                        // SPELL_EFFECT_STEAL_BENEFICIAL_BUFF
                                        // spell steal effect?
    &Spell::EffectProspecting,  // 127 SPELL_EFFECT_PROSPECTING Prospecting
                                // spell
    ...
    &Spell::EffectLeapBack,  // 138 SPELL_EFFECT_LEAP_BACK                Leap
                             // back
    &Spell::EffectQuestClear,  // 139 SPELL_EFFECT_CLEAR_QUEST Reset quest
                               // status (miscValue - quest ID)
    ...
    &Spell::EffectPullTowardsDest,  // 145 SPELL_EFFECT_PULL_TOWARDS_DEST Black
                                    // Hole Effect
    ...
    &Spell::
        EffectDamageFromMaxHealthPCT,  // 165
...
void Spell::EffectResurrectNew(SpellEffIndex effIndex) {
  if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT_TARGET) return;

  if (!corpseTarget && !unitTarget) return;
  ...
  if (!player || player->IsAlive() || !player->IsInWorld()) return;
  ...
  if (m_caster->IsPlayer() &&
      m_caster->ToPlayer()->GetGuildId() == player->GetGuildId() &&
      !m_caster->IsInCombat())
    AddPct(health, m_caster->GetTotalAuraModifier(
...
void Spell::EffectInstaKill(SpellEffIndex /*effIndex*/) {
  ...
  if (unitTarget->GetTypeId() == TYPEID_PLAYER)
    if (unitTarget->ToPlayer()->GetCommandStatus(CHEAT_GOD)) return;
  ...
  m_caster->DealDamage(unitTarget, unitTarget->GetHealth(), nullptr, NODAMAGE,
...
void Spell::EffectEnvironmentalDMG(SpellEffIndex /*effIndex*/) {
  ...
  if (unitTarget->GetTypeId() == TYPEID_PLAYER)
    ...
    DamageInfo damageInfo(m_caster, unitTarget, damage, m_spellInfo,
                          m_spellInfo->GetSchoolMask(), SPELL_DIRECT_DAMAGE,
                          BASE_ATTACK);
    ...
    m_caster->SendSpellNonMeleeDamageLog(unitTarget, m_spellInfo->Id, damage,
                                         m_spellInfo->GetSchoolMask(), absorb,
                                         resist, false, 0, false);
  }
}

void Spell::EffectSchoolDMG(SpellEffIndex effIndex) {
  if (effectHandleMode != SPELL_EFFECT_HANDLE_LAUNCH_TARGET) return;
  ...
  if (unitTarget && unitTarget->IsAlive()) {
    ...
    switch (m_spellInfo->SpellFamilyName) {
      case SPELLFAMILY_GENERIC: {
        ...
        switch (m_spellInfo->Id)  // better way to check unknown
        {
          // Consumption
          case 28865:
            damage = (((InstanceMap*)m_caster->GetMap())->GetDifficulty() ==
                              REGULAR_DIFFICULTY
                          ? 2750
                          : 4250);
            break;
          ...
          case 88084:  // Arcane Blast (Mirror Image)
            // Mage Mirror Images deal 50% of the coefficient's spell power
            // bonus as bonus damage
            if (Unit* owner = m_caster->ToTempSummon()->GetSummoner())
              damage = CalculatePct(owner->SpellDamageBonusDone(
                                        unitTarget, m_spellInfo, (uint32)damage,
                                        SPELL_DIRECT_DAMAGE, effIndex),
                                    50);
            break;
        }
        break;
      ...
      case SPELLFAMILY_DEATHKNIGHT: {
        ...
        if (m_spellInfo->SpellFamilyFlags[0] & 0x00040000) {
          if (unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE,
                                        SPELLFAMILY_DEATHKNIGHT, 0, 0,
                                        0x00000002, m_caster->GetGUID())) {
            damage += m_damage / 2;
            damage +=
                int32(m_caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.035f);
          }
        }

        // Merciless Combat (Icy Touch and Howling Blast)
        if (m_spellInfo->SpellFamilyFlags[0] & 0x2 ||
            m_spellInfo->SpellFamilyFlags[1] & 0x2)
          if (unitTarget->GetHealthPct() < 35.0f)
            if (AuraEffect const* mercilessCombat =
                    m_caster->GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 2656,
                                                 0))
    ...
    if (m_originalCaster && damage > 0 && apply_direct_bonus) {
      damage = m_originalCaster->SpellDamageBonusDone(
          unitTarget, m_spellInfo, (uint32)damage, SPELL_DIRECT_DAMAGE,
          effIndex);
      damage = unitTarget->SpellDamageBonusTaken(
          m_originalCaster, m_spellInfo, (uint32)damage, SPELL_DIRECT_DAMAGE);
    }
...
void Spell::EffectDummy(SpellEffIndex effIndex) {
  ...
  if (!unitTarget && !gameObjTarget && !itemTarget && !corpseTarget) return;
  ...
  LOG_DEBUG("spells", "Spell ScriptStart spellid %u in EffectDummy(%u)",
...
void Spell::EffectTriggerSpell(SpellEffIndex effIndex) {
  ...
  if (m_spellInfo->Effects[effIndex].Effect == SPELL_EFFECT_TRIGGER_SPELL &&
    ...
    switch (triggered_spell_id) {
      ...
      case 18461: {
        ...
        // If this spell is given to an NPC, it must handle the rest using its
        // own AI
  ...
  if (effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH_TARGET) {
    if (!spellInfo->NeedsToBeTriggeredByCaster(m_spellInfo)) return;
  ...
  if (m_spellInfo->Effects[effIndex].Effect ==
    ...
    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
      args.SpellValueOverrides.AddMod(SpellValueMod(SPELLVALUE_BASE_POINT0 + i),
                                      damage);
...
void Spell::EffectTriggerMissileSpell(SpellEffIndex effIndex) {
  ...
  SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(triggered_spell_id);
  if (!spellInfo) {
    LOG_ERROR("spells.effecttrigermissilespell",
              "Spell::EffectTriggerMissileSpell spell %u tried to trigger "
              "unknown spell %u.",
              m_spellInfo->Id, triggered_spell_id);
    return;
  }
...
void Spell::EffectTriggerRitualOfSummoning(SpellEffIndex effIndex) {
  if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT) return;
  ...
  if (!m_spellInfo->Effects[effIndex].IsTargetingArea())
    ...
    for (Group::MemberSlot const& memberSlot : group->GetMemberSlots()) {
      if (memberSlot.guid == player->GetGUID()) continue;

      Player* member = ObjectAccessor::FindPlayer(memberSlot.guid);
      if (!member || member->GetSession()->PlayerLogout()) {
        player->SendDirectMessage(
            WorldPackets::Party::SummonRaidMemberValidateFailed(
                memberSlot.guid, SummonRaidMemberValidateReasonCode::Offline)
                .Write());
        continue;
      }
      ...
      if (member->isDead()) {
        player->SendDirectMessage(
            WorldPackets::Party::SummonRaidMemberValidateFailed(
                memberSlot.guid,
                SummonRaidMemberValidateReasonCode::DeadOrGhost)
                .Write());
        continue;
      }

      Map::EnterState enterState =
          sMapMgr->PlayerCannotEnter(player->GetMapId(), member, false);
      if (enterState != Map::CAN_ENTER) {
        if (enterState == Map::CANNOT_ENTER_INSTANCE_BIND_MISMATCH) {
          // @todo: this will also cause denied summons for dynamic lockouts.
          // Gotta wait for grumpy Shauren to wrap up his lock rework.
          player->SendDirectMessage(
              WorldPackets::Party::SummonRaidMemberValidateFailed(
                  memberSlot.guid,
                  SummonRaidMemberValidateReasonCode::RaidLocked)
                  .Write());
          continue;
        } else {
          player->SendDirectMessage(
              WorldPackets::Party::SummonRaidMemberValidateFailed(
                  memberSlot.guid,
                  SummonRaidMemberValidateReasonCode::MapConditionFailed)
                  .Write());
          continue;
        }
      }
...
void Spell::EffectJump(SpellEffIndex effIndex) {
  ...
  CalculateJumpSpeeds(effIndex, m_caster->GetExactDist2d(unitTarget), speedXY,
                      speedZ);
...
void Spell::EffectJumpDest(SpellEffIndex effIndex) {
  if (effectHandleMode != SPELL_EFFECT_HANDLE_LAUNCH) return;
  ...
  float speedXY, speedZ;
  CalculateJumpSpeeds(effIndex, m_caster->GetExactDist2d(destTarget), speedXY,
                      speedZ);
  m_caster->GetMotionMaster()->MoveJump(
      *destTarget, speedXY, speedZ, EVENT_JUMP,
      !m_targets.GetObjectTargetGUID().IsEmpty());
}

void Spell::CalculateJumpSpeeds(uint8 i, float dist, float& speedXY,
                                float& speedZ) {
  float runSpeed = m_caster->IsControlledByPlayer()
                       ? playerBaseMoveSpeed[MOVE_RUN]
  ...
  float minHeight = m_spellInfo->Effects[i].MiscValue
                        ? m_spellInfo->Effects[i].MiscValue / 10.0f
                        : 0.5f;  // Lower bound is blizzlike
  float maxHeight = m_spellInfo->Effects[i].MiscValueB
                        ? m_spellInfo->Effects[i].MiscValueB / 10.0f
                        : 1000.0f;  // Upper bound is unknown
  float height;
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/) {
  ...
  if (!m_targets.HasDst()) {
    LOG_ERROR("spells",
              "Spell::EffectTeleportUnits - does not have a destination for "
              "spellId %u.",
              m_spellInfo->Id);
    return;
  }
  ...
  if (targetDest.GetMapId() == unitTarget->GetMapId())
    ...
    unitTarget->ToPlayer()->TeleportTo(
        targetDest, unitTarget == m_caster ? TELE_TO_SPELL : 0);
  else {
    LOG_ERROR("spells",
              "Spell::EffectTeleportUnits - spellId %u attempted to teleport "
...
void Spell::EffectUnlearnSpecialization(SpellEffIndex effIndex) {
  ...
  LOG_DEBUG("spells",
            ...
            player->GetGUID().GetCounter(), spellToUnlearn,
            m_caster->GetGUID().GetCounter());
}
...
void Spell::EffectPowerDrain(SpellEffIndex effIndex) {
  ...
  if (!unitTarget || !unitTarget->IsAlive() ||
      unitTarget->GetPowerType() != powerType || damage < 0)
    return;
...
void Spell::EffectSendEvent(SpellEffIndex effIndex) {
  ...
  {
    // let's prevent executing effect handler twice in case when spell effect is
    // capable of targeting an object this check was requested by scripters, but
    // it has some downsides: now it's impossible to script (using
    // sEventScripts) a cast which misses all targets or to have an ability to
    // script the moment spell hits dest (in a case when there are object
    // targets present)
    if (m_spellInfo->Effects[effIndex].GetProvidedTargetMask() &
        (TARGET_FLAG_UNIT_MASK | TARGET_FLAG_GAMEOBJECT_MASK))
      return;
    // some spells have no target entries in dbc and they use focus target
    if (focusObject) target = focusObject;
    /// @todo there should be a possibility to pass dest target to event script
  }

  LOG_DEBUG("spells", "Spell ScriptStart %u for spellid %u in EffectSendEvent ",
            m_spellInfo->Effects[effIndex].MiscValue, m_spellInfo->Id);

  GameEvents::Trigger(m_spellInfo->Effects[effIndex].MiscValue, m_caster,
                      target);
}
...
void Spell::EffectPowerBurn(SpellEffIndex effIndex) {
  ...
  // NO - Not a typo - EffectPowerBurn uses effect value multiplier - not effect
  // damage multiplier
...
void Spell::EffectHeal(SpellEffIndex effIndex) {
  ...
  if (unitTarget && unitTarget->IsAlive() && damage >= 0) {
    ...
    // Runic Healing Injector (heal increased by 25% for engineers - 3.2.0 patch
    // change)
    else if (m_spellInfo->Id == 67489) {
      if (Player* player = m_caster->ToPlayer())
        if (player->HasSkill(SKILL_ENGINEERING)) AddPct(addhealth, 25);
    }
    // Swiftmend - consumes Regrowth or Rejuvenation
      ...
      for (Unit::AuraEffectList::const_iterator i = RejorRegr.begin();
        ...
        if ((*i)->GetSpellInfo()->SpellFamilyName == SPELLFAMILY_DRUID &&
          ...
          if (!targetAura || (*i)->GetBase()->GetDuration() <
      ...
      if (!targetAura) {
        LOG_ERROR("spells",
                  "Target (%s) has the aurastate AURA_STATE_SWIFTMEND, but no "
                  "matching aura.",
                  unitTarget->GetGUID().ToString().c_str());
        return;
      }
      ...
      addhealth = caster->SpellHealingBonusDone(unitTarget, m_spellInfo,
                                                addhealth, HEAL, effIndex);
    }
    // Death Pact - return pct of max health to caster
      ...
      addhealth = caster->SpellHealingBonusDone(
          unitTarget, m_spellInfo, int32(caster->CountPctFromMaxHealth(damage)),
          HEAL, effIndex);
    else
      addhealth = caster->SpellHealingBonusDone(unitTarget, m_spellInfo,
                                                addhealth, HEAL, effIndex);

    addhealth = unitTarget->SpellHealingBonusTaken(caster, m_spellInfo,
                                                   addhealth, HEAL);
...
void Spell::EffectHealPct(SpellEffIndex effIndex) {
  ...
  uint32 heal = m_originalCaster->SpellHealingBonusDone(
      unitTarget, m_spellInfo, unitTarget->CountPctFromMaxHealth(damage), HEAL,
      effIndex);
  heal = unitTarget->SpellHealingBonusTaken(m_originalCaster, m_spellInfo, heal,
                                            HEAL);
...
void Spell::EffectHealMechanical(SpellEffIndex effIndex) {
  ...
  m_healing += unitTarget->SpellHealingBonusTaken(m_originalCaster, m_spellInfo,
                                                  heal, HEAL);
}
...
void Spell::EffectHealthLeech(SpellEffIndex effIndex) {
  ...
  float healMultiplier = m_spellInfo->Effects[effIndex].CalcValueMultiplier(
      m_originalCaster, this);

  m_damage += damage;
  // get max possible damage, don't count overkill for heal
  uint32 healthGain =
      uint32(-unitTarget->GetHealthGain(-damage) * healMultiplier);

  if (m_caster->IsAlive()) {
    healthGain = m_caster->SpellHealingBonusDone(m_caster, m_spellInfo,
                                                 healthGain, HEAL, effIndex);
    healthGain = m_caster->SpellHealingBonusTaken(m_caster, m_spellInfo,
                                                  healthGain, HEAL);

    HealInfo healInfo(m_caster, m_caster, healthGain, m_spellInfo,
                      m_spellSchoolMask);
...
void Spell::DoCreateItem(uint32 /*i*/, uint32 itemtype) {
  ...
  if (CanCreatePerfectItem(player, m_spellInfo->Id, perfectCreateChance,
                           perfectItemType))
  ...
  InventoryResult msg = player->CanStoreNewItem(
      NULL_BAG, NULL_SLOT, dest, newitemid, num_to_add, &no_space);
  if (msg != EQUIP_ERR_OK) {
    // convert to possible store amount
    if (msg == EQUIP_ERR_INV_FULL || msg == EQUIP_ERR_ITEM_MAX_COUNT)
      num_to_add -= no_space;
    else {
      // if not created by another reason from full inventory or unique items
      // amount limitation
  ...
  if (num_to_add) {
    ...
    if (projectId && !player->ArchProjectCompleteable(projectId)) {
      player->SendEquipError(EQUIP_ERR_SPELL_FAILED_REAGENTS_GENERIC, nullptr,
                             nullptr);
      return;
    }
    ...
    Item* pItem = player->StoreNewItem(dest, newitemid, true,
                                       GenerateItemRandomPropertyId(newitemid));
    ...
    if (projectId) player->CompleteArchProject(projectId);

    // set the "Crafted by ..." property of the item
    if (pItem->GetTemplate()->GetClass() != ITEM_CLASS_CONSUMABLE &&
        pItem->GetTemplate()->GetClass() != ITEM_CLASS_QUEST &&
        newitemid != 6265 && newitemid != 6948)
    ...
    if (Guild* guild = player->GetGuild()) {
      if (pProto->GetQuality() > ITEM_QUALITY_EPIC ||
        ...
        guild->AddGuildNews(GUILD_NEWS_ITEM_CRAFTED, player->GetGUID(), 0,
                            pProto->GetId());

      guild->UpdateAchievementCriteria(
          ACHIEVEMENT_CRITERIA_TYPE_CRAFT_ITEMS_GUILD, pItem->GetEntry(),
          num_to_add, 0, nullptr, player);
    }
  ...
  /*
      ...
      {
          if (Battleground* bg =
     sBattlegroundMgr->GetBattlegroundTemplate(BattlegroundTypeId(bgType)))
...
void Spell::EffectCreateItem2(SpellEffIndex effIndex) {
  ...
  if (m_spellInfo->IsLootCrafting()) {
    if (item_id) {
      if (!player->HasItemCount(item_id)) return;
      ...
      player->AutoStoreLoot(m_spellInfo->Id,
...
void Spell::EffectPersistentAA(SpellEffIndex effIndex) {
  ...
  if (!m_spellAura) {
    ...
    if (!dynObj->CreateDynamicObject(
            ...
            caster, m_spellInfo, *destTarget, radius,
    ...
    if (Aura* aura =
            Aura::TryCreate(m_spellInfo, MAX_EFFECT_MASK, dynObj, caster,
...
void Spell::EffectEnergize(SpellEffIndex effIndex) {
  ...
  switch (m_spellInfo->Id) {
    ...
    case 68082:  // Glyph of Seal of Command
      ...
      break;
    case 67490:  // Runic Mana Injector (mana gain increased by 25% for
                 // engineers - 3.2.0 patch change)
    {
      if (Player* player = m_caster->ToPlayer())
        if (player->HasSkill(SKILL_ENGINEERING)) AddPct(damage, 25);
      break;
    }
    default:
      break;
  ...
  if (gameObjTarget) {
    ...
    if (!gameObjTarget->isSpawned() && !player->IsGameMaster()) {
      LOG_INFO("entities.player.cheat",
               "Possible hacking attempt: Player %s [guid: %u] tried to loot a "
               "gameobject [entry: %u id: %u] which is on respawn timer "
               "without being in GM mode!",
               player->GetName().c_str(), player->GetGUID().GetCounter(),
               gameObjTarget->GetEntry(),
               gameObjTarget->GetGUID().GetCounter());
      return;
    }
    // special case, already has GossipHello inside so return and avoid calling
    // twice
    ...
    if (gameObjTarget->AI()->GossipHello(player)) return;
    ...
    switch (gameObjTarget->GetGoType()) {
      ...
      case GAMEOBJECT_TYPE_QUESTGIVER:
        player->PrepareGossipMenu(
            gameObjTarget, gameObjTarget->GetGOInfo()->questgiver.gossipID,
            true);
      ...
      case GAMEOBJECT_TYPE_CHEST:
        ...
        if (gameObjTarget->GetGOInfo()->chest.eventId) {
          LOG_DEBUG("spells", "Chest ScriptStart id %u for GO %u",
                    gameObjTarget->GetGOInfo()->chest.eventId,
                    gameObjTarget->GetSpawnId());
          GameEvents::Trigger(gameObjTarget->GetGOInfo()->chest.eventId, player,
                              gameObjTarget);
        }
...
void Spell::EffectOpenLock(SpellEffIndex effIndex) {
  ...
  if (gameObjTarget) {
    ...
    if (goInfo->CannotBeUsedUnderImmunity() &&
        m_caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE))
      return;
    ...
    /// @todo Add script for spell 41920 - Filling, becouse server it freze when
    /// use this spell
    // handle outdoor pvp object opening, return true if go was registered for
    // handling these objects must have been spawned by outdoorpvp!
...
void Spell::EffectSummonChangeItem(SpellEffIndex effIndex) {
  ...
  for (uint8 j = PERM_ENCHANTMENT_SLOT; j <= TEMP_ENCHANTMENT_SLOT; ++j)
    if (m_CastItem->GetEnchantmentId(EnchantmentSlot(j)))
      pNewItem->SetEnchantment(
          EnchantmentSlot(j), m_CastItem->GetEnchantmentId(EnchantmentSlot(j)),
  ...
  if (m_CastItem->GetUInt32Value(ITEM_FIELD_DURABILITY) <
    ...
    double lossPercent =
        1 - m_CastItem->GetUInt32Value(ITEM_FIELD_DURABILITY) /
  ...
  if (player->IsInventoryPos(pos)) {
    ItemPosCountVec dest;
    InventoryResult msg = player->CanStoreItem(
        m_CastItem->GetBagSlot(), m_CastItem->GetSlot(), dest, pNewItem, true);
    if (msg == EQUIP_ERR_OK) {
    ...
    ItemPosCountVec dest;
    if (player->CanBankItem(m_CastItem->GetBagSlot(), m_CastItem->GetSlot(),
                            dest, pNewItem, true) == EQUIP_ERR_OK) {
      player->DestroyItem(m_CastItem->GetBagSlot(), m_CastItem->GetSlot(),
                          true);
...
void Spell::EffectProficiency(SpellEffIndex /*effIndex*/) {
  ...
  if (m_spellInfo->EquippedItemClass == ITEM_CLASS_WEAPON &&
      !(player->GetWeaponProficiency() & subClassMask)) {
    player->AddWeaponProficiency(subClassMask);
    if (!player->IsLoading())
      player->SendProficiency(ITEM_CLASS_WEAPON,
                              player->GetWeaponProficiency());
  }
  if (m_spellInfo->EquippedItemClass == ITEM_CLASS_ARMOR &&
      !(player->GetArmorProficiency() & subClassMask)) {
...
void Spell::EffectSummonType(SpellEffIndex effIndex) {
  ...
  if (!properties) {
    LOG_ERROR("spells", "EffectSummonType: Unhandled summon type %u.",
              m_spellInfo->Effects[effIndex].MiscValueB);
    return;
  }
  ...
  ObjectGuid privateObjectOwner = [&]() {
    ...
    if (properties->Flags & SUMMON_PROP_FLAG_PERSONAL_GROUP_SPAWN)
      if (m_originalCaster->IsPlayer() &&
          m_originalCaster->ToPlayer()->GetGroup())
  ...
  if (uint8 const* parameter =
          sObjectMgr->GetSummonPropertiesParameter(properties->ID)) {
    switch (SummonPropertiesParamType(*parameter)) {
      ...
      case SummonPropertiesParamType::RideSpell:
        ...
        else  // There is one spell with RecID 161 (52200) that uses bp as ride
              // spell while most spells do not so we can assume that this
              // parameter falls back to the hardcoded spell
      ...
      case SummonPropertiesParamType::CreatureLevel:
        ...
        break;
      case SummonPropertiesParamType::MaxSummons:  // @todo: handle and research
  ...
  for (uint32 i = 0; i < summonCount; ++i) {
    ...
    if (summonCount > 1) {
      // Multiple summons are summoned at random points within the destination
      // radius
    ...
    if (TempSummon* summon =
            m_caster->GetMap()->SummonCreature(entry, *destTarget, extraArgs)) {
      ExecuteLogEffectSummonObject(effIndex, summon);

      if (summonCount == 1 && summon->IsVehicle()) {
        if (useHardcodedRideSpell)
          m_originalCaster->CastSpell(
              summon, VEHICLE_SPELL_RIDE_HARDCODED,
              CastSpellExtraArgs(true).AddSpellBP0(extraArgs.SeatNumber));
...
void Spell::EffectLearnSpell(SpellEffIndex effIndex) {
  ...
  if (unitTarget->GetTypeId() != TYPEID_PLAYER) {
    if (unitTarget->ToPet()) EffectLearnPetSpell(effIndex);
    return;
  }
  ...
  LOG_DEBUG("spells", "Spell: Player %u has learned spell %u from NpcGUID: %u",
            player->GetGUID().GetCounter(), spellToLearn,
            m_caster->GetGUID().GetCounter());
}
...
void Spell::EffectDispel(SpellEffIndex effIndex) {
  ...
  unitTarget->GetDispellableAuraList(m_caster, dispelMask, dispelList,
                                     targetMissInfo == SPELL_MISS_REFLECT);
  if (dispelList.empty()) return;
  ...
  if (successList.empty()) return;

  WorldPacket dataSuccess(SMSG_SPELLDISPELLOG,
                          8 + 8 + 4 + 1 + 4 + successList.size() * 5);
  // Send packet header
  ...
  for (DispelChargesList::iterator itr = successList.begin();
       itr != successList.end(); ++itr) {
    // Send dispelled spell info
    dataSuccess << uint32(itr->GetAura()->GetId());  // Spell Id
    dataSuccess << uint8(0);  // 0 - dispelled !=0 cleansed
    unitTarget->RemoveAurasDueToSpellByDispel(
        itr->GetAura()->GetId(), m_spellInfo->Id,
        itr->GetAura()->GetCasterGUID(), m_caster, itr->GetDispelCharges());
  }
  ...
  if (m_spellInfo->SpellFamilyName == SPELLFAMILY_WARLOCK &&
    ...
    CastSpellExtraArgs args(TRIGGERED_FULL_MASK);
    args.SpellValueOverrides.AddMod(SPELLVALUE_BASE_POINT0,
                                    m_spellInfo->Effects[EFFECT_1].CalcValue());
    m_caster->CastSpell(m_caster, 19658, args);
    // Glyph of Felhunter
    if (Unit* owner = m_caster->GetOwner())
      if (owner->GetAura(56249)) owner->CastSpell(owner, 19658, args);
  }
}
...
void Spell::EffectDistract(SpellEffIndex /*effIndex*/) {
  ...
  if (!unitTarget || unitTarget->IsEngaged()) return;
...
void Spell::EffectPickPocket(SpellEffIndex /*effIndex*/) {
  ...
  if (unitTarget->IsAlive() && (unitTarget->GetCreatureTypeMask() &
...
void Spell::EffectAddFarsight(SpellEffIndex effIndex) {
  ...
  if (!dynObj->CreateDynamicObject(
          ...
          m_caster, m_spellInfo, *destTarget, radius,
...
void Spell::EffectLearnSkill(SpellEffIndex effIndex) {
  ...
  SkillRaceClassInfoEntry const* rcEntry = sDBCManager.GetSkillRaceClassInfo(
      skillid, unitTarget->getRace(), unitTarget->getClass());
  if (!rcEntry) return;
  ...
  unitTarget->ToPlayer()->SetSkill(
      skillid, m_spellInfo->Effects[effIndex].CalcValue(),
      std::max<uint16>(skillval, 1), tier->Value[damage - 1]);
}
...
void Spell::EffectPlayMovie(SpellEffIndex effIndex) {
  ...
  if (!sMovieStore.LookupEntry(movieId)) return;
...
void Spell::EffectEnchantItemPerm(SpellEffIndex effIndex) {
  ...
  if (itemTarget->IsVellum()) {
    ...
    if (!(m_CastItem &&
          m_CastItem->GetTemplate()->GetFlags() & ITEM_FLAG_NO_REAGENT_COST))
      player->UpdateCraftSkill(m_spellInfo->Id);
    ...
    if (item_owner != player && player->IsGameMaster()) {
      sLog->outCommand(player->GetSession()->GetAccountId(),
                       "GM %s (Account: %u) enchanting(perm): %s (Entry: %d) "
                       "for player: %s (Account: %u)",
                       player->GetName().c_str(),
                       player->GetSession()->GetAccountId(),
                       itemTarget->GetTemplate()->GetDefaultLocaleName(),
                       itemTarget->GetEntry(), item_owner->GetName().c_str(),
...
void Spell::EffectEnchantItemPrismatic(SpellEffIndex effIndex) {
  ...
  // support only enchantings with add socket in this slot
  {
    ...
    if (!add_socket) {
      LOG_ERROR(
          "spells",
          "Spell::EffectEnchantItemPrismatic: attempt to apply the enchant "
          "spell %u with SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC (%u), but without "
          "ITEM_ENCHANTMENT_TYPE_PRISMATIC_SOCKET (%u), not supported yet.",
  ...
  if (item_owner != player && player->IsGameMaster()) {
    sLog->outCommand(player->GetSession()->GetAccountId(),
                     "GM %s (Account: %u) enchanting(perm): %s (Entry: %d) for "
                     "player: %s (Account: %u)",
                     player->GetName().c_str(),
                     player->GetSession()->GetAccountId(),
                     itemTarget->GetTemplate()->GetDefaultLocaleName(),
  ...
  itemTarget->SetEnchantment(PRISMATIC_ENCHANTMENT_SLOT, enchantId, 0, 0,
                             m_caster->GetGUID());
...
void Spell::EffectEnchantItemTmp(SpellEffIndex effIndex) {
  ...
  if (!enchant_id) {
    LOG_ERROR("spells",
              "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) has "
              "enchanting id 0.",
              m_spellInfo->Id, effIndex);
    return;
  }
  ...
  if (!pEnchant) {
    LOG_ERROR("spells",
              "Spell %u Effect %u (SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY) has a "
              "non-existing enchanting id %u ",
  ...
  if (item_owner != player && player->IsGameMaster()) {
    sLog->outCommand(player->GetSession()->GetAccountId(),
                     "GM %s (Account: %u) enchanting(temp): %s (Entry: %d) for "
                     "player: %s (Account: %u)",
                     player->GetName().c_str(),
                     player->GetSession()->GetAccountId(),
                     itemTarget->GetTemplate()->GetDefaultLocaleName(),
  ...
  itemTarget->SetEnchantment(TEMP_ENCHANTMENT_SLOT, enchant_id, duration * 1000,
                             0, m_caster->GetGUID());
...
void Spell::EffectTameCreature(SpellEffIndex /*effIndex*/) {
  ...
  if (m_caster->GetTypeId() == TYPEID_PLAYER) {
    ...
    if (pet->GetSlot() <= PET_SLOT_LAST_ACTIVE_SLOT) {
      m_caster->ToPlayer()->GetSession()->SendPetAdded(
          pet->GetSlot(), pet->GetCharmInfo()->GetPetNumber(),
          creatureTarget->GetEntry(), creatureTarget->getLevel(),
          pet->GetName());
...
void Spell::EffectSummonPet(SpellEffIndex effIndex) {
  ...
  // SUMMON_PET SummonPet's entries are at MiscValue, HunterPetSlot at
  // BasePoints
  uint32 petentry =
      (m_spellInfo->Effects[effIndex].MiscValue == 0 &&
       m_spellInfo->Effects[effIndex].BasePoints <= PET_SLOT_LAST_ACTIVE_SLOT)
  ...
  if (!owner) {
    if (SummonPropertiesEntry const* properties =
      ...
      if (TempSummon* summon = m_caster->GetMap()->SummonCreature(
              petentry, *destTarget, extraArgs))
        ExecuteLogEffectSummonObject(effIndex, summon);
    }
    return;
  }
  ...
  if (OldSummon) {
    ...
    if (owner->GetTypeId() == TYPEID_PLAYER)
      owner->ToPlayer()->RemovePet(
          OldSummon,
          (OldSummon->getPetType() == HUNTER_PET ? PET_SAVE_AS_DELETED
                                                 : PET_SAVE_DISMISS),
          false);
    else
      return;
  ...
  Pet* pet =
      owner->SummonPet(petentry, x, y, z, owner->GetOrientation(), petType, 0);
  if (!pet) return;
...
void Spell::EffectTaunt(SpellEffIndex /*effIndex*/) {
  ...
  // this effect use before aura Taunt apply for prevent taunt already attacking
  // target for spell as marked "non effective at already attacking target"
  if (!unitTarget || unitTarget->IsTotem()) {
    SendCastResult(SPELL_FAILED_DONT_REPORT);
    return;
  }

  // Hand of Reckoning can hit some entities that can't have a threat list
  // (including players' pets)
...
void Spell::EffectWeaponDmg(SpellEffIndex effIndex) {
  ...
  /*
      Multiple weapon damage effect case. SPELL_EFFECT_WEAPON_PERCENT_DAMAGE is
     being prioritized for calculating weapon damage values. The other effects
     only add their fixed bonus to the damage sum.
  */
  if (m_spellInfo->HasEffect(SPELL_EFFECT_WEAPON_PERCENT_DAMAGE) &&
      effectType != SPELL_EFFECT_WEAPON_PERCENT_DAMAGE)
        ...
        if (m_spellInfo->SpellFamilyFlags[0] & 0x2000000) {
          ...
          if (m_caster->GetTypeId() == TYPEID_PLAYER)
            if (Item* item = m_caster->ToPlayer()->GetWeaponForAttack(
                    m_attackType, true))
              if (item->GetTemplate()->GetSubClass() ==
      ...
      case SPELLFAMILY_DEATHKNIGHT: {
        ...
        if (m_spellInfo->SpellFamilyFlags[0] & 0x10) {
          ...
          if (AuraEffect const* aurEff =
            ...
            if (uint32 runic = std::min<uint32>(
                    uint32(m_caster->GetPower(POWER_RUNIC_POWER) / 2.5f),
                    aurEff->GetSpellInfo()->Effects[EFFECT_1].CalcValue(
                        m_caster)))
              AddPct(totalDamagePercentMod, runic);
        }

        // Obliterate / Blood Strike / Blood-Caked Strike (12.5% more damage per
        // disease) / Heart Strike (15% more damage per disease)
        if (m_spellInfo->SpellFamilyFlags[1] & 0x20000 ||
            m_spellInfo->SpellFamilyFlags[0] & 0x400000 ||
            m_spellInfo->SpellFamilyFlags[0] & 0x1000000 ||
            m_spellInfo->Id == 50463) {
          float bonusPct = m_spellInfo->Effects[EFFECT_2].CalcValue(m_caster);
          uint8 diseaseCount =
              unitTarget->GetDiseasesByCaster(m_caster->GetGUID(), false);

          if (m_spellInfo->SpellFamilyFlags[1] &
              0x20000)  // Obliterate - 50% of Basepoints as bonus
            bonusPct *= 0.5f;
          else if (m_spellInfo->SpellFamilyFlags[0] &
                   0x400000)  // Blood Strike - 10% of Basepoints as bonus
            bonusPct *= 0.1f;

          float bonusAmount = bonusPct * diseaseCount;
          // Death Knight T8 Melee 4P Bonus
          if (AuraEffect const* aurEff =
                  m_caster->GetAuraEffect(64736, EFFECT_0))
        ...
        if (m_spellInfo->SpellIconID == 1736)
          AddPct(totalDamagePercentMod,
                 unitTarget->GetDiseasesByCaster(m_caster->GetGUID()) * 50.0f);

        // Merciless Combat (Obliterate and Frost Strike)
        if (m_spellInfo->SpellFamilyFlags[1] & 0x20000 ||
            m_spellInfo->SpellFamilyFlags[1] & 0x4)
          if (unitTarget->GetHealthPct() < 35.0f)
            if (AuraEffect const* mercilessCombat =
                    m_caster->GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 2656,
                                                 0))
      ...
      case SPELLFAMILY_WARLOCK: {
        if (useWeaponDamage) {
          ...
          if (m_spellInfo->Id == 89753) {
            if (m_caster->IsPet())
              if (Unit* owner = m_caster->GetOwner())
                m_damage += ((owner->SpellBaseDamageBonusDone(
  ...
  // if (addPctMods) { percent mods are added in Unit::CalculateDamage } else {
  // percent mods are added in Unit::MeleeDamageBonusDone } this distinction is
  // neccessary to properly inform the client about his autoattack damage values
  // from Script_UnitDamage
  bool const addPctMods =
      !m_spellInfo->HasAttribute(SPELL_ATTR6_LIMIT_PCT_DAMAGE_MODS) &&
      (m_spellSchoolMask & SPELL_SCHOOL_MASK_NORMAL);
  if (addPctMods) {
    UnitMods unitMod;
    switch (m_attackType) {
      default:
    ...
    if (fixed_bonus) fixed_bonus = int32(fixed_bonus * weapon_total_pct);
  }

  int32 weaponDamage =
      useWeaponDamage
          ? m_caster->CalculateDamage(m_attackType, normalized, addPctMods)
          : 0;
  ...
  weaponDamage = m_caster->MeleeDamageBonusDone(
      unitTarget, weaponDamage, m_attackType, m_spellInfo, !useWeaponDamage);
  m_damage += unitTarget->MeleeDamageBonusTaken(m_caster, weaponDamage,
...
void Spell::EffectThreat(SpellEffIndex /*effIndex*/) {
  ...
  if (!unitTarget || !m_caster->IsAlive()) return;

  if (!unitTarget->CanHaveThreatList()) return;

  unitTarget->GetThreatManager().AddThreat(m_caster, float(damage), m_spellInfo,
                                           true);
}
  ...
  for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_AUTOREPEAT_SPELL;
    ...
    if (Spell* spell = unitTarget->GetCurrentSpell(CurrentSpellTypes(i))) {
      ...
      if ((spell->getState() == SPELL_STATE_CASTING ||
        ...
        int32 duration = m_spellInfo->GetDuration();
        duration = unitTarget->ModSpellDuration(m_spellInfo, unitTarget,
                                                duration, false, 1 << effIndex);
        unitTarget->GetSpellHistory()->LockSpellSchool(
            curSpellInfo->GetSchoolMask(), duration);
...
void Spell::EffectSummonObjectWild(SpellEffIndex effIndex) {
  ...
  if (!pGameObj->Create(map->GenerateLowGuid<HighGuid::GameObject>(),
                        gameobject_id, map, Position(x, y, z, o), rot, 255,
                        GO_STATE_READY)) {
    delete pGameObj;
    return;
  }
  ...
  if (pGameObj->GetGoType() == GAMEOBJECT_TYPE_FLAGDROP)
    if (Player* player = m_caster->ToPlayer())
      if (Battleground* bg = player->GetBattleground())
        bg->SetDroppedFlagGUID(
            pGameObj->GetGUID(),
            player->GetTeam() == ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE);
...
void Spell::EffectSurvey(SpellEffIndex effIndex) {
  ...
  if (Player* player = m_caster->ToPlayer())
    if (player->HasSkill(SKILL_ARCHAEOLOGY)) player->SurveyDigSite();
}
...
void Spell::EffectScriptEffect(SpellEffIndex effIndex) {
      ...
      switch (m_spellInfo->Id) {
        ...
        case 69961: {
          Unit::AuraEffectList const& mPeriodic =
              unitTarget->GetAuraEffectsByType(SPELL_AURA_PERIODIC_DAMAGE);
          for (Unit::AuraEffectList::const_iterator i = mPeriodic.begin();
            ...
            if (spellInfo->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT &&
                spellInfo->SpellFamilyFlags[2] & 0x2 &&
              ...
              countMax += 9000;
              // talent Epidemic
              if (AuraEffect const* epidemic = m_caster->GetAuraEffect(
                      SPELL_AURA_ADD_FLAT_MODIFIER, SPELLFAMILY_DEATHKNIGHT,
                      234, EFFECT_0))
                countMax += epidemic->GetAmount();

              if (countMin < countMax) {
                aurEff->GetBase()->SetDuration(
                    aurEff->GetBase()->GetDuration() + 3000);
        ...
        case 41931: {
          ...
          if (bag) {
            if (m_caster->ToPlayer()->GetItemByPos(bag, slot)->GetCount() == 1)
              ...
              m_caster->ToPlayer()->GetItemByPos(bag, slot)->SetCount(
        ...
        case 45151: {
          ...
          if (unitTarget && unitTarget->GetTypeId() == TYPEID_PLAYER &&
              ...
              !unitTarget->HasAura(46394) && unitTarget != m_caster)
        ...
        case 53110:  // Devour Humanoid
          if (unitTarget) unitTarget->CastSpell(m_caster, damage, true);
          return;
        ...
        case 58983:  // Big Blizzard Bear
        {
          ...
          if (uint16 skillval =
                  unitTarget->ToPlayer()->GetSkillValue(SKILL_RIDING)) {
            if (skillval >= 150)
          ...
          if (unitTarget) {
            if (Unit* seat = m_caster->GetVehicleBase()) {
              if (Unit* parent = seat->GetVehicleBase()) {
                /// @todo a hack, range = 11, should after some time cast,
                /// otherwise too far
                m_caster->CastSpell(parent, 62496, true);
                unitTarget->CastSpell(
                    parent, m_spellInfo->Effects[EFFECT_0].CalcValue());
              }
            }
          }
          return;
        ...
        case 60123:  // Lightwell
        {
          ...
          switch (m_caster->GetEntry()) {
            ...
            case 31883:
              ...
              break;
            default:
              LOG_ERROR("spells", "Unknown Lightwell spell caster %u.",
                        m_caster->GetEntry());
              return;
          }
          ...
          if (Aura* chargesAura = m_caster->GetAura(59907)) {
            m_caster->CastSpell(unitTarget, spell_heal,
  ...
  // normal DB scripted effect
  LOG_DEBUG("spells", "Spell ScriptStart spellid %u in EffectScriptEffect(%u)",
            m_spellInfo->Id, effIndex);
  m_caster->GetMap()->ScriptsStart(sSpellScripts,
                                   uint32(m_spellInfo->Id | (effIndex << 24)),
                                   m_caster, unitTarget);
...
void Spell::EffectSanctuary(SpellEffIndex /*effIndex*/) {
  ...
  if (unitTarget->GetTypeId() == TYPEID_PLAYER &&
    ...
    // in dungeons (or for nonplayers), reset this unit on all enemies' threat
    // lists
    for (auto const& pair :
         unitTarget->GetThreatManager().GetThreatenedByMeList())
...
void Spell::EffectAddComboPoints(SpellEffIndex /*effIndex*/) {
  ...
  if (!m_caster->IsMovedByClient()) return;

  if (damage <= 0) return;

  m_caster->GetGameClientMovingMe()->GetBasePlayer()->AddComboPoints(
      unitTarget, damage, this);
}
...
void Spell::EffectDuel(SpellEffIndex effIndex) {
  ...
  if (!pGameObj->Create(map->GenerateLowGuid<HighGuid::GameObject>(),
                        gameobject_id, map, pos, QuaternionData(), 0,
                        GO_STATE_READY)) {
    delete pGameObj;
    return;
  }
...
void Spell::EffectStuck(SpellEffIndex /*effIndex*/) {
  ...
  LOG_DEBUG("spells",
            "Player %s (guid %u) used the auto-unstuck feature at map %u (%f, "
            "%f, %f).",
            player->GetName().c_str(), player->GetGUID().GetCounter(),
            player->GetMapId(), player->GetPositionX(), player->GetPositionY(),
            player->GetPositionZ());

  if (player->IsInFlight()) return;

  // if player is dead without death timer is teleported to graveyard, otherwise
  // not apply the effect
  if (player->isDead()) {
    if (!player->GetDeathTimer()) player->RepopAtGraveyard();

    return;
  }

  // the player dies if hearthstone is in cooldown, else the player is
  // teleported to home
  ...
  player->TeleportTo(player->m_homebindMapId, player->m_homebindX,
                     ...
                     player->GetOrientation(), TELE_TO_SPELL);
  ...
  if (m_targets.GetOrigUnitTargetGUID().IsEmpty()) {
    if (Player* target = ObjectAccessor::FindPlayer(m_caster->GetTarget()))
      target->SendSummonRequestFrom(m_caster);
                 ...
                 ObjectAccessor::FindPlayer(m_targets.GetOrigUnitTargetGUID()))
    target->SendSummonRequestFrom(m_caster);
}
...
void Spell::EffectActivateObject(SpellEffIndex effIndex) {
  ...
  switch (action) {
    ...
    case GameObjectActions::AnimateCustom3:
      gameObjTarget->SendCustomAnim(uint32(action) -
                                    uint32(GameObjectActions::AnimateCustom0));
      break;
    ...
    case GameObjectActions::Lock:
      gameObjTarget->ApplyModFlag(GAMEOBJECT_FLAGS, GO_FLAG_LOCKED,
    ...
    case GameObjectActions::MakeActive:
      gameObjTarget->ApplyModFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE,
    ...
    case GameObjectActions::UseArtKit4: {
      ...
      // Non-sequential here
      if (action == GameObjectActions::UseArtKit4) artKitIndex = 4;
      ...
      if (artKitValue == 0)
        LOG_ERROR("sql.sql",
                  "GameObject %d hit by spell %d needs `artkit%d` in "
                  "`gameobject_template_addon`",
                  gameObjTarget->GetEntry(), m_spellInfo->Id, artKitIndex);
      else
    ...
    case GameObjectActions::GoTo10thFloor:
      static_assert(int32(GO_STATE_TRANSPORT_ACTIVE) ==
                    int32(GameObjectActions::GoTo1stFloor));
      if (gameObjTarget->GetGoType() == GAMEOBJECT_TYPE_TRANSPORT)
        gameObjTarget->SetGoState(GOState(action));
      else
        LOG_ERROR("spell",
                  "Spell %d targeted non-transport gameobject for transport "
                  "only action \"Go to Floor\" %d in effect %d",
                  m_spellInfo->Id, int32(action), int32(effIndex));
      break;
    case GameObjectActions::None:
      LOG_FATAL("spell", "Spell %d has action type NONE in effect %d",
                m_spellInfo->Id, int32(effIndex));
      break;
    default:
      LOG_ERROR("spell", "Spell %d has unhandled action %d in effect %d",
                m_spellInfo->Id, int32(action), int32(effIndex));
      break;
  }
}
...
void Spell::EffectApplyGlyph(SpellEffIndex effIndex) {
  ...
  if (m_glyphIndex >= MAX_GLYPH_SLOT_INDEX) return;
  ...
  if (uint32 newGlyph = m_spellInfo->Effects[effIndex].MiscValue) {
    if (GlyphPropertiesEntry const* newGlyphProperties =
            sGlyphPropertiesStore.LookupEntry(newGlyph)) {
      if (GlyphSlotEntry const* newGlyphSlot =
              sGlyphSlotStore.LookupEntry(player->GetGlyphSlot(m_glyphIndex))) {
      ...
      // remove old glyph
      if (uint32 oldGlyph =
              player->GetGlyph(player->GetActiveSpec(), m_glyphIndex)) {
        if (GlyphPropertiesEntry const* oldGlyphProperties =
  ...
  } else if (uint32 oldGlyph = player->GetGlyph(
                 player->GetActiveSpec(),
                 m_glyphIndex))  // Removing the glyph, get the old one
  {
    if (GlyphPropertiesEntry const* oldGlyphProperties =
            sGlyphPropertiesStore.LookupEntry(oldGlyph)) {
      player->RemoveAurasDueToSpell(oldGlyphProperties->SpellID);
...
void Spell::EffectEnchantHeldItem(SpellEffIndex effIndex) {
  ...
  // this is only item spell effect applied to main-hand weapon of target player
  // (players in area)
  if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER) return;
  ...
  if (m_spellInfo->Effects[effIndex].MiscValue) {
    ...
    if (!duration)
      duration = 10 * IN_MILLISECONDS;  // 10 seconds for enchants which don't
                                        // have listed duration
...
void Spell::EffectDisEnchant(SpellEffIndex /*effIndex*/) {
  ...
  if (!itemTarget || !itemTarget->GetTemplate()->DisenchantID) return;
...
void Spell::EffectFeedPet(SpellEffIndex effIndex) {
  ...
  int32 benefit = pet->GetCurrentFoodBenefitLevel(
      foodItem->GetTemplate()->GetBaseItemLevel());
  if (benefit <= 0) return;
...
void Spell::EffectDismissPet(SpellEffIndex effIndex) {
  ...
  if (!unitTarget || !unitTarget->IsPet()) return;
...
void Spell::EffectSummonObject(SpellEffIndex effIndex) {
  ...
  uint8 slot =
      m_spellInfo->Effects[effIndex].Effect - SPELL_EFFECT_SUMMON_OBJECT_SLOT1;

  if (ObjectGuid guid = m_caster->m_ObjectSlot[slot]) {
    if (GameObject* obj = m_caster->GetMap()->GetGameObject(guid)) {
      // Recast case - null spell id to make auras not be removed on object
      // remove from world
  ...
  if (m_targets.HasDst()) destTarget->GetPosition(x, y, z, o);
  ...
  if (!go->Create(map->GenerateLowGuid<HighGuid::GameObject>(), go_id, map,
                  Position(x, y, z, o), QuaternionData(), 255,
                  GO_STATE_READY)) {
    delete go;
    return;
  }
...
void Spell::EffectLeap(SpellEffIndex /*effIndex*/) {
  ...
  unitTarget->NearTeleportTo(pos.GetPositionX(), pos.GetPositionY(),
...
void Spell::EffectReputation(SpellEffIndex effIndex) {
  ...
  repChange = player->CalculateReputationGain(REPUTATION_SOURCE_SPELL, 0,
                                              repChange, factionId);
...
void Spell::EffectQuestComplete(SpellEffIndex effIndex) {
  ...
  if (questId) {
    ...
    else if (quest->HasFlag(
                 QUEST_FLAGS_TRACKING))   // Check if the quest is used as a
                                          // serverside flag.
      player->SetRewardedQuest(questId);  // If so, set status to rewarded
                                          // without broadcasting it to client.
  }
}
...
void Spell::EffectForceDeselect(SpellEffIndex /*effIndex*/) {
  ...
  for (Unit::AttackerSet::const_iterator itr = m_caster->getAttackers().begin();
       itr != m_caster->getAttackers().end(); ++itr)
...
void Spell::EffectSelfResurrect(SpellEffIndex effIndex) {
  ...
  if (!m_caster || m_caster->IsAlive()) return;
  if (m_caster->GetTypeId() != TYPEID_PLAYER) return;
  if (!m_caster->IsInWorld()) return;
...
void Spell::EffectCharge(SpellEffIndex effIndex) {
  ...
  if (effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH_TARGET) {
    ...
    float speed = G3D::fuzzyGt(m_spellInfo->Speed, 0.0f) ? m_spellInfo->Speed
                                                         : SPEED_CHARGE;
    // Spell is not using explicit target - no generated path
    if (!m_preGeneratedPath) {
      // unitTarget->GetContactPoint(m_caster, pos.m_positionX, pos.m_positionY,
      // pos.m_positionZ);
      Position pos = unitTarget->GetFirstCollisionPosition(
          unitTarget->GetCombatReach(), unitTarget->GetRelativeAngle(m_caster));
      m_caster->GetMotionMaster()->MoveCharge(pos.m_positionX, pos.m_positionY,
                                              pos.m_positionZ, speed);
    } else
...
void Spell::EffectChargeDest(SpellEffIndex effIndex) {
  ...
  if (effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH) {
    ...
    if (!m_caster->IsWithinLOS(pos.GetPositionX(), pos.GetPositionY(),
                               pos.GetPositionZ())) {
      float angle =
          m_caster->GetRelativeAngle(pos.GetPositionX(), pos.GetPositionY());
    ...
    if (int32 spellId = GetSpellInfo()->Effects[effIndex].TriggerSpell)
      m_caster->CastSpell(nullptr, spellId,
                          CastSpellExtraArgs(TRIGGERED_FULL_MASK)
                              .SetOriginalCaster(m_originalCasterGUID));
}
...
void Spell::EffectKnockBack(SpellEffIndex effIndex) {
  ...
  if (m_caster->GetTypeId() == TYPEID_PLAYER ||
      m_caster->GetOwnerOrCreatorGUID().IsPlayer() || m_caster->IsHunterPet())
    if (Creature* creatureTarget = unitTarget->ToCreature())
      if (creatureTarget->isWorldBoss() || creatureTarget->IsDungeonBoss())
        return;

  // Spells with SPELL_EFFECT_KNOCK_BACK (like Thunderstorm) can't knockback
  // target if target has ROOT/STUN
  if (unitTarget->HasUnitState(UNIT_STATE_ROOT | UNIT_STATE_STUNNED)) return;
  ...
  if (speedxy == 0.0f && speedz < 0.01f) return;
...
void Spell::EffectQuestClear(SpellEffIndex effIndex) {
  ...
  if (player->GetQuestStatus(quest_id) == QUEST_STATUS_NONE) return;
...
void Spell::EffectPullTowards(SpellEffIndex effIndex) {
  ...
  Position pos = m_caster->GetFirstCollisionPosition(
      m_caster->GetCombatReach(), m_caster->GetRelativeAngle(unitTarget));

  // This is a blizzlike mistake: this should be 2D distance according to
  // projectile motion formulas, but Blizzard erroneously used 3D distance.
  float distXY = unitTarget->GetExactDist(pos);

  // Avoid division by 0
  if (distXY < 0.001) return;

  float distZ = pos.GetPositionZ() - unitTarget->GetPositionZ();
  float speedXY = m_spellInfo->Effects[effIndex].MiscValue
                      ? m_spellInfo->Effects[effIndex].MiscValue / 10.0f
                      : 30.0f;
  float speedZ =
  ...
  if (!std::isfinite(speedZ)) {
    LOG_ERROR(
        "spells",
        "Spell %u with SPELL_EFFECT_PULL_TOWARDS called with invalid speedZ.",
        m_spellInfo->Id);
    return;
  }
...
void Spell::EffectPullTowardsDest(SpellEffIndex effIndex) {
  ...
  if (!m_targets.HasDst()) {
    LOG_ERROR("spells",
              "Spell %u with SPELL_EFFECT_PULL_TOWARDS_DEST has no dest target",
              m_spellInfo->Id);
    return;
  }
  ...
  if (!std::isfinite(speedZ)) {
    LOG_ERROR("spells",
              "Spell %u with SPELL_EFFECT_PULL_TOWARDS_DEST called with "
              "invalid speedZ",
              m_spellInfo->Id);
    return;
  }
...
void Spell::EffectDispelMechanic(SpellEffIndex effIndex) {
  ...
  for (auto itr = dispel_list.begin(); itr != dispel_list.end(); ++itr)
    unitTarget->RemoveAura(itr->first, itr->second, 0,
                           AuraRemoveFlags::ByEnemySpell);
}
...
void Spell::EffectResurrectPet(SpellEffIndex /*effIndex*/) {
  ...
  if (hadPet) {
    ...
    player->MovePositionToFirstCollision(pos, DEFAULT_FOLLOW_DISTANCE_PET,
                                         float(M_PI_2));
    pet->NearTeleportTo(pos.GetPositionX(), pos.GetPositionY(),
                        pos.GetPositionZ(), player->GetOrientation());
    pet->Relocate(
...
void Spell::EffectDurabilityDamage(SpellEffIndex effIndex) {
  ...
  if (Item* item =
          unitTarget->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, slot)) {
    unitTarget->ToPlayer()->DurabilityPointsLoss(item, damage);
    ExecuteLogEffectDurabilityDamage(effIndex, unitTarget, item->GetEntry(),
                                     slot);
  }
}
...
void Spell::EffectDurabilityDamagePCT(SpellEffIndex effIndex) {
  ...
  if (slot < 0) {
    unitTarget->ToPlayer()->DurabilityLossAll(float(damage) / 100.0f,
                                              (slot < -1));
    return;
  }
...
void Spell::EffectTransmitted(SpellEffIndex effIndex) {
  ...
  if (!goinfo) {
    LOG_ERROR("sql.sql",
              "Gameobject (Entry: %u) does not exist and is not created by "
              "spell (ID: %u) cast.",
  ...
  if (m_targets.HasDst()) destTarget->GetPosition(fx, fy, fz, fo);
  ...
  if (!pGameObj->Create(cMap->GenerateLowGuid<HighGuid::GameObject>(), name_id,
                        cMap, Position(fx, fy, fz, m_caster->GetOrientation()),
  ...
  switch (goinfo->type) {
    case GAMEOBJECT_TYPE_FISHINGNODE: {
      ...
      // channeling duration
      duration =
          std::min(duration, duration - lastSec * IN_MILLISECONDS +
    ...
    case GAMEOBJECT_TYPE_GUILD_BANK:
      if (m_caster->IsPlayer()) {
        if (ObjectGuid guildGUID = m_caster->GetGuidValue(OBJECT_FIELD_DATA)) {
          pGameObj->SetGuidValue(OBJECT_FIELD_DATA,
                                 m_caster->GetGuidValue(OBJECT_FIELD_DATA));
...
void Spell::EffectProspecting(SpellEffIndex /*effIndex*/) {
  ...
  if (!itemTarget ||
      !(itemTarget->GetTemplate()->GetFlags() & ITEM_FLAG_IS_PROSPECTABLE))
    return;
...
void Spell::EffectMilling(SpellEffIndex /*effIndex*/) {
  ...
  if (!itemTarget ||
      !(itemTarget->GetTemplate()->GetFlags() & ITEM_FLAG_IS_MILLABLE))
    return;
...
void Spell::EffectStealBeneficialBuff(SpellEffIndex effIndex) {
  ...
  for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end();
    ...
    if ((aura->GetSpellInfo()->GetDispelMask()) & dispelMask) {
      ...
      if (!aurApp->IsPositive() || aura->IsPassive() ||
          aura->GetSpellInfo()->HasAttribute(SPELL_ATTR4_NOT_STEALABLE))
        continue;

      // 2.4.3 Patch Notes: "Dispel effects will no longer attempt to remove
      // effects that have 100% dispel resistance."
      int32 chance = aura->CalcDispelChance(
          unitTarget, !unitTarget->IsFriendlyTo(m_caster));
      if (!chance) continue;

      // The charges / stack amounts don't count towards the total number of
      // auras that can be dispelled. Ie: A dispel on a target with 5 stacks of
      // Winters Chill and a Polymorph has 1 / (1 + 1) -> 50% chance to dispell
      // Polymorph instead of 1 / (5 + 1) -> 16%.
      bool dispelCharges =
          aura->GetSpellInfo()->HasAttribute(SPELL_ATTR7_DISPEL_CHARGES);
      uint8 charges =
          dispelCharges ? aura->GetCharges() : aura->GetStackAmount();
      if (charges > 0) stealList.emplace_back(aura, chance, charges);
    }
  }

  if (stealList.empty()) return;
  ...
  for (int32 count = 0; count < damage && remaining > 0;) {
    ...
    if (itr->RollDispel()) {
      successList.emplace_back(itr->GetAura()->GetId(),
                               itr->GetAura()->GetCasterGUID());
      if (!itr->DecrementCharge()) {
        --remaining;
...
void Spell::EffectKillCredit(SpellEffIndex effIndex) {
  ...
  if (creatureEntry)
    unitTarget->ToPlayer()->RewardPlayerAndGroupAtEvent(creatureEntry,
                                                        unitTarget);
}
...
void Spell::EffectQuestStart(SpellEffIndex effIndex) {
  ...
  if (Quest const* quest = sObjectMgr->GetQuestTemplate(
          m_spellInfo->Effects[effIndex].MiscValue)) {
    if (!player->CanTakeQuest(quest, false)) return;
    ...
    if (quest->IsAutoAccept() && player->CanAddQuest(quest, false)) {
      ...
      player->PlayerTalkClass->SendQuestGiverQuestDetails(
          quest, player->GetGUID(), true, true);
    } else {
...
void Spell::EffectActivateRune(SpellEffIndex effIndex) {
  ...
  if (m_caster->GetTypeId() != TYPEID_PLAYER || !damage) return;
  ...
  if (miscValue & (1 << RUNE_DEATH)) miscValue |= (1 << RUNE_BLOOD);

  for (uint32 i = 0; i < MAX_RUNES && count > 0; ++i) {
    if ((1 << player->GetCurrentRune(i)) & miscValue &&
...
void Spell::EffectCreateTamedPet(SpellEffIndex effIndex) {
  ...
  if (!unitTarget || unitTarget->GetTypeId() != TYPEID_PLAYER ||
      unitTarget->GetPetGUID() || unitTarget->getClass() != CLASS_HUNTER)
    return;
  ...
  unitTarget->MovePositionToFirstCollision(pos, DEFAULT_FOLLOW_DISTANCE_PET,
                                           float(M_PI_2));
  pet->Relocate(pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(),
...
void Spell::EffectTitanGrip(SpellEffIndex effIndex) {
  ...
  if (m_caster->GetTypeId() == TYPEID_PLAYER)
    m_caster->ToPlayer()->SetCanTitanGrip(
        true, m_spellInfo->Effects[effIndex].MiscValue);
}
...
void Spell::EffectRedirectThreat(SpellEffIndex /*effIndex*/) {
  ...
  if (unitTarget)
    m_caster->GetThreatManager().RegisterRedirectThreat(
        m_spellInfo->Id, unitTarget->GetGUID(), uint32(damage));
}
...
void Spell::EffectGameObjectDamage(SpellEffIndex /*effIndex*/) {
  ...
  FactionTemplateEntry const* targetFaction =
      sFactionTemplateStore.LookupEntry(gameObjTarget->GetFaction());
  // Do not allow to damage GO's of friendly factions (ie: Wintergrasp
  // Walls/Ulduar Storm Beacons)
  if (!targetFaction ||
      (casterFaction && !casterFaction->IsFriendlyTo(*targetFaction)))
...
void Spell::EffectGameObjectSetDestructionState(SpellEffIndex effIndex) {
  ...
  if (!gameObjTarget || !m_originalCaster) return;
...
void Spell::EffectRenamePet(SpellEffIndex /*effIndex*/) {
  ...
  unitTarget->SetByteFlag(UNIT_FIELD_BYTES_2, UNIT_BYTES_2_OFFSET_PET_FLAGS,
                          UNIT_CAN_BE_RENAMED);
}
...
void Spell::EffectPlayMusic(SpellEffIndex effIndex) {
  ...
  if (!sSoundEntriesStore.LookupEntry(soundId)) {
    LOG_ERROR("spells",
              "EffectPlayMusic: Sound (Id: %u) does not exist in spell %u.",
              soundId, m_spellInfo->Id);
    return;
  }

  unitTarget->ToPlayer()->SendDirectMessage(
      WorldPackets::Misc::PlayMusic(soundId, unitTarget->GetGUID()).Write());
}
...
void Spell::EffectActivateSpec(SpellEffIndex /*effIndex*/) {
  ...
  unitTarget->ToPlayer()->ActivateSpec(damage -
                                       1);  // damage is 1 or 2, spec is 0 or 1
}
...
void Spell::EffectPlaySound(SpellEffIndex effIndex) {
  ...
  if (!sSoundEntriesStore.LookupEntry(soundId)) {
    LOG_ERROR("spells",
              "EffectPlaySound: Sound (Id: %u) does not exist in spell %u.",
              soundId, m_spellInfo->Id);
    return;
  }
...
void Spell::EffectGiveCurrency(SpellEffIndex effIndex) {
  ...
  unitTarget->ToPlayer()->ModifyCurrency(
      m_spellInfo->Effects[effIndex].MiscValue, damage);
}
...
void Spell::EffectCastButtons(SpellEffIndex effIndex) {
  ...
  for (; n_buttons; --n_buttons, ++button_id) {
    ...
    if (!p_caster->HasSpell(spell_id) ||
        p_caster->GetSpellHistory()->HasCooldown(spell_id))
      continue;

    if (!spellInfo->HasAttribute(SPELL_ATTR9_SUMMON_PLAYER_TOTEM)) continue;

    int32 cost = spellInfo->CalcPowerCost(m_caster, spellInfo->GetSchoolMask());
    if (m_caster->GetPower(POWER_MANA) < cost) continue;
...
void Spell::EffectBind(SpellEffIndex effIndex) {
  ...
  LOG_DEBUG(
      ...
      homeLoc.GetPositionX(), homeLoc.GetPositionY(), homeLoc.GetPositionZ(),
...
void Spell::EffectUnlockGuildVaultTab(SpellEffIndex effIndex) {
  ...
  if (Guild* guild = caster->GetGuild())
    guild->HandleBuyBankTab(caster->GetSession(),
                            m_spellInfo->Effects[effIndex].BasePoints -
                                1);  // Bank tabs start at zero internally
}
...
void Spell::EffectSummonPersonalGameObject(SpellEffIndex effIndex) {
  ...
  if (!go->Create(map->GenerateLowGuid<HighGuid::GameObject>(), goId, map, pos,
                  rot, 255, GO_STATE_READY)) {
    LOG_WARN("spells",
             "SpellEffect Failed to summon personal gameobject. SpellId %u, "
             "effect %u",
             m_spellInfo->Id, effIndex);
    delete go;
    return;
...
void Spell::EffectResurrectWithAura(SpellEffIndex effIndex) {
  ...
  if (resurrectAura && player->HasAura(resurrectAura)) return;
...
void Spell::EffectCreateAreaTrigger(SpellEffIndex effIndex) {
  ...
  // trigger entry/miscvalue relation is currently unknown, for now use
  // MiscValue as trigger entry
  uint32 triggerEntry = GetSpellInfo()->Effects[effIndex].MiscValue;

  AreaTrigger* areaTrigger = new AreaTrigger();
  if (!areaTrigger->CreateAreaTrigger(
          m_caster->GetMap()->GenerateLowGuid<HighGuid::AreaTrigger>(),
          triggerEntry, GetCaster(), GetSpellInfo(), pos))
    delete areaTrigger;
}
