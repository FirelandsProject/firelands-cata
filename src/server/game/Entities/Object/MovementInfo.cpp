...
void MovementInfo::OutDebug()
{
    ...
    LOG_DEBUG("misc", "time %u current time " UI64FMTD "", flags2, uint64(::GameTime::GetGameTime()));
    ...
    if (transport.guid)
    {
        ...
        if (transport.vehicleId)
            LOG_INFO("misc", "vehicleId: %u", transport.vehicleId);
    }
    ...
    {
        LOG_INFO("misc", "fallTime: %u j_zspeed: %f", jump.fallTime, jump.zspeed);
