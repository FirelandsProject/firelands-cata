...
uint32 PlayerSocial::GetNumberOfSocialsWithFlag(SocialFlag flag) {
  uint32 counter = 0;
  for (PlayerSocialMap::const_iterator itr = _playerSocialMap.begin();
       itr != _playerSocialMap.end(); ++itr)
    if ((itr->second.Flags & flag) != 0) ++counter;

  return counter;
}

bool PlayerSocial::AddToSocialList(ObjectGuid const& friendGuid,
                                   SocialFlag flag) {
  // check client limits
  ...
  if (itr != _playerSocialMap.end()) {
    ...
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_UPD_CHARACTER_SOCIAL_FLAGS);
...
void PlayerSocial::RemoveFromSocialList(ObjectGuid const& friendGuid,
                                        SocialFlag flag) {
  PlayerSocialMap::iterator itr = _playerSocialMap.find(friendGuid);
  if (itr == _playerSocialMap.end()) return;
...
void PlayerSocial::SendSocialList(Player* player, uint32 flags) {
  ...
  WorldPacket data(
      SMSG_CONTACT_LIST,
      (4 + 4 + _playerSocialMap.size() * 25));  // just can guess size
  ...
  for (auto& v : _playerSocialMap) {
    if (!(v.second.Flags & flags)) continue;
    ...
    if (count >= (((flags & SOCIAL_FLAG_FRIEND) != 0) ? SOCIALMGR_FRIEND_LIMIT
                                                      : SOCIALMGR_IGNORE_LIMIT))
      break;
  }
...
bool PlayerSocial::_HasContact(ObjectGuid const& guid, SocialFlag flags) {
  ...
  if (itr != _playerSocialMap.end()) return (itr->second.Flags & flags) != 0;
...
void SocialMgr::GetFriendInfo(Player* player, ObjectGuid const& friendGUID,
  ...
  PlayerSocial::PlayerSocialMap::iterator itr =
      player->GetSocial()->_playerSocialMap.find(friendGUID);
  if (itr != player->GetSocial()->_playerSocialMap.end())
    friendInfo.Note = itr->second.Note;

  // PLAYER see his team only and PLAYER can't see MODERATOR, GAME MASTER,
  // ADMINISTRATOR characters MODERATOR, GAME MASTER, ADMINISTRATOR can see all

  if (!AccountMgr::IsPlayerAccount(security) &&
      target->GetSession()->GetSecurity() >
  ...
  if (target->IsVisibleGloballyFor(player)) {
    if (target->isDND())
      ...
      if (target->GetSession()->GetRecruiterId() ==
              player->GetSession()->GetAccountId() ||
          target->GetSession()->GetAccountId() ==
              player->GetSession()->GetRecruiterId())
        friendInfo.Status =
            FriendStatus(uint32(friendInfo.Status) | FRIEND_STATUS_RAF);
    }
...
void SocialMgr::BroadcastToFriendListers(Player* player,
  ...
  AccountTypes gmSecLevel =
      AccountTypes(sWorld->getIntConfig(CONFIG_GM_LEVEL_IN_WHO_LIST));
  for (SocialMap::const_iterator itr = _socialMap.begin();
       itr != _socialMap.end(); ++itr) {
    PlayerSocial::PlayerSocialMap::const_iterator itr2 =
        itr->second._playerSocialMap.find(player->GetGUID());
    if (itr2 != itr->second._playerSocialMap.end() &&
        (itr2->second.Flags & SOCIAL_FLAG_FRIEND) != 0) {
...
PlayerSocial* SocialMgr::LoadFromDB(PreparedQueryResult result,
  ...
  if (result) {
    do {
      ...
      social->_playerSocialMap[friendGuid] =
          FriendInfo(flag, fields[2].GetString());
