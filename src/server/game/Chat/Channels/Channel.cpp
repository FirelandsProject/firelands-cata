...
Channel::Channel(uint32 channelId, uint32 team /*= 0*/,
  ...
  ChatChannelsEntry const* channelEntry =
      sChatChannelsStore.AssertEntry(channelId);
  if (channelEntry->Flags & CHANNEL_DBC_FLAG_TRADE)  // for trade channel
    _channelFlags |= CHANNEL_FLAG_TRADE;

  if (channelEntry->Flags &
      CHANNEL_DBC_FLAG_CITY_ONLY2)  // for city only channels
...
Channel::Channel(std::string const& name, uint32 team /*= 0*/)
    : _announceEnabled(true),
      ...
      _channelPassword(),
      _zoneEntry(nullptr) {
  // If storing custom channels in the db is enabled either load or save the
  // channel
  if (sWorld->getBoolConfig(CONFIG_PRESERVE_CUSTOM_CHANNELS)) {
    CharacterDatabasePreparedStatement* stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHANNEL);
    stmt->setString(0, name);
    stmt->setUInt32(1, _channelTeam);
    if (PreparedQueryResult result = CharacterDatabase.Query(stmt))  // load
    {
      Field* fields = result->Fetch();
      _channelName = fields[0].GetString();  // re-get channel name. MySQL table
        ...
        for (auto const& token : tokens) {
          ...
          if (banned_guid) {
            LOG_DEBUG("chat.system",
                      "Channel(%s) loaded player %s into bannedStore",
                      name.c_str(), banned_guid.ToString().c_str());
...
void Channel::GetChannelName(std::string& channelName, uint32 channelId,
  ...
  if (channelId) {
    ...
    if (!(channelEntry->Flags & CHANNEL_DBC_FLAG_GLOBAL)) {
      if (channelEntry->Flags & CHANNEL_DBC_FLAG_CITY_ONLY)
        channelName = Firelands::StringFormat(
            channelEntry->Name,
            sObjectMgr->GetFirelandsString(LANG_CHANNEL_CITY, locale));
      else
        channelName = Firelands::StringFormat(
            channelEntry->Name, ASSERT_NOTNULL(zoneEntry)->AreaName);
    } else
...
void Channel::UpdateChannelInDB() const {
  if (_persistentChannel) {
    ...
    for (BannedContainer::const_iterator iter = _bannedStore.begin();
         iter != _bannedStore.end(); ++iter)
    ...
    LOG_DEBUG("chat.system", "Channel(%s) updated in database",
              _channelName.c_str());
  }
}
...
void Channel::CleanOldChannelsInDB() {
  if (sWorld->getIntConfig(CONFIG_PRESERVE_CUSTOM_CHANNEL_DURATION) > 0) {
    ...
    stmt->setUInt32(
        0, sWorld->getIntConfig(CONFIG_PRESERVE_CUSTOM_CHANNEL_DURATION) * DAY);
    CharacterDatabase.Execute(stmt);
...
void Channel::JoinChannel(Player* player, std::string const& pass) {
  ...
  if (_announceEnabled &&
      (!AccountMgr::IsGMAccount(player->GetSession()->GetSecurity()) ||
  ...
  if (!IsConstant()) {
    ...
    // owner. or if the owner was a GM with .gm visible off don't do this if the
    // new player is, too, an invis GM, unless the channel was empty
    if (_ownershipEnabled && (newChannel || !pinfo.IsInvisible()) &&
        (!_ownerGuid || _isOwnerInvisible)) {
...
void Channel::LeaveChannel(Player* player, bool send) {
  ...
  if (!IsConstant()) {
    ...
    // If the channel owner left and there are players still inside, pick a new
    // owner do not pick invisible gm owner unless there are only invisible gms
    // in that channel (rare)
    if (changeowner && _ownershipEnabled && !_playersStore.empty()) {
      ...
      for (itr = _playersStore.begin(); itr != _playersStore.end(); ++itr) {
        if (!itr->second.IsInvisible()) break;
      }

      if (itr == _playersStore.end()) itr = _playersStore.begin();
      ...
      // if the new owner is invisible gm, set flag to automatically choose a
      // new owner
      if (itr->second.IsInvisible()) _isOwnerInvisible = true;
    }
  }
}

void Channel::KickOrBan(Player const* player, std::string const& badname,
                        bool ban) {
  ...
  if (!info.IsModerator() && !AccountMgr::IsGMAccount(sec)) {
...
void Channel::SetMode(Player const* player, std::string const& p2n, bool mod,
                      bool set) {
  uint32 sec = player->GetSession()->GetSecurity();
  ObjectGuid guid = player->GetGUID();
  ...
  if (!victim || !IsOn(victim) ||
      ...
      // team
      ((!AccountMgr::IsGMAccount(sec) ||
        !AccountMgr::IsGMAccount(newp->GetSession()->GetSecurity())) &&
...
void Channel::SetInvisible(Player const* player, bool on) {
  ...
  if (itr == _playersStore.end()) return;

  itr->second.SetInvisible(on);

  // we happen to be owner too, update flag
  if (_ownerGuid == player->GetGUID()) _isOwnerInvisible = on;
}
...
void Channel::List(Player const* player) const {
  ...
  WorldPacket data(SMSG_CHANNEL_LIST, 1 + (channelName.size() + 1) + 1 + 4 +
                                          _playersStore.size() * (8 + 1));
  ...
  uint32 count = 0;
  for (PlayerContainer::const_iterator i = _playersStore.begin();
       i != _playersStore.end(); ++i) {
...
void Channel::Say(ObjectGuid guid, std::string const& what, uint32 lang) const {
  ...
  auto builder = [&](WorldPacket& data, LocaleConstant locale) {
    ...
    if (Player* player = ObjectAccessor::FindConnectedPlayer(guid))
      ChatHandler::BuildChatPacket(data, CHAT_MSG_CHANNEL, Language(lang),
                                   player, player, what, 0, GetName(localeIdx));
    else
      ChatHandler::BuildChatPacket(data, CHAT_MSG_CHANNEL, Language(lang), guid,
                                   guid, what, 0, "", "", 0, false,
                                   GetName(localeIdx));
  };
...
void Channel::SetOwner(ObjectGuid guid, bool exclaim) {
  if (_ownerGuid) {
    ...
    if (itr != _playersStore.end()) itr->second.SetOwner(false);
  }
...
void Channel::JoinNotify(ObjectGuid guid) const {
  auto builder = [&](WorldPacket& data, LocaleConstant locale) {
    ...
    data.Initialize(IsConstant() ? SMSG_USERLIST_ADD : SMSG_USERLIST_UPDATE,
                    8 + 1 + 1 + 4 + 30 /*channelName buffer*/);
    data << uint64(guid);
...
void Channel::LeaveNotify(ObjectGuid guid) const {
  auto builder = [&](WorldPacket& data, LocaleConstant locale) {
    ...
    data.Initialize(SMSG_USERLIST_REMOVE,
                    8 + 1 + 4 + 30 /*channelName buffer*/);
    data << uint64(guid);
    data << uint8(GetFlags());
    data << uint32(GetNumPlayers());
    data << GetName(localeIdx);
