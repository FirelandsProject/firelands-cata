...
std::vector<ChatCommand> const& ChatHandler::getCommandTable() {
  if (!commandTableCache) InitializeCommandTable();
...
void ChatHandler::InitializeCommandTable() {
  ...
  if (result) {
    do {
      ...
      SetDataForCommandInTable(*commandTableCache, name.c_str(),
                               fields[1].GetUInt16(), fields[2].GetString(),
                               name);
    } while (result->NextRow());
  }
}

void ChatHandler::invalidateCommandTable() { commandTableCache.reset(); }
...
bool ChatHandler::isAvailable(ChatCommand const& cmd) const {
  return IsConsole() ? true
                     : m_session->GetSecurity() >= AccountTypes(cmd.Permission);
}
...
bool ChatHandler::HasLowerSecurity(Player* target, ObjectGuid guid,
                                   bool strong) {
...
bool ChatHandler::HasLowerSecurityAccount(WorldSession* target,
                                          uint32 target_account, bool strong) {
  uint32 target_sec;

  // allow everything from console and RA console
  if (!m_session) return false;

  // ignore only for non-players for non strong checks (when allow apply command
  // at least to same sec level)
...
void ChatHandler::SendSysMessage(const char* str, bool escapeCharacters) {
  ...
  while (char* line = LineFromMessage(pos)) {
    BuildChatPacket(data, CHAT_MSG_SYSTEM, LANG_UNIVERSAL, nullptr, nullptr,
                    line);
...
bool ChatHandler::ExecuteCommandInTable(std::vector<ChatCommand> const& table,
  ...
  for (uint32 i = 0; i < table.size(); ++i) {
    if (!hasStringAbbr(table[i].Name, cmd.c_str())) continue;

    bool match = false;
    if (strlen(table[i].Name) > cmd.length()) {
      for (uint32 j = 0; j < table.size(); ++j) {
        if (!hasStringAbbr(table[j].Name, cmd.c_str())) continue;
    ...
    if (!table[i].HasHandler() || !isAvailable(table[i])) continue;
    ...
    if (table[i](this, table[i].Name[0] != '\0' ? text : oldtext)) {
      ...
      if (!AccountMgr::IsPlayerAccount(m_session->GetSecurity())) {
        ...
        sLog->outCommand(
            ...
            "Command: %s [Player: %s (%s) (Account: %u) X: %f Y: %f Z: %f Map: "
            "%u (%s) Area: %u (%s) Zone: %s Selected: %s (%s)]",
            fullcmd.c_str(), player->GetName().c_str(),
            player->GetGUID().ToString().c_str(), m_session->GetAccountId(),
            player->GetPositionX(), player->GetPositionY(),
            player->GetPositionZ(), player->GetMapId(),
            player->FindMap() ? player->FindMap()->GetMapName() : "Unknown",
            areaId, areaName.c_str(), zoneName.c_str(),
            (player->GetSelectedUnit())
                ? player->GetSelectedUnit()->GetName().c_str()
                : "",
            guid.ToString().c_str());
      }
    }
    // some commands have custom error messages. Don't send the default one in
    // these cases.
...
bool ChatHandler::SetDataForCommandInTable(std::vector<ChatCommand>& table,
                                           char const* text, uint32 permission,
  ...
  for (uint32 i = 0; i < table.size(); i++) {
    ...
    if (table[i].Name != cmd) continue;

    // select subcommand from child commands list (including "")
    if (!table[i].ChildCommands.empty()) {
      if (SetDataForCommandInTable(table[i].ChildCommands, text, permission,
                                   help, fullcommand))
        return true;
      else if (*text)
        return false;
    ...
    // expected subcommand by full name DB content
      ...
      LOG_ERROR("sql.sql",
                "Table `command` contains an unexpected subcommand '%s' in "
                "command '%s', skipped.",
                text, fullcommand.c_str());
      return false;
    }

    if (table[i].Permission != permission)
      LOG_INFO("misc",
               "Table `command` overwrite for command '%s' default permission "
               "(%u) by %u",
               fullcommand.c_str(), table[i].Permission, permission);
  ...
  if (!cmd.empty()) {
    if (&table == &getCommandTable())
      LOG_ERROR(
          "sql.sql",
          "Table `command` contains a non-existing command '%s', skipped.",
          cmd.c_str());
    else
      LOG_ERROR("sql.sql",
                "Table `command` contains a non-existing subcommand '%s' in "
                "command '%s', skipped.",
                cmd.c_str(), fullcommand.c_str());
  }
...
bool ChatHandler::_ParseCommands(char const* text) {
  if (ExecuteCommandInTable(getCommandTable(), text, text)) return true;
...
bool ChatHandler::isValidChatMessage(char const* message) {
  ...
  |cffffff00|Hachievement:546:0000000000000001:0:0:0:-1:0:0:0:0|h[Safe
  Deposit]|h|r |cff66bbff|Hglyph:21:762|h[Glyph of Bladestorm]|h|r

  | will be escaped to ||
  */

  if (strlen(message) > 255) return false;
  ...
  if (sWorld->getIntConfig(CONFIG_CHAT_STRICT_LINK_CHECKING_SEVERITY) < 3) {
    ...
    while (*message) {
      ...
      if (validCommands.find(commandChar) == std::string::npos) return false;
...
bool ChatHandler::ShowHelpForSubCommands(std::vector<ChatCommand> const& table,
                                         char const* cmd, char const* subcmd) {
  std::string list;
  for (uint32 i = 0; i < table.size(); ++i) {
    // must be available (ignore handler existence to show command with possible
    // available subcommands)
    if (!isAvailable(table[i])) continue;

    // for empty subcmd show all available
    if (*subcmd && !hasStringAbbr(table[i].Name, subcmd)) continue;
    ...
    if (!table[i].ChildCommands.empty()) list += " ...";
  }
...
bool ChatHandler::ShowHelpForCommand(std::vector<ChatCommand> const& table,
  ...
  if (*cmd) {
    for (uint32 i = 0; i < table.size(); ++i) {
      // must be available (ignore handler existence to show command with
      // possible available subcommands)
      if (!isAvailable(table[i])) continue;

      if (!hasStringAbbr(table[i].Name, cmd)) continue;

      // have subcommand
      char const* subcmd = (*cmd) ? strtok(nullptr, " ") : "";

      if (!table[i].ChildCommands.empty() && subcmd && *subcmd) {
        if (ShowHelpForCommand(table[i].ChildCommands, subcmd)) return true;
      }

      if (!table[i].Help.empty()) SendSysMessage(table[i].Help.c_str());

      if (!table[i].ChildCommands.empty())
        if (ShowHelpForSubCommands(table[i].ChildCommands, table[i].Name,
                                   subcmd ? subcmd : ""))
          return true;
    ...
    for (uint32 i = 0; i < table.size(); ++i) {
      ...
      if (strlen(table[i].Name)) continue;

      if (!table[i].Help.empty()) SendSysMessage(table[i].Help.c_str());

      if (!table[i].ChildCommands.empty())
        if (ShowHelpForSubCommands(table[i].ChildCommands, "", "")) return true;
...
size_t ChatHandler::BuildChatPacket(
    WorldPacket& data, ChatMsg chatType, Language language,
    ObjectGuid senderGUID, ObjectGuid receiverGUID, std::string const& message,
    uint8 chatTag, std::string const& senderName /*= ""*/,
    std::string const& receiverName /*= ""*/, uint32 achievementId /*= 0*/,
    bool gmMessage /*= false*/, std::string const& channelName /*= ""*/,
  ...
  switch (chatType) {
    ...
    case CHAT_MSG_RAID_BOSS_WHISPER:
      ...
      if (language == LANG_ADDON) data << addonPrefix;
      break;
  ...
  else if (chatType == CHAT_MSG_RAID_BOSS_WHISPER ||
           chatType == CHAT_MSG_RAID_BOSS_EMOTE) {
    data << float(0.0f);  // Display time in middle of the screen (in seconds),
                          // defaults to 10 if not set (cannot be below 1)
    data << uint8(
        0);  // Hide in chat frame (only shows in middle of the screen)
  }

  return receiverGUIDPos;
}

size_t ChatHandler::BuildChatPacket(WorldPacket& data, ChatMsg chatType,
                                    Language language,
  ...
  return BuildChatPacket(data, chatType, language, senderGUID, receiverGUID,
                         message, chatTag, senderName, receiverName,
                         achievementId, gmMessage, channelName, addonPrefix);
}
...
Player* ChatHandler::getSelectedPlayer() {
  ...
  if (!selected) return m_session->GetPlayer();
...
WorldObject* ChatHandler::getSelectedObject() {
  ...
  if (!guid) return GetNearbyGameObject();
...
Player* ChatHandler::getSelectedPlayerOrSelf() {
  ...
  if (!targetPlayer) targetPlayer = m_session->GetPlayer();
...
char* ChatHandler::extractKeyFromLink(char* text, char const* linkType,
  ...
  if (!check) return nullptr;       // end of data

  char* cLinkType = strtok(nullptr, ":");  // linktype
  if (!cLinkType) return nullptr;          // end of data

  if (strcmp(cLinkType, linkType) != 0) {
    strtok(nullptr, " ");  // skip link tail (to allow continue strtok(nullptr,
                           // s) use after retturn from function
...
GameObject* ChatHandler::GetNearbyGameObject() {
  ...
  Firelands::GameObjectLastSearcher<Firelands::NearestGameObjectCheck> searcher(
      pl, obj, check);
  Cell::VisitGridObjects(pl, searcher, SIZE_OF_GRIDS);
  return obj;
}

GameObject* ChatHandler::GetObjectFromPlayerMapByDbGuid(
  ...
  auto bounds = m_session->GetPlayer()
                    ...
                    .equal_range(lowguid);
  if (bounds.first != bounds.second) return bounds.first->second;
  return nullptr;
}

Creature* ChatHandler::GetCreatureFromPlayerMapByDbGuid(
    ObjectGuid::LowType lowguid) {
  if (!m_session) return nullptr;
  // Select the first alive creature or a dead one if not found
  Creature* creature = nullptr;
  auto bounds =
      m_session->GetPlayer()->GetMap()->GetCreatureBySpawnIdStore().equal_range(
          lowguid);
  for (auto it = bounds.first; it != bounds.second; ++it) {
    creature = it->second;
    if (it->second->IsAlive()) break;
  }
...
uint32 ChatHandler::extractSpellIdFromLink(char* text) {
  ...
  // [name] Shift-click form |color|Hglyph:glyph_slot_id:glyph_prop_id|h[%s]|h|r
  // number or [name] Shift-click form |color|Hspell:spell_id|h[name]|h|r
  // number or [name] Shift-click form |color|Htalent:talent_id,
  // rank|h[name]|h|r number or [name] Shift-click form |color|Htrade:spell_id,
  // skill_id, max_value, cur_value|h[name]|h|r
  int type = 0;
...
GameTele const* ChatHandler::extractGameTeleFromLink(char* text) {
  ...
  if (cId[0] >= '0' && cId[0] <= '9')
    if (uint32 id = atoi(cId)) return sObjectMgr->GetGameTele(id);
...
static char const* const guidKeys[] = {"Hplayer", "Hcreature", "Hgameobject",
                                       nullptr};
...
ObjectGuid::LowType ChatHandler::extractLowGuidFromLink(char* text,
  ...
  switch (type) {
    case GUID_LINK_PLAYER: {
      ...
      std::string name = idS;
      if (!normalizePlayerName(name)) return 0;
...
std::string ChatHandler::extractPlayerNameFromLink(char* text) {
  ...
  if (!normalizePlayerName(name)) return "";

  return name;
}

bool ChatHandler::extractPlayerTarget(char* args, Player** player,
  ...
  if (args && *args) {
    ...
    if (player_guid) *player_guid = pl ? pl->GetGUID() : guid;

    if (player_name) *player_name = pl || guid ? name : "";
  } else {
    Player* pl = getSelectedPlayerOrSelf();
    // if allowed player pointer
    if (player) *player = pl;
    // if allowed player guid (if no then only online players allowed)
    if (player_guid) *player_guid = pl ? pl->GetGUID() : ObjectGuid::Empty;

    if (player_name) *player_name = pl ? pl->GetName() : "";
  }

  // some from req. data must be provided (note: name is empty if player does
  // not exist)
...
bool ChatHandler::GetPlayerGroupAndGUIDByName(char const* cname,
  ...
  if (cname) {
    ...
    if (!name.empty()) {
      ...
      if (offline) guid = sCharacterCache->GetCharacterGuidByName(name);
    }
  }

  if (player) {
    group = player->GetGroup();
    if (!guid || !offline) guid = player->GetGUID();
  } else {
...
bool AddonChannelCommandHandler::ParseCommands(char const* str) {
  if (memcmp(str, "Firelands\t", 12)) return false;
  char opcode = str[12];
  if (!opcode)  // str[12] is opcode
    return false;
  if (!str[13] || !str[14] || !str[15] ||
      !str[16])  // str[13] through str[16] is 4-character command counter
    return false;
  echo = str + 13;
...
void AddonChannelCommandHandler::SendSysMessage(char const* str,
  ...
  msg.append(echo, 4);
  std::string body(str);
  if (escapeCharacters) boost::replace_all(body, "|", "||");
  size_t pos, lastpos;
