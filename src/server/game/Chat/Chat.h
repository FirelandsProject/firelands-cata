...
class FC_GAME_API ChatHandler {
 public:
  ...
  static size_t BuildChatPacket(
      WorldPacket& data, ChatMsg chatType, Language language,
      ObjectGuid senderGUID, ObjectGuid receiverGUID,
      std::string const& message, uint8 chatTag,
      std::string const& senderName = "", std::string const& receiverName = "",
      uint32 achievementId = 0, bool gmMessage = false,
      std::string const& channelName = "", std::string const& addonPrefix = "");

  // Builds chat packet and returns receiver guid position in the packet to
  // substitute in whisper builders
  static size_t BuildChatPacket(WorldPacket& data, ChatMsg chatType,
                                Language language, WorldObject const* sender,
  ...
  void PSendSysMessage(char const* fmt, Args&&... args) {
    SendSysMessage(
        Firelands::StringFormat(fmt, std::forward<Args>(args)...).c_str());
  }
  ...
  std::string PGetParseString(uint32 entry, Args&&... args) const {
    return Firelands::StringFormat(GetFirelandsString(entry),
  ...
  bool HasLowerSecurityAccount(WorldSession* target, uint32 account,
                               bool strong = false);
  ...
  char* extractKeyFromLink(char* text, char const* const* linkTypes,
                           int* found_idx, char** something1 = nullptr);
  ...
  bool GetPlayerGroupAndGUIDByName(char const* cname, Player*& player,
                                   Group*& group, ObjectGuid& guid,
                                   bool offline = false);
  std::string extractPlayerNameFromLink(char* text);
  // select by arg (name/link) or in-game selection online/offline player or
  // self if a creature is selected
  bool extractPlayerTarget(char* args, Player** player,
  ...
  bool ShowHelpForCommand(std::vector<ChatCommand> const& table,
                          char const* cmd);

 protected:
  explicit ChatHandler()
      : m_session(nullptr), sentErrorMessage(false) {}  // for CLI subclass
  static bool SetDataForCommandInTable(std::vector<ChatCommand>& table,
                                       char const* text, uint32 permission,
                                       std::string const& help,
                                       std::string const& fullcommand);
  bool ExecuteCommandInTable(std::vector<ChatCommand> const& table,
                             char const* text, std::string const& fullcmd);
  bool ShowHelpForSubCommands(std::vector<ChatCommand> const& table,
