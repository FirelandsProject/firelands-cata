...
namespace MMAP
{
    ...
    void MapBuilder::discoverTiles()
    {
        ...
        for (TileList::iterator itr = m_tiles.begin(); itr != m_tiles.end(); ++itr)
        {
            ...
            files.clear();
            getDirContents(files, "vmaps", Firelands::StringFormat("%03u*.vmtile", mapID));
            for (uint32 i = 0; i < files.size(); ++i)
            {
            ...
            files.clear();
            getDirContents(files, "maps", Firelands::StringFormat("%03u*", mapID));
            for (uint32 i = 0; i < files.size(); ++i)
            {
    ...
    {
        // console output
        std::string tileString = Firelands::StringFormat("[Map %03i] [%02i,%02i]: ", mapID, tileX, tileY);
